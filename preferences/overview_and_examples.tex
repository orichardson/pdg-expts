...
\begin{document}
	...
	\begin{enumerate}[nosep]
		...
		\item Dynamics: A formulation of how to revise preferences due to inconsistencies that may arise as beliefs change.
	\end{enumerate}
...
%		\draw[arr, dashed] (D) to[bend left] (E);
	...
	\subsubsection{Reasons for This Choice of Representation}
	It allows us to:
	\begin{itemize}[nosep]
		\item represent inconsistent preferences
		\item accommodate the formation of new preferences on new domains which 
		\item represent expected utility as a special case of filtering through beliefs
		\item store preferences about common choices so they can be re-used and nudged without fully recomputing everything for each decision
	\end{itemize}
...
%	Moreover, maintaining it as more domains are added to the picture becomes very expensive.
	...
	This differs from a Bayseian Network in several important ways:
	\begin{enumerate}
		\item It encodes additional preference information in the domains.
	
		
		\item Unlike a Bayseian Network, the fact that there are two arrows into $\sf C$ in the diagram above does not mean that there is a distribution $\Pr({\sf C \mid D, E})$, but rather two different ones $\Pr\sf( C \mid E)$ and $\Pr\sf(C \mid D)$. It is possible to recover this meaning if we are allowed to introduce new product nodes:
		\begin{center}
			\begin{tikzpicture}
		...
		\item There is no need to have a probability distribution 
...
%		\item The premises, that we prefer $a_1$ to $a_2$ and $b_2$ to $b_1$, are 
	...
	Suppose our preference on $A$ ($a_1 \geqc a_2$) has an image of 
	\begin{center}
	\begin{tikzpicture}
		\node[dpadded] (A) {$A$};
