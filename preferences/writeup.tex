...
\title{A Theory of Dynamic Preferences and Robot Empathy}
...
%	\subsubsection{Uses}
	...
	The decision of what drink to have does not exist in a vacuum. Suppose you also have to decide what restaurant to go to get lunch $(L)$, and also where to buy appliances $(A)$, and groceries $(G)$. Each of these depend on what drinks you like, among other things. This might look something like the diagram below.
	\begin{center}
		\begin{tikzpicture}
...
%			\sum_{s \in S} \mat B_{i,s} \mat C_{s,j} &= \sum_{s \in S} \mat B_{i,s} \mat C_{s,j}  \\
	...
	\subsection{Dynamics}
	In addition to keeping track of preferences, the whole point is to provide a prescription for how they should change over time. There are several sources of change we consider: we have already vaguely suggested iterative matrix powers and low-rank approximation as methods of compressing data and pushing towards transitivity, as well as simpler forces to push for complete, reflexive, and anti-symmetric preferences. All of these are within a single domain, and would quickly stabilize if left alone (it would take only a single matrix factorization to find a loss-minimizing solution). Our final component, the loss based on inconsistency of the links, is the most important and novel one.
	
	As a first pass, note that if we had a degenerate distribution (a function) $f : X \to Y$, and we had preferences on $Y$ but not $X$, we could define $x_1 \leqc_X x_2 := f(x_1) \leqc_Y f(x_2)$. It may be worth noting that this process of building up preferences backwards, in the reverse direction of the links, is the natural way to form them. In fact, to go the other direction, you need to apply Bayes' rule or compute a pseudo-inverse to get something that can be used the link going the other way. This is because anything we do with probabilities must necessarily be pre-composed with our existing framework; this is the filtering that was mentioned earlier.
	
	\begin{example}
		If the preferences on $Y$ are represented by utilities $u : Y \to \mathbb R$, then the preferences on $X$ induced by $f$ and $Y$ are given by $u \circ f$. 
	...
	For each link, whose conditional distribution we imagine being parameterized by some latent variables $\theta$, we define the inconsistency as 
	\[ \]
