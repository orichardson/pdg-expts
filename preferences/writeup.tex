...
%\newenvironment{idxmat}[1]{
%	\begin{matrix} & \begin{matrix} \footnotesize\noexpandarg\StrSubstitute{#1}{,}{&\footnotesize}\end{matrix} \\[0.2em]
%		\begin{matrix*}[r]   \footnotesize\noexpandarg\StrSubstitute{#1}{,}{\\\footnotesize}  \end{matrix*} &\left[ \begin{matrix}		
%}{
%	\end{matrix}\hfill\right]\end{matrix}
%}
...
\def\hwrapcells#1[#2]{
	...
	&	
	% {~~\color{gray} \tmp~~~~}
	\expandarg\IfSubStr{\tmp}{,}{\hwrapcells{\tmp}[#2]}{ \expandafter#2{\tmp} }
}
\def\vwrapcells#1[#2]{
	\expandafter#2{\expandarg\StrBefore{#1}{,}}
	\expandarg\StrBehind{#1}{,}[\tmp] 
	...
	\\	
	% {~~\color{gray} \tmp~~~~}
	\expandarg\IfSubStr{\tmp}{,}{\vwrapcells{\tmp}[#2]}{ \expandafter#2{\tmp} }
}


\newenvironment{idxmat}[1]{%\footnotesize\noexpandarg\StrSubstitute{#1}{,}{&\footnotesize}
	\def\idxstyle{\footnotesize\color{gray}\text}
	\def\itemlist{#1}
	\vspace{-2em}
	\begin{matrix}  \begin{matrix} \hwrapcells{\itemlist}[\idxstyle]  \end{matrix} \\[0.2em]
		 \left[ 
			\begin{matrix}
				\hwrapcells{\itemlist}[\idxstyle\phantom]  \\[-1em]
}{
		\end{matrix}\right]		&\begin{matrix*}[l] \vwrapcells{\itemlist}[\idxstyle] \end{matrix*}
	\end{matrix}
	\vspace{2em}
}
...
%	\[ \frac{\partial \zeta(B) }{\partial B(x)(y) } \]
	...
	\[ \mat D = \begin{idxmat}{coffee,beer} 0 & 1 \\ -1 & 0 \end{idxmat}\]
	\[ \mat{D} =~~\begin{matrix} & \begin{matrix}\text{coffee} & \text{beer}\end{matrix} \\[0.2em]
					\begin{matrix*}[r] \text{coffee} \\ \text{beer} \end{matrix*} &\left[ \begin{matrix}
						0 & 1 \\ -1 & 0 
					\end{matrix}\hfill\right]\end{matrix} \] 
					
	%%%%%%%%%% TESTS %%%%%%%
%	\[ \mat{D} =~~\begin{matrix} & \begin{matrix}\text{coffee} & \text{beer} & \text{n3} & \text{another} \end{matrix} \\[0.2em]
%		\begin{matrix*}[r] \text{coffee} \\ \text{beer} \end{matrix*} &\left[ \begin{matrix}
%			0 & 1 & 0 & 0\\ -1 & 0 & 0 & 0
%		\end{matrix}\hspace*{1cm}\right]\end{matrix} \] 	
%					
%	\[ \mat{D} = \begin{blockarray}{ccc} 
%			&\text{coffee}& \text{beer} \\
%			\begin{block}{r[cc]} 
%				\text{coffee} & 0 & 1 \\ \text{beer} &-1 & 0 \\
%			 \end{block}
%		\end{blockarray} \] 
		
	\[ \begin{idxmat}{coffee,beer} 1 & 2 \\ 3 & 4 \end{idxmat} \]
	
	
	
	\[ \begin{matrix}
		\begin{matrix}
			a & \text{asdfffdsa} &\text{v}\\
			\text{bafds} & q & zz\gamma\zeta
		\end{matrix}
	\end{matrix}\]
	...
	\[ \mat{C} = \begin{matrix} & \begin{matrix}\text{caff} & \text{nocaff}\end{matrix} \\[0.2em]
							\begin{matrix} \text{caff} \\ \text{nocaff} \end{matrix} & \begin{bmatrix}
								0 & -2 \\ 2 & 0 
							\end{bmatrix}\end{matrix}
		\qquad\qquad\text{and}\qquad\qquad 
		L = \Pr( C \mid D ) = ~~\begin{matrix} & \begin{matrix}\text{caff} & \text{no caff}\end{matrix} \\[0.2em]
