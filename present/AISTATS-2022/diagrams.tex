...
\begin{frame}[label=current]
 \def\xsamp{{\mathcal D}}
    \begin{align*}
		...
		\begin{tikzpicture}[center base]
			\node[dpad0] (Z) {$Z$};
			\node[dpad0,right=0.5 of Z] (X) {$X$};
			\coordinate (A) at ($ (X)!.5!(Z) + (0,0.45)$);
            \node[dpad0,above=0.4 of A] (Y) {$Y$};
            %
            \draw[arr1,-,shorten >=0pt] (Y) -- node[left]{$p$} (A);
            \draw[arr1,shorten <=0pt] (A) -- (X);
            \draw[arr1,shorten <=0pt] (A) -- (Z);
            %
			\coordinate (B) at ($ (X)!.5!(Y) + (0.3,0.2)$);
			%
            \draw[arr1,-,shorten <=0pt] (B) -- 
                node[below,pos=0.65, inner sep=2pt]
                    {${\color{gray}\scriptscriptstyle(\infty)}$}
				node[above,pos=0.65] {$\datadist\xsamp$}
                ++(0.9,0);
            \draw[arr1,shorten <=0pt] (B) -- (X);
            \draw[arr1,shorten <=0pt] (B) -- (Y);
			% \draw[arr1, <-] (X) --
			% 	node[above,pos=0.65] {$\datadist\xsamp$}
			% 	node[below,pos=0.65, inner sep=2pt]
			% 		{${\color{gray}\scriptscriptstyle(\infty)}$}
        ...
        &
        %% Conditional but not multisample or marginal
        \aar*{\!\begin{tikzpicture}[center base]
			\node[dpad0] (X) at (0,0) {$X$};
			\node[dpad0] (2) at (1.1,0) {$Y$};
            %
			\draw[arr2,->] (X) to
				node[above, pos=0.4,inner sep=2pt]{$p$}
				% node[below, pos=0.4, inner sep=2pt]{${\color{gray}\scriptstyle(\beta)}$}
				(2);
			\draw[arr2, <<-] (X) to
				node[above, pos=0.7,inner sep=2pt]{$x$}
		...
		\end{tikzpicture}\!}        
        \\
        % Conditional + Marginal but not multisample
        &\aar**{
        \begin{tikzpicture}[center base]
            \node[dpad0] (Z) {$Z$};
            \node[dpad0,right=0.5 of Z] (X) {$X$};
            ...
            \draw[arr1,shorten <=0pt] (A) -- (Z);
            %
            \draw[arr1,<<-] (X) -- node[above]{$x$} ++(1.1,0);
            \draw[arr1,<<-] (Y) -- node[above]{$y$} ++(1.1,0);
        \end{tikzpicture}
        }%_{\!\!0}
		% \right)
		% + \H(\datadist\xsamp).[1em]        
        \\&
        \aar**{
    		\begin{tikzpicture}[center base]
    			\node[dpad0] (X) {$X$};
    			\node[dpad0,left=.9 of X] (Y) {$Y$};
...
\begin{frame}[label=current]
    \[
    \aar{\dg M} :=
    \inf_\mu \Inc_{\dg M}(\mu)
    \]
    \[
        \aar{\dg M} :=
        \inf_\mu
        \sum_{\ed LXY}\!\! \beta\ssub L  \Ex_{{x \sim \mu(\mskip-2.6muX\mskip-2mu)}} \kldiv[\Big]{\mu(Y|\, x)}{\bp(Y |\, x)}.
    \]
    \[
        \Inc_{\dg M}(\mu) \qquad \IDef{\dg M}(\mu)
    \]
    \[ 
        \kldiv[\big]{e(Z|x)}{p(Z)}
    \]
