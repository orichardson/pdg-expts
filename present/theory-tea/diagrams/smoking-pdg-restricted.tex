% \documentclass[convert={density=300,size=1080x800,outext=.png}]{standalone}
\documentclass[convert]{standalone}

\input{../../../writing/consolidated/the-pdg-manual.preamble.tex}

\begin{document}
\begin{tikzpicture}
	% \colorlet{fillcolor}{blue!80!black}
	\colorlet{mattfillcolor}{color1}
	\fill[fill opacity=0.1, mattfillcolor, draw, draw opacity=0.5] (2.73,1.35) rectangle (6.8, -1.35);

	%oli24: modifying positions to fit things...
	% \node[dpadded] (1) at (0,0) {$\var 1$};
	% \node[dpadded] (PS) at (1.65,0) {$\mathit{PS}$};
	% \node[dpadded] (1) at (1.65,1) {$\var 1$};
	\node[dpadded] (1) at (0, 0) {$\var 1$};
	\node[dpadded] (PS) at (1.65,0) {$\mathit{PS}$};
	\node[dpadded] (S) at (3.2, 0.8) {$S$};
	\node[dpadded] (SH) at (3.35, -0.8) {$\mathit{SH}$};
	\node[dpadded] (C) at (4.8,0) {$C$};

	\draw[arr1] (1) -- (PS);
	\draw[arr2] (PS) -- (S);
	\draw[arr2] (PS) -- (SH);
	\mergearr{SH}{S}{C}

	\node[dpadded] (T) at (6.25,0) {$T$};
	\draw[arr1] (T) -- (C);


    \begin{scope}
      \clip (2.6, 1.37) rectangle (7,1.25);
      \path[draw, very thick, |-|, color=mattfillcolor!50!black,text=black] (2.7, 1.35) --coordinate(Q) (6.83,1.35);%(7.13,1.35);
    \end{scope}
	% \useasboundingbox (current bounding box);
	\node[above=0.05em of Q]{\small Restricted PDG
		%in \cref{ex:grok-ablate,ex:grok-union}
		};

\end{tikzpicture}
\end{document}
