...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item {\onslide<4->{\color<4->{gray!80}(}a utility function $u:\Omega \to \mathbb R$,
				% \item
				 	some actions $A$\onslide<4->{)}.}
    \end{itemize}
    \medskip
    ...
    \begin{center}
        \begin{tikzpicture}
					\onslide<2->{
            \node[dpadded, onslide=<4->{fill=blue!90}] (W) at (0,0) {$W$};
...
\relax % frame setup
	...
	\edef\INCfst{\the\numexpr \INCmotfst + \INCmotlen \relax}
		% \def\INClen{5}
		%\def\INClen{4} % doesn't get bit story
		\def\INClen{3} % No expansion into expectation
...
%     		For a BN $\mathcal B$ with $N$ nodes and a vector $\beta \in \mathbb R^N$,
			For any $\beta$, let $\PDGof{\mathcal B, \beta}$ be the PDG
			corresponding to $\mathcal B$, \\
			with $\alpha=\mathbf 1$, and confidences $\beta$.
%			Straightforward to convert BN to PDG, except for confidences.
		}

    \begin{center}\only<1-2>{\smash{
      \vspace{1em}
...
\section{Inference}
%\begin{frame}
%	BNs and FGs: simple semantics; ~~complex inference.\\
%	\pause
%	PDGs: more complicated semantics; inference follows from semantics.
%\end{frame}
...
%					\node (X5) at (2,0) {$X_5$};
					\node (X4) at (1.7,-0.9) {$\mathit{Ice}$};
					\node (X5) at (2,0) {$\mathit{Cat}$};
      ...
      % \hfill% \dotfill
		...
		\begin{defn}[PDG]
			\begin{description}%
				...
				\item[$\Ed$]<hideme@1,5 |alert@2-4,6,7> \notation{\color<6,7>{benchcolor2!60!gray!50!black}%
					...
					\begin{description}%[<+-| alert@+>]
                        ...
                        \only<6,7>{\setbeamercolor{alerted text}{fg=benchcolor1}%
                            \setbeamercolor{alerted notation}{fg=benchcolor1!60!gray!50!black}}
                        % \color<6>{benchcolor1}
						\item[$\bp$]<hideme@1-3,5 | alert@4,6,7> %:\big(\!(X,Y,\ell)\in\!\Ed \big) \to
                        ...
                        % \onslide<-5>{
                     \only<6,7>{\setbeamercolor{alerted text}{fg=benchcolor1}%
                      	\setbeamercolor{notation}{fg=benchcolor1!60!gray!50!black}}
						\item[$\alpha\ssub L$]<hideme@1-2,4-6|alert@7> \notation{$: \mathbb R$}%
							\hfill{
							\color<6,7>{benchcolor2!60!gray!50!black}
							\small(functional determination)}
					 \only<6,7>{\setbeamercolor{alerted text}{fg=benchcolor2}%
							\setbeamercolor{alerted notation}{fg=benchcolor2!60!gray!50!black}}
						\item[$\beta\ssub L$]<hideme@1-6|alert@7> \notation{$: \mathbb R$}%
