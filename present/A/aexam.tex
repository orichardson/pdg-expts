...
\begin{frame}[label=semantics2]
    ...
    \[%\displaystyle%
        \bbr{\dg M}_\gamma(\mu) := {\color{alertcolor!30!benchcolor1!99!structurecolor}
                \Inc_{\dg M}(\mu)}
            + \gamma%
            \extra[below right=0.5em and -1 em of call point, 
            ...
            \;{\color{alertcolor!30!benchcolor2!99!structurecolor}
                \IDef{\dg M}(\mu)}
    \]
    \tikzro{ \draw[alertcolor] ([yshift=-0.2em,xshift=-0.3em]call point.center) -- ([xshift=1.0em]tp.north west);}
...
\begin{frame}
    ...
    \pause[\thebeamerpauses]
    \alert<.(1)->{Note: not a graphical model (desipte shoehorning attempts)}
