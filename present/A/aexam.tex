...
\begin{frame} %% Surprise as Inconsistency
    ...
    \begin{prop}%[Surprise as Inconsistency]
		...
		\alt<\multislide->{\alert{Cross Entropy}}{Surprise}
		is the inconsistency of simultaneously believing $p$ and
		\alt<\multislide->{\alert{an emperical distribution $\datadist\xsamp$, with high confidence}}{$X=x$}. That is,
        \[\I_p(x) =
					% \log \frac{1}{p(X\!=\!x)} =
            \aar[\Big] {
            \begin{tikzpicture}[center base]
                ...
                \draw[arr2, <<-] (X) -- 
									...
									++(1.3, 0);
            \end{tikzpicture}
            }\only<\multislide->{+\H(\datadist\xsamp)}.
        \]
    \end{prop}
    		...
    		\aar**{\!\!\begin{tikzpicture}[center base]
                ...
                \onslide<2->{
        			\draw[arr, <-, priorcolor] (theta) --
        				node[above right=0.5pt and 1pt, pos=0.7] {$p$}
        				node[below left=0.5pt and 1pt,pos=0.6]{{$\color{gray}\scriptscriptstyle(\infty)$}}
        				++(-1.0, 0.5);}
