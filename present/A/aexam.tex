...
\subsection{Standard Metrics as Inconsistency}
	\def\xentoverlay{5}
\begin{frame}<-\numexpr\xentoverlay-1\relax>[label=frame:surprise] %% Surprise
    ...
    \end{itemize}
		\gdef\customnav{\hyperlink{frame:entiredataset}{\beamergotobutton{For entire dataset}}}
\end{frame}
\gdef\customnav{}
    ...
    \resizebox{\paperwidth}{!}{
    	...
    	\begin{tikzpicture}[scale=1.8]
    		...
    		\fill (1,-1) -- ++(-45:\ptradius) arc (-45:135:\ptradius)
				...
				\node[align=center, draw, structurecolor, fill=white,inner sep=0.1em] at (5.0, 2.9){
					Plot of \\
					$\displaystyle\aar*{\begin{tikzpicture}[center base]
					\node[dpad0, fill=structurecolor!50] (X) {$X$};
					\draw[arr2, <-] (X) --
						% node[above] {$\overset{(\beta : r)}p$}
						node[above, pos=0.6, inner sep=2pt, align=center] {$p$}
						node[below, pos=0.65, inner sep=3pt, align=center] {
							$\scriptscriptstyle{\color{structurecolor!20}(\beta : r)}$}
						++(-1.1,0);
							\draw[arr2, <-] (X) --
						% node[above,pos=0.5] {$\overset{(\beta : s)}q$}
						node[above, pos=0.6, inner sep=2pt, align=center] {$q$}
						node[below, pos=0.65, inner sep=3pt, align=center] {
							$\scriptscriptstyle{\color{structurecolor!20}(\beta : s)}$}
						++(1.1, 0);
...
\begin{frame}
	...
	\gdef\customnav{\hyperlink{sec:cats}{\beamergotobutton{More Category Theory}}}
\end{frame}
	\gdef\customnav{}
...
\begin{frame}\frametitle{Open Problems and Future Work}
	% \TODO
	\begin{itemize}[<+->]
		\item Fleshing out the details of belief propagation as local inconsistency reduction
		\item Properties of ``sub-stochastic'' PDGs: incomplete cpds
...
\begin{frame}[label=current]
	...
	\vfill
	\onslide<8->{
		Python library available at~~ \url{https://orichardson.github.io/pdg/}
	}
...
\section{More Losses}
	% \againframe<\xentoverlay->[label=frame:entiredataset]{frame:surprise}
