...
\begin{frame}\frametitle{Inconsistency}
	...
	\[ \aar{\dg M} := \inf_{\mu} \bbr{\dg M}(\mu) \]
	
	\bigskip
	\pause
	Nice property for minimization:
	\begin{itemize}
		\item Strictly convex, smooth in every cpd ~~ {\small\color{gray}(on the interior)}
	\end{itemize}
	\end{frame}
...
\begin{frame}<-\numexpr\xentoverlay-1\relax>[label=surpriseframe] %% Surprise
    ...
    \end{itemize}
		\gdef\customnav{\hyperlink{entiredatasetframe}{\beamergotobutton{For entire dataset}}}
\end{frame}
\gdef\customnav{}

\begin{frame}
	\frametitle{Big Table}
...
\begin{frame}
	...
	\only<3->{	
		...
		PDG inconsistency measure how close the diagram is to commuting
		\[ \exp \left( -\frac1\gamma \aar{\dg M}_\gamma \right) = \Big\{ a : k\!\,f = hg \Big\} \]
	}
		% \qquad = - \log \#\{a : kfa = hga \} \]
	% \pause
...
\end{frame}
\againframe<\xentoverlay->[label=entiredatasetframe]{surpriseframe}
