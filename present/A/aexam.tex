...
% Details of PDG definition componnets & subsets + Category Theory
{
    ...
    \begin{frame} %%%%%%%%%%%%% Definition of PDG, details (removed for AAAI) %%%%%%%%%%%
		...
		\begin{defn}[PDG]
			\begin{description}%
				...
				\item[$\N$]<hideme@5|alert@1-4,6> \notation{\color<6>{benchcolor2!60!gray!50!black}$:\Set$}<1-4,6>%
					\hfill\small (node set)
					\begin{description}
                        \only<6>{\setbeamercolor{alerted text}{fg=benchcolor1}}
						\item[$\V$]<hideme@2,3,5|alert@6> \!{\color<1,4>{notationcolor}\color<6>{benchcolor1!60!gray!50!black}{$:\N \to \Set$}}\quad%
    ...
    \begin{frame}
        \usebeamercolor[fg]{normal text}
        What do you do with diagrams? Take {\color{blue!50!gray!60!white}limits} and colimits.
        ...
        \begin{center}
            \begin{tikzpicture}[scale=1.2]
                ...
                \begin{scope}[orange!50!gray!60!white]
                    \node (V) at (-0.5,1.2){$V$};
                    \draw[arr2] (V) -- (X1);
                    \draw[arr2] (V) -- (X2);
                    \draw[arr2] (V) -- (X3);
        ...
        % \begin{prop}
        For the deterministic sub-PDG $\dg M_{\mathrm{det}}$:
            \[ \lim {\dg M}_{\mathrm{det}} 
                % = \faktor{\V(\dg M)}{ \sim }
                = \Bigg(\parbox{0.9in}{\raggedleft natural \\[-0.12em] sample space}~\Omega,~ \parbox{0.62in}{\raggedleft random\\[-0.12em] variables}~\Big\{ \tilde X : \Omega \to \V(X) \Big\}_{X \in \N}  \Bigg) \] 
        % \end{prop}
        \bigskip
        
        For all of $\dg M$:
        % \begin{prop}
            \[ \lim \dg M =  \mathrm{Verts}( \underbrace{ \mathbb L \dg M }_{\mathclap{\text{Locally Consistent Polytope}}} )  \] 
        % \end{prop}
    \end{frame}
}
...
\begin{frame} %% Regularization
    \begin{prop}[Regularizers as (inconsistent) priors]
        Suppose you believe $Y \sim f_\theta(Y)$, and have a prior $p(\theta)$, and an empirical distribution $D(Y)$ which you trust. The inconsistency of also believing $\Theta = \theta_0$ is the \emph{regularized}-cross entropy loss, and controlled by the strength $\beta_p$ of the prior.
    	That is,
