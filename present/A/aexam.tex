...
\begin{frame}[label=current]
    ...
    % Motivation: 
    %  Suppose you're
    The usual way of modeling an agent with uncertainty:
    
    \begin{itemize}
        \item a probability distribution $p : \Delta S$,
        \item a utility function $\Omega \to \mathbb R$
        \item 
    \end{itemize} ,, and infinite computation.
    ...
    But such agents cannot have internal conflict.
\end{frame}

\begin{frame}[label=current]
    
    % Rhetorical Question
    {\Large Why build systems that can be inconsistent, if inconsistency is bad? }
    {\Large Why entertain the possibility of being wrong, if being wrong is bad? }
    {\Large Why }
    
    
    External conflict:    
    \begin{itemize}        
        \item test cases for software
        \item losses for training neural networks
            \hfill\texttt{\color{gray} // probabilistic}
        \item 
    \end{itemize}
    
    Internal:
    \begin{itemize}
        \item Assertion statements
    \end{itemize}
		...
		\begin{defn}[Probabilistic Dependency Graph]\label{def:model}
			...
			\begin{description}%
				...
				\item[$\Ed$]<+-> %\notation{$\subseteq \N \times \N \times \mathit{Label}~~~$}%
					is a set of labeled edges $\{ \ed LXY \}$, \hfill\hyperlink{hypergraphextra}{\beamerbutton{(hyper-edges)}}\\
...
\begin{frame}\frametitle{Inconsistency as Loss}
