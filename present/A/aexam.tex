...
\begin{frame}
    ...
    \TODO[maximum entropy result for BNs]
		Gaussian: Maximum entropy with mean $\mu$, variance $\sigma^2$.
		Exponential: maximum entropy with mean $\lambda$.

		\begin{coro}
			Among the distributions in $\SD{\mathcal B}$,
			$\Pr_{\mathcal B}$ has the maximum entropy, beyond the (actualized) entropy of the given cpds.
		\end{coro}
		\[ \]
\end{frame}
...
\begin{frame} \frametitle{Factor Graphs}
	...
	\begin{defn}
		...
		\[ {\Pr}_{\alt<-6>{\Phi}{\Psi}}(\vec x) := \frac{1}{Z_{\Phi}}
...
\begin{frame}
    ...
    \setlength{\fboxsep}{0pt}
    \pause\Put(-300,180){\color{structurecolor}\fbox{\includegraphics[height=5cm]{figs/vae-diagram-2-colored}}}
    \pause\Put(-130,165){\color{structurecolor}\fbox{\includegraphics[height=4cm]{figs/vae-diagram-3-colored}}}
    \pause\Put(-200,155){\color{structurecolor}\fbox{\includegraphics[height=3cm]{figs/vae-diagram-1-cropped}}}
    \pause
...
\begin{frame}[label=current]
	...
	\begin{center}
		...
		\only<2>{
			...
			\node[dpadded] (U) at (3.8,0) {$U$};
			%
			\node[dpadded] (X) at (-1.4, -0.3) {$X$};
			\node[dpadded] (Y) at (-0.4,-1.2) {$Y$};
			\node[dpadded] (Z) at (0,0) {$Z$};
			\node[dpadded, fit={(X)(Y)(Z)}, label={[label distance=-1em]120:$W$},
			fill opacity=0.05,draw opacity=0.1,dashed] (W) {};
			\draw[arr2] (X) to (Y);
			\mergearr YZX
			\mergearr AZO
			\draw[arr2] (A) to[bend right] (X);
			%
			\draw[arr2, ->>] (O) -- node[above]{$u$}  (U);
			\end{tikzpicture}
		}
