...
\begin{frame}
    ...
    \begin{itemize}[<+->]
        ...
        \item a utility function $u:\Omega \to \mathbb R$
				\item some actions $A$.
    \end{itemize}
    \medskip
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            %
					\onslide<2->{ \draw[arr2, <-] (W) -- node[above]{$p$} ++(-1.5, 0); }
					\onslide<3->{ \draw[arr2, ->>] (O) -- node[above]{$u$}  (U);}
					\onslide<4->{ \mergearr[->>] WAO	}
        \end{tikzpicture}
    \end{center}
...
\begin{frame}
    % Rhetorical Question
		...
		\bigskip
		\pause[\thebeamerpauses]
		Why \textbf{build a system} that can be inconsistent%
		...
		\begin{itemize}[<+->]
		\item Why entertain the possibility of being wrong?
			\begin{itemize}
    ...
    % \begin{block}{As a tool}
    %     {Why build systems that can be inconsistent, if inconsistency is bad? }
		% 		\pause
		% 
    % 		{\small Why entertain the possibility of being wrong, if being wrong is bad? }
		% 		\begin{itemize}
		% 			\item because you can learn 
		% 			\item 
		% 		\end{itemize}
    % \end{block}
		% 
...
\begin{frame}\frametitle{Yet Another Probabilistic Graphical Model}
	...
	We introduce \textit{probabilistic dependency graphs} (PDGs), a new class of
	graphical models for representing uncertainty.
	
	\begin{itemize}[<+-|alert@+>]
		\item Designed to tolerate inconsistency, so we can model it.
		\item In doing so, we get \emph{much} more \ldots
	\end{itemize}
...
\begin{frame}
		\frametitle{Bayesian Networks: Maximum Entropy?}
...
\begin{frame} \frametitle{Factor Graphs}
	...
	\begin{defn}
		 ...
		 \pause
		 \alt<-6>{and}{} \alert<3>{\emph{factors}} $\{\phi_J\colon \V(X_J) \to \mathbb R_{\geq0}\}_{\alert<3>{J \in
		\mathcal J }}$,
		\only<7->{\alert<7>{and weights $(\theta_J)_{J \in \mathcal J}$}}
		with $X_J \subseteq \mathcal X$;
		\pause
		\alt<-6>{$\Phi$}{$\Psi$} defines a distribution
		\[ \alert<6>{{\Pr}_{\alt<-6>{\Phi}{\Psi}}(\vec x) :=}~\frac{1}{Z_{\Phi}}
	...
	% \begin{defn}
		...
		\alt<-6>{$\Phi$}{$\Psi$} defines a ``variational free energy''
...
\begin{frame}\frametitle{Inference via Inconsistency Reduction}
	...
	\begin{itemize}[<+-|alert@+>]
		...
		\item The choice of cpd $p$ that minimizes the inconsistency of $\dg M \sqcup {p}$ (which is strictly convex and smooth in $p$) is $\bbr{\dg M}^*(Y\!\mid \!X)$,
		\begin{itemize}
...
\begin{frame}
    ...
    \pause\Put(-230,195){\color{structurecolor}\fbox{\includegraphics[height=3cm]{figs/vae-diagram-1-cropped}}}
    \pause
...
\subsection{Open Problems + Future Work}
\begin{frame}\frametitle{Open Problems and Future Work}
	% \TODO
    \begin{itemize}
				\item Technical tool: PDGs with incomplete cpds 
				...
				\item $\ast\ast~~$ Do PDGs capture Dependency Networks? $~~\ast\ast$
