			...
			\begin{center}
				...
				\begin{tikzpicture||precompiled}[paperfig]{smoking-BN}
					\begin{scope} % BN Nodes...
						...
						\node[onslide=<8->{opacity=0.2, text opacity=0.2}] (PS) at (0,0) {$\mathit{PS}$};
						\node (SH) at (1.5, -0.6) {$\mathit{SH}$};
						\node (S) at (1.5, 0.6) {$\mathit{S}$};
						\node (C) at (3, 0) {$\mathit{C}$};
						\end{scope}
					\draw[->,onslide=<8->{opacity=0.2}, onslide=<5>{benchcolor2,thick}] (PS) to (SH);
					\draw[->,onslide=<8->{opacity=0.2}] (PS) to (S);
					\draw[->] (S) to (C);
					\draw[->,onslide=<5>{benchcolor2,thick}] (SH) to (C);

					\only<8->{% Describe why restriction of BN is not a BN
					...
					\end{tikzpicture||precompiled}%
				\hfill\pause\only<-8>{{\color<8->{gray!50}\vrule}}\hfill%
				\onslide<-7>{
				\begin{tikzpicture||precompiled}[paperfig]{smoking-PDG}
					\onslide<7->{ % Matt for restriction
					...
					\node[dpadded, % (S) ...
					 	onslide=<7->{fill=black!.16, fill opacity=0.9}]
					...
					\onslide<3-4>{ % Draw cpts attached to each edge...
						% \node;
					...
					\draw[arr1,  onslide=<4>{benchcolor1}] (1) -- (PS);
					\draw[arr2,  onslide=<4>{benchcolor1}] (PS) -- (S);
					\draw[arr2, onslide=<5>{benchcolor2}] (PS) -- (SH);
					\mergearr{SH}{S}{C}
					\only<5>{
					...
					\onslide<6->{ % Add Tanning Beds & arrow...
						\node[dpadded,
							 	onslide=<6>{fill=benchcolor1!36, fill opacity=0.75,
								 	draw=benchcolor1!20!black, dashed},
								onslide=<7->{fill=black!.16, fill opacity=0.9}]
							(T) at (6.25,0) {$T$};
						\draw[arr1,onslide=<6>{dashed,draw=benchcolor1!60!black}] (T) -- (C);
						}
					\onslide<7>{ % Draw matt & label for restriction...
					...
					\onslide<8-9>{ % Dim this picture for BN description...
...
\begin{frame} %% Surprise as Inconsistency
    ...
    \begin{prop}%[Surprise as Inconsistency]
		...
		\alt<\multislide->{\alert{Average}}{} Surprise
		is the inconsistency of simultaneously believing $p$ and
		\alt<\multislide->{\alert{an emperical distribution $\datadist\xsamp$, with high confidence}
                ...
                \draw[arr2, <<-] (X) -- 
									node[above,pos=0.7]{%
										\alt<\multislide->%
											...
											{$\scriptstyle X\!=\,\!x$}}%
									node[below,pos=0.7]{\only<\multislide->{\alert<\multislide>{%
										$\scriptscriptstyle\color{alertcolor!80!gray}(\beta:\infty)$}}}
									++(1.3, 0);
            \end{tikzpicture}
            }\only<\multislide->{{\color{alertcolor!50!gray}+\H(\datadist\xsamp)}}.
        \]
    \end{prop}
...
\begin{frame} %% Marginal Surprise as Inconsistency
    ...
    \begin{prop}[marginal information as inconsistency]<+-|alert@+>
        If $p(X,Z)$ is a joint distribution, the (marginal) information of the (partial) observation $X=x$ is given by\\[-1.2em]
	...
	\begin{prop}[supervised  setting]<+->
		% Consider a cpd $f(Y \mid X)$.
		The inconsistency of the PDG containing $f(Y\mid X)$ and a high-confidence empirical distribution $\datadist\xysamp$ of samples $\xysamp = \{(x_i,y_i)\}$ is equal to the cross entropy {\color{gray}\small( plus $\H(Y\mid X)$, a constant that depends only on the data $\datadist\xysamp$)}. That is,\\[-1.2em]
		\[ \aar**{
			\begin{tikzpicture}[center base]
				\node[dpad0] (Y) {$Y$};
    		...
    		\aar**{\!\!\begin{tikzpicture}[center base]
                ...
                \onslide<2->{
        			\draw[arr, <-, priorcolor] (theta) --
        				node[above right=-2pt and 0pt, pos=0.7] {$p$}
        				node[below left=-2pt and -1pt,pos=0.6]{{$\color{gray}\scriptscriptstyle(\beta)$}}
        				++(-1.0, 0.5);}
    ...
    % \column{0.84\linewidth}
        \begin{itemize}[<+->]
            ...
            \begin{itemize}
                ...
                \item Combine to get VaE objective:\\ $\mathrm{ELBO}_{p,e,d}(x) :=$
									\\[-2.1em]
							  \onslide<+->{
                	\[
...
\begin{frame}\frametitle{Open Problems and Future Work}
    ...
    \begin{itemize}
				...
				\begin{itemize}
					\item the probabilistic automaton generated by a PDG
...
%	\TODO[update with second half]
	...
	\begin{itemize}[<+-|alert@+>]
		...
		\item cleanly separate quantitative from qualitative information, 
			and can express confidence in each ($\beta$ and $\alpha$).
...
\begin{frame}[label=current]
	\frametitle{A Different Picture of Agency}
	%% Draw:
	...
	\begin{itemize}
		...
		\item<7> driven by pursuit of coherent identity, not necessarily ``favorite number go up''
	\end{itemize}
