			...
			\begin{itemize} %% bullet points
				...
				\only<4>{ % New Information! The green block with cpts
					...
					\begin{exampleblock}{}
						...
						{\small Grok comes to believe that Floomps and Guns have the same legal status (92\%)}
						\vspace{-0.6em}
	...
	\subsection{Capturing Bayesian Networks}
		\begin{frame} %%%%%%%%%        SMOKING: BN vs PDG         %%%%%%%%%%
			...
			\begin{itemize}%[<+-| alert@+>]
				...
				\item<+-| alert@+> a variable can be the target of incorporate more than one cpd;
...
\section{Formalism and Technical Results}
	...
	\relax % frame setup
		...
		\def\beforeINCandIDEF{-\the\numexpr\INCandIDEF-1\relax}
		\def\atslide#1{\the\numexpr#1\relax}
		...
		{\alt<\INCexrange,\IDEFexrange>{\vspace{-0.5em}\small}{\centering}\only<\INCandIDEF>{\centering}
			$\displaystyle%
				\bbr{\dg M}_\gamma(\mu) := \hl[alertcolor!30!benchcolor1!99!structurecolor]%
					...
					+ \gamma\;\hl[alertcolor!30!benchcolor2!99!structurecolor]<\IDEFmotfst->{\IDef{\dg M}(\mu)}
				$\par}
		
		\only<\beforeINCandIDEF>{\medskip}
