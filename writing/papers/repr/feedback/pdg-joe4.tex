...
such models in the literature, including Bayesian networks
(BNs) and factor graphs. (For an overview,
  see \cite{koller2009probabilistic}.)  Why does the world need one more? 
...
%
%	Still, conditional independences are useful, and as Pearl
%\parencite{pearl1989conditional} has argued forcefully, omnipresent. 
	%: in $\chi^2$ test for instance, independence is the null
        %hypothesis.
%joe4: moved this above        
...
%joe4: typo
%  now walk through what is requried to represent $\cal
    now walk through what is required to represent $\cal
                B$ as a \MN, which we call $\Gamma(\mathcal B)$, shown
...
together with the 
special node $\sf 1$ from \Cref{ex:guns-and-floomps}. and
...
%joe4: typo
%                (2) recovering the additional independece assumptions
...
%joe4
%        \Cref{thm:bns-are-pdgs}, we will show that this distribution
...
%joe4
%                control (call the cpt for this $\mat p$), and would
...
%		{\color{gray} The rise of the communist party also
%came with changes in smoking habits, so a new unconditional
%distribution on $S$ could not be obtained by eliminating the variable
%$PS$. } 
%		
...
%                require an association prior beliefs with $S$ and
		...
		\end{tikzcd}},
%joe4
%		one concludes that $A$ and $C$ are independent, an
...
%joe4		
%		PDGs are more modular in both respects. Consider the
...
%joe4: typo
        %        perhaps clearest visually, but for cliarity, if $\mat S$ is
...
%        $[\vec a; \vec b]$ is a vertical stacking of the vectors $\vec
        blue hyperedge that computes $\mathit{SL}$ from $\mathit{C, T}$, and
we use the notation 
...
%joe4
%	        will all be probabilistic estimates of $\mathit{SL}$, which
...
%joe4:
%                  case of a cpt that assigns all of its mass to $f(a)$
...
%joe4: typo?
%        tIn addition to the cpts themselveshe multi-tailed edges that
...
%joe4
%In the next section, we will give semantics to PDGs which make use of
...
%        measures of uncertinty. In particular, for each link $L =
...
%consistent.  
%joe4
%                The \emph{inconsistency of PDG $\sfM = \mnvars[]$} is the
...
%                minimum over all distributions $p$ of the inconsistency of
                %        $\sfM$ with respect to $p$:
                                minimum over all distributions $\mu$
                                of the inconsistency of $\sfM$ with
                                respect to $\mu$:  
%joe3: you're switching between \mu and p again
%		\[ \inconsist(M) = \inf_{ \mu \in \Delta [W_{\cal V}]}
%\inconsist (M; \mu) . 
		\[ \inconsist(\sfM) = \max_{\mu \in \Delta [\V(\sfM)]}
                \inconsist (M; \mu) . \]
...
%joe4
        %        \in \Ed^\sfM$, values $x,y \in \V(X)$ is chosen according to
%        $\mu_{_{XY}}$ and $x$ given to both
...
%consistency''.  It's ust that more consistency
%mak=infμ∈∆V(M)ζ(M;μ)Proposition 3.1(??5.1
%analog).JM∪M′K=JMK+JM′KLemma 3.2(Lemma 5.2 analog).ζis a convex
%function.Corollary 3.2.1.Uα(M;p)is convex inpforα≥0, and strictly so
%forα >0. Furthermore,J−Kisquasiconvex—that is, all of its level sets
%are convex sets.As a result of??3.2.1,JMKis a quasiconvex function In
%particular, Lemma 5.2 is the special case forthe level setU ≤0forα=
%0.The semantics given in Section 3.1 does not allow us to express
%independencies very effectively.For instance, in Example 2, to fully
%get the joint representation given by the BN, we also needto somehow
%assume thatSC⊥⊥S|PS.  This is possible to do explicitly with an extra
%arrowmeaningless arrow, but this solution doe not scale well, looks
%like it dependence (the opposite ofour intention), and clutters the
%diagram. We now show that by applying the principal of
%maximumentropy, we can recover all of a BN’s independence assumptions
%at once. This should make intuitivesense: maximizing entropy tend to
%‘make things as independent as possible’. Those familiar with theview
%of graphical models as separable exponential families may have even
%seen a similar result inan undirected setting e.g., (cf. WJ+08,
%pp. 37-39). We proceed with the theorem, and discuss someconsequences
%below.Definition 3.6.IfMis a PDG,JMKS⊆∆V(M)is a set of
%distributions,(D,)is an ordered set,and`: ∆WM→(D,≤)is a scoring
%function for probabilities, letthe upper≤-frontier
%under`,denotedJMK`S, be the set of distributions that are not
%strictly dominated by any
%others. Explicitly,JMK`S={μ∈JMKS∣∣∣∀μ′∈JMKS. `(μ′)`(μ)}Theorem
%3.3.IfMis a consistent PDG, then there is a uniqueμ∈JMKsdthat
%maximizes theShannon entropy,H(μ) =∑x∈Supp(μ)μ(x)
%log1μ(x),whereSupp(μ)is the support ofμ.Proof.This is a direct
%consequence of Lemma 5.2 and the fact that entropy is strictly
%concave.Definition 3.7.IfMis a consistent PDG, letJMKHbe the unique
%distributionμ∈JMKsdof maximumentropy.Example 7(composition).Consider
%a slight alteration of Example 13 in whichA, which had
%anunconditional distribution onX,  is replaced withA′:=Zp′−→X,
%representing a distributionconditioned onZ. As
%before,rA′∪Bzsd=rZp−→Xq−→Yzsd9 
...
%	\end{center}
%	\vspace{0.5em}
	The solution, as motivated in \Cref{sec:extra} is to minimize
        the $\emph{extra information}$ (\Cref{def:extra}) instead of
...
%joe4
%                $X_1, \ldots, X_n$, we construct its corresponding PDG
...
$X_i$ has no parents, then we take $\V(\Pa(X_i)) = \{\star\}$.                   %joe4
...
%joe4
%        $\alpha \in \cal I$ is an undirected hyper-edge, drawn as a
...
\item[$\phi_4$.] The test is on factor graphs, which was unlikely, so $\phi_4() = 0.25$. This is true independent of anyone's scores, and doesn't bear on the distribution, so it will get normalized out.
		...
		\begin{enumerate}[nosep]
%joe4: this should be mntion earlier (when yo udefine \phi_3)
...
As \Cref{ex:fg-exam} shows,: there is a
...
%% By the their global semantics, we have
%By the semantics of BNs, we have that
%	\begin{align*}
%joe4: to get the \B to be positions right relative to the \Pr.  But I
%think that we should cut this anyway
%	  \Pr_{\cal B}(\vec x) = \prod_{N \in \N} p_N( \vec x_N
...
%\Cref{subfig:fg-gf,subfig:fg-smoking}
\Cref{ex:guns-and-floomps,ex:smoking} give the factor graphs corresponding  to
the BNs in \Cref{ex:guns-and-floomps,ex:smoking}, respectively.  
...
%joe4
%                as a hypergraph directly, or translate it to a graph
%                first as we formalize in \Cref{sec:formal+syntax}. 
...
%joe4
%        directions result in wildly different distributions, none of
%        which are necessarily related to the original factor
%        graph. However, we can do much better by giving up on creating
...
%joe4
%        to interpret the factor graph. However, we claim the
