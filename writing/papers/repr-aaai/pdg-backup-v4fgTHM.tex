...
A \emph{Probabilistic Dependency Graph} is a tuple $\dg M = (\Gr,\mat p, \beta,\alpha)$, where
...
roughly speaking, is a subjective confidence in the functional dependence of $Y$ on $X$ implicit in $L$. 
\end{description}
%oli22: added
...
%
In this paper we focus discussion on the case where $\alpha_L=1$ for all edges $L$,  
and abbreviate $\dg M = (\Gr, \mat p, \beta)$.
\end{defn}
...
%oli19: removed \pdgvars above, putting ^{\dg M} subscripts here instead.
	\Inc_{\dg M }( \mu) := 
		\!\!\!\!\!\sum_{\ed L{X\,}{\,Y} \in \Ed^{\dg M}} \!\!\beta_L^{\dg M} \E_{x \sim \mu_{_X}}
...
%oli22: our new notation comes in handy:
...
%oli20: this is a more natural explanation I think?
	...
	given the value $\mat w_X$, in expectation. \label{item:localinfo}
\end{enumerate}
...
%	 a specification along each edge of $G$
%	 has the same expected length as an optimal description of the world. 
...
For a PDG ${\dg M}$, we take $\IDef{\dg M} = \IDef{((\N^{\dg
M}, \V^{\dg M}), \Ed^{\dg M})}$.   
\end{defn}
...
Factor graphs 
%oli22: The original paper is actually really persuasive and arguably a better introduction
...
%oli19:
\end{defn}
%oli21: minor definition restructure to clarify weights and eliminate the need for
% the footnote.
%joe19
...
%oli21: footnote removed for this presentation
% \footnote{\label{note:fg-or-expfam}
% 	Typically, a factor graph is taken to be the special case where each
% 	$\theta_J=1$, while the more general case is the exponential family associated
% 	with the factor graph. For convenience, we allow the weights from the outset.}
%joe19
...
%oli21: adding parentheses so the single subscript is more reasonable
%joe19*: why not replace (\Phi,\theta) by \Psi everywhere?
%oli22: I wanted to be able to supply an external weight and use an
% unweighted factor graph; but that is still possible if I introduce it using
% \Psi. I'm conflicted though, because \Phi and \phi have the same relationship
% as a variable X and x, and we explicitly give \theta, the parameters now are 
% clear, but we might have to justify extra subscripts otherwise. Also, I've
% changed the theorems to use the unweighted objects for clarity, which may
% alter your calculation. In any case, I have the \Psi version commented out below.
...
and a canonical scoring function 
\begin{equation}
%oli22: we're using \mathcal G for graphs now; I'm putting this in the macro \GFE
% with \mathit + kearning, but feel free to modify the macro. Silently 
% replacing this everywhere.
%oli22: again, the \Psi version is much less clear where \theta and \phi come
% from , so I'm not sold on it. It's commented out below.
% \GFE_{\Psi}(\mu)
   \GFE_{(\Phi,\theta)}(\mu)
...
%oli21: actually this terminology is from
...
%oli22: added. Note: this is where I introduce the notation {\bf 1} for the
% all-ones vector, which I use in the theorems to simplify the verbage a lot.
A factor graph $\Phi$ defines a distribution $\Pr_\Phi$ and scoring function 
$\GFE_{\Phi}$ by implicitly taking $\theta = \mathbf 1^{\mathcal J}$, the vector 
equal to 1 at each component $J$ of $\cal J$.
%oli22: this maybe should be moved. 
...
%oli22: state what we're about to do?
At $\gamma = 1$, a PDG is essentially a factor graph, and its semantics
also capture the associated exponential family when $\alpha = \beta$. 
...
We extend transformation to one that takes a (weighted) PDG $\dg M = (\dg N, \beta)$ 
...
%oli22: was extremely tricky to read and edit in its fragle run-on-sentence form. 
...
% that they did; adding clarification.
...
% (I've kept my colon but I'll globally modify once I hear back from you on the
% above)
...
%and equal to $\gamma$,
%oli21: PDGs have a parametric family of semantics, one for each \gamma.
% The specific one of these that is preserved depends on what each \beta is.
% [[M]]^* is not defined if only one value of \gammma works.
...
% $\bbr{\dg M}_{\gamma} = \gamma\,\mathcal G_{ \WFGof{\dg M}, \beta^{\dg M}}$ and
...
%oli22: I agree with the sentiment above, but was not able to reorganize
% enough of the document to make that happen last time. Here's an attempt.
...
values other than 1.  Specifically, we can generalize 
to the case where the ratio of
$\alpha_L$ to $\beta_L$ is a constant.
%joe19
(Recall that that before we implicitly assumed that $\alpha_L = 1$ for
all edges $L$, so that if $\beta_L = \gamma$ for all $L$, then the
ratio of $\alpha_L$ to $\beta_L$ will be a constant.)
...
%this is the flavor of whwat we can exppdgs}ect. I'm pretty sure the conjecture is
...
%oli21*: Proved conjecture and added this back in.
% \commentout
{
...
%oli21: using language directly from their paper.
% even if they are inconsistent.
without enforcing the consistency constraints.

%oli21: let's define them.
\begin{defn}
	A DN is a tuple $\mathcal D = (\mathcal G, \mathcal P) $, where $\mathcal G = (\N,\V,\Ed)$ is a
	directed graph of variables, and $\mathcal P$, gives for each $X \in \N$ a cpd $\bp[X](X \mid \Pa(X))$, where $\Pa(X)$ are the parents of $X$ in $\mathcal G$.
	Given a total order $\prec$ on $\N$,
	$\mathcal D$ defines a joint distribution on $\V(\N)$ by an
%joe19*: I have no clue what an ordered Gibbs sampler is.  You can't
%assume taht the reader will know this!
ordered Gibbs sampler.
%joe19*: the next sentence doesn't parse
The process of repeatedly cycling
	through the variables (ordered by $\prec$). 
\end{defn}

%oli21: describe transltation. It's the same as a BN. We do nothing 
% fancy, just do what we need to do to include the cpts, and set all weights to
% 1 as there is nothing to break symmetry.
A dependency network $\mathcal D$, and a vector of confidences $\beta$ for the cpds on each variable
 % (defaulting to $\beta_X=1$) 
can be naturally regarded as a PDG $\PDGof{\mathcal D}$ 
via the same translation used for a Bayesian Network.
Although on the surface PDG and DN semantics are quite
%oli21:
% different (the latter is in terms of an ordered Gibbs sampler),
different,
%oli21
% it turns out that DNs too are specific case of PDGs
PDGs are more general.
%joe18*: It's strange to say ``it turns out'' and then to state a
%conjecture, rather than a theorem.
%oli21: Like most of the material I added last cycle, I didn't want to 
% deveote too much time to proving it lest it get cut. 

%oli21: this simple theorem is just a repackaging of their Theorem 2, so I 
% am now attributing it to them.
\begthm[\citeauthor{heckerman2000dependency}]{theorem}{thm:dns-are-pdgs}
%oli21: updating theorem statement
	...
	If $\mathcal D$ is a consistent dependency network with
	positive local distributions, then $\SD{\PDGof{\mathcal D}}
	=  \{ \Pr_{\cal D}^\prec \}$, where $\Pr_{\cal D}^\prec$ is
	the unique stationary distribution of the $\prec$-ordered
%joe19*: here you call it ``Gibbs sampling procedure''.  Before it was
%``Gibbs sampler''.  I don't know what it is, but you should be
%consistent.  YOu should also point out somewhere that the
%distribuiton is independent of the ordering \succ
Gibbs sampling procedure.  
\end{theorem}

%joe19*: Doesn't this need Proposition 3.4, not 3.2?
It follows from \Cref{thm:dns-are-pdgs,prop:sem3} that
$\bbr{\PDGof{\mathcal D}}^* = \Pr_{\cal D}$. This is comforting, but
not particularly surprising; it merely states that PDGs and DNs have
the same notion of inconsistency. Perhaps more surprisingly, the
independencies of $\mathcal D$ are captured by the qualitative term
$\IDef{}$.   


\begin{prop}
	A distribution $\mu$ that locally minimizes $\IDef{\mathcal G^{\mathcal D}}$ satisfies all of the independencies of $\mathcal D$.
	% For any distribution $\mu$ violating the independencies of $\mathcal D$, there is a $\mu'$ for which $\IDef_{\mathcal G^{\mathcal D}}(\mu') < \IDef_{\mathcal G^{\mathcal D}}(\mu)$.
\end{prop}
% \begin{prop}
...
A PDG can simulate the effect of the order by 
...
\begin{theorem}
	...
	such that  $\beta_1 \ll_\epsilon \beta_2 \ll_\epsilon  \ldots \ll_\epsilon \beta_n$, 
	then $\Pr_{\cal D}^{\prec} = \bbr{\dg M; \beta}^*$
\end{theorem}

A \emph{symmetrized} Gibbs sampler, rather than cycling though variables in a 
particular order, randomly draws the variable to update. Doing so results 

\begin{theorem}
	The fixed point $\mu^*$ of a symmetrized Gibbs sampler for a DN $\mathcal D$ 
\end{theorem}
...
\bbr{M}_\gamma(\mu) 
	...
	&= \E_{\mat w \sim \mu} \Bigg\{   \sum_{ X \xrightarrow{\!\!L} Y  } \left[ \gamma \valpha{\alpha_L} \log \frac{1}{\bp(y\mid x)} + 
	...
	&=  \sum_{ X \xrightarrow{\!\!L} Y  } \left[ \gamma \valpha{\alpha_L} \E_{x,y \sim \mu_{\!_{XY}}} \left[ \log \frac{1}{\bp(y\mid x)} \right] + 
		(\beta_L - \valpha{\alpha_L} \gamma) \E_{x\sim\mu_X}
