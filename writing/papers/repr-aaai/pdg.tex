...
%joe26: changed \sf 1 to \var 1, so that you change all occurrences
...
with the special node $\var 1$ from \Cref{ex:guns-and-floomps}; we add an edge
from ${\var 1}$ to $\mathit{PS}$, to which we associate the unconditional
...
$\Phi$ (recall that we treat the unweighted PDG $(\Gr,\mat p)$ with the
 weighted PDG $(\Gr,\mat p,  \mat 1, \mat 1)$).
$\Phi$.
%joe21
...
Suppose that we
want to compute the probability of $Y$ given $X$ in a PDG $\dg M$. 
...
%oli28: a key point is that we don't have to do it for each distribution,
%	because it's convex. Also, it is both helpful and standard to include the
%	source and target of the conditional distribution in the notation.
% For each distribution $p$, we consider the PDG 
We judge a candidate answer $p(Y \!\mid\! X)$ by the inconsistency of the PDG
...
, i.e., the best possible score $\bbr{\dg M^{+p}}(\mu)$ of any distribution $\mu$.
%
