...
%  weighted PDG $(\Gr,\mat p,  \mat 1, \mat 1)$).
$\Phi$.\footnote{recall that we treat the unweighted PDG $(\Gr,\mat p)$ semantically 
	like the weighted PDG $(\Gr,\mat p,  \mat 1, \mat 1)$}
%joe21
...
inconsistency%
\footnote{equal to $\inf_{\mu} \bbr{\dg M^{+p}}_{\gamma}(\mu)$ for fixed $\gamma$ (which is  $\inf_\mu \Inc_{\dg M^{+p}}(\mu)$ in the limit as $\gamma\rightarrow0$), although this example does not depend on $\gamma$.}
of the PDG
...
by associating $p$ with a new edge in $\dg M$ from $X$ to $Y$, with $\alpha_p \!=\! 0$.
%BEGIN_FOLD: comments
...
with standard gradient methods.  This approach is
