...
%by adding $p$ as an edge of $\dg M$ with $\alpha_p\!\!=\!0$%
by adding an edge labeled $p$ from $X$ to $Y$ to $\dg M$ with
$\alpha_p\!\!=\!0$. 
...
is $\bbr{\dg M}^*(Y \!\mid\! X)$.  Moreover, since the expression that
we want to minimize is smooth and strongly convex in $p$, we  can
calculate it with standard gradient methods.  This approach is
inefficient as written (since it involves computing the full joint
distribution  $\bbr{{\dg M}^-p}^*$), but we believe that good
approximation methods will allow us to draw inferences effiently.
...
%write it an exlain it.
	...
	probability 1 to $y$, to get the (possibly inconsistent) PDG ${\dg
