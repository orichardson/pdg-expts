...
%joe25: I can't distinguish \var 1 from a regular 1.  Can we use a
...
%oli27: "onlyl"
% the special variable $\var 1$ whose range consists of onlyl $\star$,
the special variable $\var 1$ whose range consists of only $\star$,
...
% reason the entire paragaph expands and it ends up costing more than one line)
%joe25
%\end{itemize}
...
%oli27: I used "corresponding to" instead of "when" because to me "when" makes it
...
(recall that we treat an unweigthed PDG $(\Gr,\mat p)$ 
semantically like the weighted PDG  $(\Gr,\mat p,  \mat 1, \mat 1)$).
...
poses no problem for a PDG trying to emulate a factor graph. 
...
Conversely, the fact that the reverse correspondence requires $\gamma=1$ suggests
...
%oli27: I'm sure what I have to say here can be done with the scoring function,
% and I view that as hugely important: it shows that what we've covered here lays
...
How should we query a PDG for probabilistic information? How should a PDG 
respond to new information, and resolve its inconsistencies?
%oli27:
% It turns out that these issues are closely related.  
These issues turn out to be closely related,
and fundamentally connected to the PDG scoring function.
%oli27:
...
and just briefly give some intuitions here.
...
%answer $p(Y|X)$ may be judged via the best score of the PDG obtained
answer $p(Y \!\mid\! X)$ may be judged via the best possible score of any distribution,
with respect to the PDG
%oli27: changing notation back to union; it's clearer, and does not conflict with
...
%	doing infrence on it. I think this is true because now we have this metric
%	of quality, which is amenable to approximation. The approximation is critical. 
...
Nevertheless, to the extent that we can estimate
%%joe25
%%$\mathit{score}_\gamma$ efficiently
%oli27: it's not so clearly "the" scrore, but a variant one we just described,
%	 so I'm going to try to hold out with my preposition a wee bit longer...
...
%oli27: The cpd has some parameterization, and so do the rest of the cpds in M.
% If rather than modifiying the paramteters of p, we modify the parmeters of the
...
\commentout{
	...
	the case where $X = \var 1$, an observation $Y\!=\!y$  can be added to
	$\dg M$ in the form of an edge $\ed{\delta_y}{\var 1}Y$, getting the
...
% 
If, rather than fixing $\dg M$ and varying $p$, we fix $p$ and optimize the
parameters of the other cpds of $\dg M$, the same process computes an update,
rather than inference. For example, to condition on an observation $Y\!=\!y$, we
can add to $\dg M$ an edge $\var 1 \tto Y$, whose cpd is the degenerate
distribution $\delta_y$, thereby obtaining the (possibly inconsistent) PDG ${\dg
M}_{Y=y}$.  The distribution $\bbr{{\dg M}_{Y=y}}^*$ turns out to be the result
of conditioning $\bbr{\dg M}^*$ on $Y\!=\!y$.
...
inconsistent until fully trained) this situation cannot be represented with other
graphical models.
