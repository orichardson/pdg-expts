...
% from a huge number of samples.
and the other edges are randomly initialized networks.
% In other cases, such as the one detailed below, different choices of $\beta$ may be appropriate for the data distribution.
...
% for instance, the Bhattacharya distance: a loss that is much smaller if the the model does not generate a training point ($\exists x. p(x) \ll q(x)$).
