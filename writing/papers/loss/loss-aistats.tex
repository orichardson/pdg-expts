...
%TODO: For other\beta
%
But even specify everything with $\beta =\infty$ may not be enough to nail everything down.
...
the analogous modification of PDG in \Cref{prop:supervised-cross-entropy}, and models $\dg S$upervised $\dg L$earning
for an arbitrary loss function $\ell$ (assume $\ell(y,y') \ge 0$ with equality iff $y=y'$).
\def\mypdg{\dg{S}\!\dg{L}}
\[
	\mypdg
	:=
...
The optimal $p(Y|x)$ by minimizing $L$ is a point mass on the label $y^*_x$ with minimal expected loss given $x$, but the optimal $p$ according to $\aar{\mypdg}$ is $\datadist\xysamp(Y|X)$, which is \emph{calibrated} \parencite{}.
We can still get $L$ as $\aar{\mypdg}_\infty$ by strictly enforcing the causal picture (i.e., setting $\alpha_p, \alpha_{\datadist\xysamp} := 1$).
