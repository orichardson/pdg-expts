...
\aistatstitle{ Loss as the Inconsistency of a Probabilistic Dependency Graph: \\ Choose your Model, not your Loss}
...
function which the system minimizes. A wide variety of losses are used in pratice.
...
the choice is usually made by empirics, tradition, and an instinctive calculus acquired through the practice---not explicitly laying out beliefs.
Furthermore, there is something to be gained by fiddling with these
loss functions: one can add regularization terms, to (dis)incentivize
(un)desirable behavior.
...
What if we replace $x$ with an empirical distribution on many samples?
...
In the (more common) \emph{supervised} setting, we train \emph{discriminative} models to predict $Y$ from $X$, from labeled samples $\{(x_i,y_i)\}_i$.
...
Perhaps most importantly, we would like to have low \emph{reconstruction error} \eqref{eq:rec}---when we decode an encoded image, we would like it to be reasonably similar to the original.
\vspace{-0.5em}
\begin{equation}
...
%have argued
argue
that one might want to weight the reconstruction error \eqref{eq:rec} and
% the KL term
the `KL term'
...
and measures how far the product of factors is from being a probability distribution.
...
\section{RE\kern-0.07emV\kern-0.05emERSE-EN\kern-0.07emG\kern-0.05emINEERIN\kern-0.07emG A L\kern-0.05emOSS}
...
Now, given a cost function
$c: \V(X) \to \mathbb R_{\ge 0}$,
% $f(X)$,
define the cpd $\tilde c(\Truth |X)$ by
$
	\tilde c(\trut | x) := \exp( - c(x)).
$
$\tilde c$ ties cost to inconsistency, by threatening to generate the falsehood {\tt f}, in proportion to cost.
% \TODO
...
% But that doesnt mean \ldots TODO.
...
%TODO: For other\beta
%
But even for those willing to specify everything with infinite confidence,
For a more subtle illustration, consider the following PDG, which is
...
the analogous trick applied to the natural modification of the PDG in \Cref{prop:supervised-cross-entropy}
for an arbitrary loss function $\ell$ for which ($\ell(y,y') \ge 0$ with equality iff $y=y'$).
\def\mypdg{\dg{S}\!\dg{L}}
\[
	\mypdg
	:=
	...
	\begin{tikzpicture}[center base]
		...
		% 	node[below, pos=0.6, inner sep=2pt]{${\color{gray}\scriptstyle(\infty)}$}
		% 	+(-1, 0);
		\unmergearr[arr2]{dstart}{X}{Yt}
			\node[above=2pt of center-dstartXYt] {$\datadist\xysamp$};
			\node[below right=2pt and 0.5pt of center-dstartXYt, inner sep=0pt, rotate=30]
				{${\color{gray}\scriptscriptstyle(\infty)}$};

		\mergearr[arr2]{Yt}{Yp}{2}
			\node[above=2pt of center-YtYp2] {$\tilde\ell$};
...
you might think that $\aar{\mypdg}$ is the expected classifier loss $L := \Ex_{x,y\sim \mathcal D}\Ex_{y' \sim p|x}\ell(y,y')$, but this is not so. It is arguably nicer.
The optimal $p$ by minimizing $L$ is a point mass on the label with minimal expected loss, but the optimal $p$ according to $\aar{\mypdg}$ is $\datadist\xysamp(Y|X)$, which is \emph{calibrated.}
