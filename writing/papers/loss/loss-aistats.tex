...
We conflate a cpd's symbol with its edge label, so we draw the PDG with a single edge attached to $f(Y|X)$ as
...
information
needed to separately describe the target of each edge $L$ given its source ($\alpha_L$ times), beyond the information needed to fully describe a sample from $\mu$.
...
In general, PDG inconsistency can be viewed as a vast generalization of divergences to arbitrary structured objects.
...
The exponential families generated by weigted factor graphs
...
%joe1*: I *strongly* suggest remactuallyg this paragraph.   You have
...
% TODO Fill out acknowledgements
...
\begin{lproof}\label{proof:inc-two-gaussians}
	...
	\begin{align*}
		\aar{\dg M} &= \inf_\mu \Ex_{x\sim D}\Ex_{y \sim\mu|x}
			...
			&= \inf_\mu \Ex_{x\sim D}\Ex_{y \sim\mu|x}
			 	\left[ \log \mu(y \mid x)^{\beta_1 + \beta_2}
					\begin{array}{ll}
					+ \frac{\beta_1}{2} \left( \frac{y-f(x)}{t(x)} \right)^2 &+
					 	\frac{\beta_2}{2} \left( \frac{y-h(x)}{s(x)} \right)^2 \\
					+ \beta_1 \log( t(x) \sqrt{2\pi}) &+ \beta_2 \log( s(x) \sqrt{2\pi})
					\end{array}
					\right].
	\end{align*}
	At this point, we make use of the fact that the sum of two parabolas is itself a parabola. Concretely, we apply \Cref{claim:parabola-reparam}, whose statement and proof are at the end of the present proof, to rewrite the two terms on the top right of the previous equation. Following the claim, define
	\[
	...
	\]
	Applying \cref{claim:parabola-reparam} leaves us with:
	\begin{align*}
		\aar{\dg M}
			&= \inf_\mu \Ex_{x\sim D}\Ex_{y \sim\mu|x}
				\left[ \log \mu(y \mid x)^{\beta_1 + \beta_2}
					...
					\right]
	\end{align*}
	Pulling the term on the top right, which does not depend on $Y$, out of the expectation, and folding the rest of the terms back inside the logarithm (and in particular, by replacing the top middle term $\varphi$ by $-\log(\exp(-\varphi))$), we obtain:
	\begin{align*}
	\aar{\dg M}
		&=\! \Ex_{x\sim D}\left[
		 	\begin{array}{l}
	...
	% On the right, we can shuffle the factor $(\beta_1+\beta_2)$ into the exponent, which for the denominator means just removing the hats from the $\beta$s. We now have:
		...
		\right]
	\end{align*}
	Whew! Taking the square root out of the log proves the tedious technical half of the proposition.
	Now, we massage it into into a more readable form.

	To start, let $\hat\beta$ without the subscript denote the vector $(\hat\beta_2,\hat\beta_1) = (\frac{\beta_2}{\beta_1+\beta_2}, \frac{\beta_1}{\beta_1+\beta_2})$, which we will use for weighted means.
	...
	\end{align*}
	Which means \eqref{eq:gauss-midform} can be written as
	\begin{align*}
		\aar{\dg M} &=\! \Ex_{x\sim D}\left[
			(\beta_1 \!+\! \beta_2) \log \frac
				{ \mathrm{QM}_{\hat\beta}(\sigma_1,\sigma_2) }
		...
		% 	\right)
	\end{align*}
	\ldots which is slightly more comprehensible, and proves the proposition statement.
	% \end{align*}
	% \xi(x) =
	% \frac{\beta_1\beta_2}{\beta_1 t(x)^2 + \beta_2 s(x)^2}\, \Big(f(x)-h(x)\Big)^{\!2}
...
\begin{lproof}\label{proof:MSE}
	An immediate corolary of \Cref{prop:inc-two-gaussians}.
\end{lproof}
...
\begin{lproof}\label{proof:fg-inconsistency-is-partition-function}
	...
	\end{equation}
	and note that if  $\gamma = 1$, and $\alpha,\beta$ are both equal to $\theta$ in $\PDGof{\Psi}$,
	 % (since $\PDGof{\Psi} = \PDGof{(\Phi,\theta)} = (\UPDGof\Phi, \theta,\theta)$), 
	the {\color{blue!50!red!90!black} middle term (in purple)} is zero, and since $\bp(Y|X) = \phi_L(\mathbf y_L)$, we obtain the standard variational free energy
	\[
		\VFE_\Psi(\mu) = \Ex_{\mu} \sum_{J} 
	\]
	Recall also that
	...
	\begin{align*}
		\aar*{\!\begin{tikzpicture}[center base]
			...
			\draw[arr2, <<-] (2) to
				...
				+(0.9,0);
		\end{tikzpicture}\!}
		&= \Ex_{\mu} \frac{\mu(X,{\tt T})}{\hat c({\tt t}\,|X)}
...
\TODO[ Example of \Cref{sec:reverse-engineer} ]
