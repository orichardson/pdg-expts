...
\relax %%%%% restatables and links
	...
	\newenvironment{linked}[3][]{%
		...
		% \hspace{2em}
			...
			\hyperref[proof:\linkedproof]{%
			\color{blue!50!white}
			\scaleleftright{$\Big[$}{\,\mbox{\small\raggedright\tt\begin{tabular}{@{}c@{}}
...
In the appendix, we also give the analogue for many samples, with a single application of the inequality and the multi-sample versions of \Cref{prop:pdg-elbo-vae}
...
which for $\beta = 1$ is equivalent to the ELBO from the previous section. The authors view it as a regularization parameter, annd argue that in some cases, you can do better with a stronger prior.
Sure enough, the $\beta$-VaE objective is , and the parameter $\beta$ parameter exactly coincides with $p$.\footnote{the two parameters even share a name, both coming from the usage of $\beta$ for inverse temperature in thermodynamics.}
