...
So the only way to selectively choose samples to make things consistent is to select a distribution $\mu$ on \emph{inputs}, so that it is always the case that $f(x) = g(x)$.
to change $h(Y|X)$.
...
% with respect to $h$.
Note that this is the \emph{exact opposite} of how we captured cross entropy (\cref{prop:supervised-cross-entropy}): there we were unwilling to budge on the data distribution, and must instead modify our predictor $h$.
