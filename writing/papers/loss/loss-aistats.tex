...
\begin{abstract}
	...
	that the ELBO, a difficult-to-motivate function for training variational
...
let $\PDGof{\Psi}$ be the PDG with edges $\{ \raisebox{-0.3ex}{$\smash{\stackrel{J}{\rightarrow}}$} \mathbf X_J \}_{\mathcal J}$, cpds $p_J(\mathbf X_J) \propto \phi_J(\mathbf X_J)$, and weights $\alpha_J \!=\! \beta_J \!=\! \theta_J$. 
...
The factored exponential families generated by weighted factor graphs are a cornerstone statistical mechanics, where $- \log Z_{\Psi}$ is known as the (Heimholz) free energy.
The principle of
...
% \end{linked}
% 
% Seldom do they simply combine aditively. 
