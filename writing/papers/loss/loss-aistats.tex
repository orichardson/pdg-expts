...
\relax %Header
	...
	\usepackage{amsthm,thmtools} % Theorem Packages
		...
		\declaretheorem[name=Corollary,parent=theorem]{coro}
	    \declaretheorem[name=Proposition,sibling=theorem,postheadhook={%
		% \ifmarginprooflinks\marginpar{\vspace{2ex}\usebox\marginprooflinkbox}\fi%
			}]{prop}
	...
	\relax %%%%% restatables and links
		...
		% \newcommand{\fixedmarginpar}[2]{%
	  	% 	\setbox1=\vtop{#2}\marginpar{\vspace{-\ht1}\vspace{#1}#2}%
		% }
		...
		\newenvironment{linked}[3][]{%
			...
			\sbox\marginprooflinkbox{%
				...
				% \vspace{1.5em} %% original size; but now I have small props
				% \vspace{2ex}%
				% \vspace{9ex}%
				\centering%
				\hyperref[proof:\linkedproof]{%
...
\begin{abstract}
	...
	% Finally, we illustrate how our approach may yield better loss functions in new settings.
\end{abstract}
...
% The fact that the incompatibility of $\dg M$ with a \emph{specific} joint distribution $\mu$ is an upper bound on the inconsistency is not a deep one, but it is of variational character.
% % but it does yield an efficient approximate inference procedure for PDGs: choose a tractable parametric family $\mathcal P$ of joint distributions $\mu$, and optimize \eqref{eq:semantics}. Making this precise is the focus of a parallel work.
...
\parencite{kingma2013autoencoding}, or VAE,
...
% Here, the analog of the ELBO becomes
% A VAE is trained with another form of the ELBO:
...
% $\mathcal L_2$ cannot be written as the inconsistency
...
% At first glance, the answer appears to be yes, although not without unsavory modeling choices.
% At first glance, the answer appears to be yes, but only by .
% The answer is yes, but not without unsavory modeling choices.
% The answer is yes, but not without unsavory modeling choices that don't compose well.
% The answer is yes, but not without unsavory modeling choices that compose poorly.
% The answer is yes---but not without unsavory modeling choices.
% The answer appears to be yes---but not in a satisfying way.
% The answer appears to be yes---but not without making unsavory modeling choices.
...
% So unlike $\ell$, it is calibrated.
...
% and also dragged the global parameter $\gamma$, which is technically not even part of a PDG.
What's more, we've dragged in the global parameter $\gamma$,
% (technically not even part of the PDG)
...
% while desirable behavior as we compose pieces of models together (\Cref{sec:datsim}),
% while the losses generated by composing PDGs,
% To summarize: while the inconsistency of a composite model often seems to be the ``right'' loss function,
To summarize: while model inconsistency readily generates appropriate loss functions,
% encoding loss functions as PDGs may not yeild nice models.
% the reverse does not work as well.
the converse does not work as well.
% Although reverse-enerineering a loss is possible, doing so may require making questionable modeling choices with absolute certainty, resulting brittle model with gimped potential for composition.
...
% In the end, we must confront our modeling choices, and this framework forces us to put them on display.
% Good loss functions come from good models.
