...
One can add the variable $\Truth$, whose values are $\{\trut, \truf\}$, and the event $\Truth \!\!=\! \trut$ into any PDG, without semantic impact.
...
\begin{linked}{prop}{expected-cost}
	...
	\]
\end{linked}
So \emph{every} cost function arises as an an inconsistency. 
...
But it is still not easy to simply reverse-engineer functions without tyranical micromanagement.
% Above, we had to set $\beta_p := \infty$.
Above, setting $\beta_p := \infty$ was necessary.
For a more subtle illustration, consider the following PDG, which is the natural extension of \Cref{prop:supervised-cross-entropy} along the same lines. 
\def\mypdg{\dg{S\!\!L}\mathrm{earn}}
\[
	\mypdg
	:=
	% \aar*{
