...
For instance, one key component of a machine learning system is a `loss function' --- the 
...
\begin{defn}
    ...
    the moderler's confidence in the  $X$.
...
Most directly, a PDG $\dg M$ determines two scoring functions on joint distributions, one qualitative and one quantitative. The qualitative score, called the \emph{incompatibility} of $\mu$ with $\dg M$, is given by
\begin{equation}
...
There is more than one coherent interpretation of this expression; we will give one in terms of codes.
% We are primarily concerned with the second term of \eqref{eq:semantics}, which
...
But if one uses edges in proportion to the condfidence one has in them, then violations of high-confidence cpds are compounded, and hence more costly.
