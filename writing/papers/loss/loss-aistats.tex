...
\aistatstitle{Natural Loss Functions from Probabilistic Dependency Graphs: Choose your Model, not your Loss}
...
\begin{abstract}
    ...
    Probabilistic Depencency Graphs (PDGs) are probabilistic models that can capture inconsistency.
    We prove that many standard objectives are the incinstencies of the natural PDG describing the scenario at hand, and corroborate an existing connection between regularizaters and priors.
    % We then focus on two surprising examples.
    We then show that PDGs generate a large class of statistical divergences, and detail the benefits of doing so, including a visual proof language for proving inequalities between them.
    Finally, we focus turn to variational inference, and show that the ELBO, a difficult-to-motivate function for training Variational models, arises for free out of the modeling assumptions, as do an intuitive visual proofs of the variational bounds used to motivate it. 
\end{abstract}
...
the inconsistency of the PDG containing just a distribution $p(X)$ and a sample $x$ happens to be the standard relationship between a distribution $p$ and sample $x$ --- and it is even known known as ``surprise'', a particular kind of cognitive dissonance.
...
\begin{linked}{prop}{expected-surprise}
    ...
    If 
    $p(X)$ is a model, and $\xsamp = \{ x_i \}_{i=1}^m$ are samples with empirical distribution $\datadist\xsamp$, then
    %
    \begin{align*}
...
In fact, PDG containing both a continuous distribution and a finitely supported one will generally have infinite inconsistency.
...
\begin{linked}[log accuracy as inconsistency]
    ...
    Consider $D(X)$ over inputs $X$, and functions $f,h : X \to Y$, where $h$ is a predictor and $f$ generates the true labels. The inconsistency of believing all three is:
...
\)
% \end{equation*}
and is known as the \emph{partition function}, and is computing it is intimately related to many probabilistic inference tasks.
...
Outside of computer science, such factored exponential families form the mathematical backbone of statistical mechanics, and in this world $- \log Z_{\Psi}$ is the Heimholtz free energy. The principle of free-energy minimization has been enormously succesful in describing the evolution of chemical systems.
