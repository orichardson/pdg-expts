...
We show (\Cref{prop:pdg-elbo-x}) that $\mathrm{ELBO}(x)$ is also precisely the inconsistency of a PDG containing the probabilistic information of the autoencoder ($p, d$, and $e$) and the sample $x$.
...
and also that it (always) equals a certain value $x$. These beliefs are entirely consistent if $p(X=x) = 1$, and become more inconsistent
%(in our sense)
 as $p(X = x)$ decreases.
...
%
Even so, it is nice that
% With a little more focus on the context and less on the algebra, perhaps \Cref{prop:pdg-Ix} becomes more curous:
 the inconsistency of the PDG containing just a distribution $p(X)$ and a sample $x$ just so happens to equal the overlwhelming favorite measure of incompatibility between a distribution $p$ and sample $x$ --- and it is known as ``surprise'', a particular kind of cognitive dissonance.
...
\begin{linked}{prop}{expected-surprise}
	Given a model determining a probability distribution with mass function $p(X)$, and samples $\xsamp = \{ x_i \}_{i=1}^m$ determining an empirical distribution $\datadist\xsamp$,  then
    %
    \begin{align*}
...
In much the same way as \cref{prop:expected-surprise} generalizes \cref{prop:pdg-Ix} by considering more than one sample at once, we also can obtain a multi-sample analogue of analog of \Cref{prop:marginal-ll}.
...
\begin{linked}{prop}{supervised-cross-entropy}
    ...
    Consider a probabilistic predictor $f(Y \mid X)$. The inconsistency of the PDG containing $f$ and the empirical distribution $\datadist\xysamp$ of samples $\xysamp$ is equal to the cross-entropy loss, (plus the empirical uncertainty in $Y$ given $X$, a constant that depends only on the data). That is,
	\[ \aar**{
		\begin{tikzpicture}[center base]
			\node[dpad0] (Y) {$Y$};
...
\cref{prop:pdg-Ix,prop:expected-surprise,prop:marginal-ll,prop:pdg-loglikelihood,prop:supervised-cross-entropy} all require the distribution to be represented with a mass function (instead of a density function).
In fact, a PDG containing both a continuous distribution and a finitely supported one wil generally be \emph{infinitely} inconsistent.
...
However, such a rescaling ultimately only adds a constant to the quantity in question.
Similarly, any approximation of a density $p(X)$ with a discretization $\tilde p_k(X)$ where $k$ discretization size, only contributes an additive constant dependent on $k$  for the purposes of \cref{prop:pdg-Ix,prop:expected-surprise,prop:marginal-ll,prop:pdg-loglikelihood,prop:supervised-cross-entropy}.
But this additive constant is irrelevant for optimization, and so in this sense the results here also justify use of the continuous analogues (such as $- \log p(X)$ for a pdf $p$, or differential entropy) as loss functions, even though these functions are not invariant with respect to changes of parameterization.
\end{remark}
...
\begin{linked}[log accuracy as inconsistency]
    ...
    Consider a distribution $D(X)$ over inputs, and functions $f,h : X \to Y$, where $h$ is a predictor and $f$ generates the true labels. The inconsistency of believing all three is:
...
\begin{linked}[Mean Square Error as Inconsistency]{prop}{MSE}% \label{coro:MSE}
	\begin{align*}
		% \aar**{\begin{tikzpicture}[center base]
		% 	\node[dpad0] (Y) {$Y$};
...
Thus the inconsistency of a PDG comprising $p$ and $q$ is a divergence measure.
...
It turns out that a large classs of statistical distances arise as the inconsistency of such a PDG.
We start with the one that we've already seen.
...
To demonstrate, we begin by showing that the `Evidence Lower BOund' (ELBO), a common objective for training latent variable models, arises naturally as a PDG inconsistency.
Suppose we have a joint distribution $p(X,Z)$, but only have access to observations $X$. Evaluating $p(X)$ requires marginalizing over $Z$, and could be intractable if $Z$ complicated.
...
Nevertheless, if we interpret $p$ and $q$ are mass functions, it turns out that the ELBO also arises naturally as the inconsistency of the obvious PDG modeling the data of the VaE.
...
We submit that this presentation is clearer than the standard one. The second PDG has more edges so it is clearly at least as inconsistent, and one can see that equality holds when $q(Z) = p(Z)$, because that is the marginal on $Z$ in the first PDG anyway.
...
\begin{linked}{prop}{pdg-elbo-vae}
	The conditional ELBO used as a VaE objective is the inconsistency of the PDG containing the encoder $e$, decoder $d$ prior $p$, and a sample $x$.\dfootnote{See appendix, for a multi-sample analog.}
	That is,
    \vspace{-1.8em}
	\[
...
Sure enough, the $\beta$-VaE objective is the inconsistency of same PDG as before, but with confidence $\beta$ in $p(Z)$.\footnote{the two parameters even share a name, both coming from thermodynamic $\beta$ (inverse temperature).}
...
So PDG semantics capture not only structured objects such as Bayesian Networks and Factor Graphs as in \cite{richardson2020probabilistic}, but also in the same stroke grant VaEs legitimate graphical-model-hood, blurring the line between model and objective.
