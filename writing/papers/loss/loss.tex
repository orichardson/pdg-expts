...
The first step in the proof of \cref{prop:pdg-Ix}, is no longer valid, but we can get the same effect by dramatically increasing our confidence in the data distribution.
...
So far we have been considering models of a distribution $p(X)$, which correspond to generative models, or an unsupervised setting. The supervised setting, in which our model predicts $Y$ from $X$, and is trained from joint samples ($X,Y$), cross entropy is perhaps even more dominant. And equals the inconsistency of a PDG consisting of the predictor $f(Y|X)$ and high-confidence data. 
...
\begin{linked}{prop}{supervised-cross-entropy}
	...
	\[ \aar**{
		...
		\end{tikzpicture}} = \frac1{|\xysamp|}\sum_{(x,y) \in \xysamp} \log \frac1{f(y \mid x)} \quad + \H_{\datadist\xysamp}(Y\mid X).
	\]
\end{linked}
...
\begin{lproof} \label{proof:supervised-cross-entropy}
	...
	\Ex_{x \sim \datadist\xysamp} \kldiv[\Big]{\datadist\xysamp(Y\mid x)}{f(Y\mid x)}
		...
		&= \frac1{|\xysamp|}\sum_{(x,y) \in \xysamp} \log \frac1{f(y \mid x)} \quad + \H_{\datadist\xysamp}(Y\mid X)
	\end{align*}
	\[  \]
\end{lproof}
...
\begin{lproof}\label{proof:pdg-elbo-vae-whole}
