...
exists and is unique for small enough $\gamma$, including in the limit as $\gamma \to 0$. We write $\bbr{\dg M}^*$, without a subscript, to indicate the limit of small $\gamma$. 
...
\begin{figure}
	...
	\begin{tikzpicture}[scale=1.8]
		...
		% points
		\fill (0.5,0.5) circle (0.05) node[above right, align=center]
			% {Bhattacharya\\distance};
			% {$\thickD^{\text{Bhattacharya}}$};
			{$\thickD_{B}(p,q)$};
		\fill (1,3.2) circle (0.05) 
			node[below]{$\vdots$}
			node[above, align=center]{$\kldiv qp$};
		\fill (6.2,1) circle (0.05)
			node[left]{$\cdots$}
			node[right, align=center] {$\kldiv pq$};
		\fill (2,-1) circle (0.05) node[below right, align=center]
			{$\chi^2\infdivx pq$};
		\fill (0,1) circle (0.05) 
			node[above right, align=center, 
				% label={[yshift=-1ex]\footnotesize min entropy}
				]
			{$\I_q(p > 0)$};
		\fill[gray, fill opacity=0.2] (-1,1) -- (1.5,-1.5) -- (-1,-1.5) --cycle;
		\draw[gray, opacity=0.5, thick] (-1,1) -- (1.5,-1.5);
		

	\end{tikzpicture}
	\caption{A map of inconsistency 
	% $\aar[\bigg]{\begin{tikzpicture}[center base]
	% 	\node[dpad0] (X) {$X$};
	% 	\draw[arr2, <-] (X) --
	% 			node[above, pos=0.6, inner sep=2pt, align=center] {$p$} 
	...
	% 		 ++(1.1, 0);
	% \end{tikzpicture}}$ 
	as we vary the confidences $r = \beta_p$ and $s = \beta_q$. }
	\label{}
\end{figure}
