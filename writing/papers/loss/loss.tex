...
It turns out that PDG inconsistencies, as we vary the confidences, essentially carve out the same class: every R\'enyi entropy is the inconsistency of some PDG of this form, and every PDG divergence for concrete values of $\beta$ is a scaled R\'enyi entropy.
...
However, the two classes are not exactly identical, because the PDG divergences admit extra limit points. One of the biggest differences is that the reverse KL divergence $\kldiv q p$ is not a Renyi entropy of the form $\thickD_\alpha\infdivx p q$ for any value (or limit) of $\alpha$. This lack of symmetry has caused other authors \cite{} to instead work with a re-scaled symmetric version of the R\'enyi entropy, called $\alpha$-divergence.  The relationship between them can be seen in \cref{fig:statdistmap}.

Furthermore, the PDG view allows us to 

The Chernoff information 
\begin{coro}[Chernoff Divergences]
\[
	\inf_{\beta \in (0,1)}
	\aar[\bigg]{\begin{tikzpicture}[center base]
		\node[dpad0] (X) {$X$};
