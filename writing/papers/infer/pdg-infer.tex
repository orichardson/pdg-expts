...
the case where $X = \pdgunit$, an observation $Y\!=\!y$  can be added to
$\dg M$ in the form of an edge $\ed{\delta_y}{\pdgunit}Y$, getting the
(possibly inconsistent) PDG $\dg M'$. One might imagine that the distribution
$\bbr{\dg M'}^*$ is the result of conditioning the best distribution $\bbr{\dg M}^*$ on $Y\!=\!y$, as is true for factor graphs, but this is not always the case.
