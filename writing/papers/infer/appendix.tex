...
This algorithm is not very fast, but it is guaranteed to make progress, since
...
One can see in \cref{fig:gamma-v-gap-fine}, for example, that when $\gamma = 2 > 1 = \max_a {(\beta_a / \alpha_a)}$, the problem is concave, which creates
significant difficulties for the gradient-based methods.
CCCP performs better, and the only reason it doesn't do significantly better than it does, is because we restrict to a mere five iterations to collect the data. 
...
This explains why, even though $\bbr{\dg M}_\gamma$ is infinitely differentiable in $\mu$ and $\gamma$-strongly convex, it can still be challenging to optimize, especially when the $\beta$'s are very different, or when $\gamma$ is small.
...
\subsection{Comparison with Black-Box Optimizers, on Joint Distributions.}
...
\begin{itemize}
    ...
    \begin{itemize}[nosep]
        ...
        \item \texttt{n\_params}$\,:= \V\!\Ar$, the total number of parameters across all the cpds of $\dg M$, and
        \item \texttt{n\_worlds}$\,:= \V\!\X$, the size of the joint distributions on the variables of $\dg M$.
    \end{itemize}
    ...
    Let $\theta = [\theta_{\mat x}]_{\mat x \in \V\X} \in \mathbb R^{\V\X}$, be a vector of optimization variables, and define
    $\displaystyle
...
\begin{figure}
    ...
    \caption{
        ...
        The ExpCone methods \texttt{cvx\_opt\_joint} are split into two variants, depending on whether or not it also computed the second step described in \cref{sec:empirical-limit} to account for $\SInc_{}$.
        The CCCP variants are \texttt{cccp\_opt\_joint} split into regimes where the entire problem is convex ($\gamma \le 1$, \verb|cccp-VEX|), and the entire problem is concave ($\gamma > 1$, \verb|cccp-CAVE|).
