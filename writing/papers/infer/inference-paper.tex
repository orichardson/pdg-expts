...
the extent to which $\mu$ fails to model
...
% $\alpha_a$
a situation in which each unit of $\alpha$ corresponds to an independent causal mechanism
that determines its target variables given its source variables.
...
in which observational data dominates, and structural information is used only to break ties.
% When $\gamma$ is small enough.
So long as $\bbeta > \mat 0$,
there is a unique optimal distribution $\bbr{\dg M}^*$ in the limit, which we call the \emph{\obslimit\ of $\dg M$}. 
%
...
we are referring to the limit as $\gamma\to 0$, which, unlike before, obeys $\aar{\dg M} = \aar{\dg M}_0$.
As previously mentioned, when $\dg M$ models a common learning scenario, 
the inconsistency $\aar{\dg M}$ is typically the standard loss function used in that setting \parencite{one-true-loss}.
...
So a solution to this exponential conic program calculates the inconsistency of $\dg M$, and finds a distribution that minimizes $\Inc$. 
...
$[\mu_C(c)]_{C \in \mathcal C, c \in C}$.
...
\begin{align*}
    ...
    \text{\sf subject to:}&\quad
        ...
        \forall a \in \Ar.~ 
            \big(&\!- \! \mat u_a,\, \mu_{C\!_a}\!(\Src a,\mskip-2mu \Tgt L), \mu_{C\!_a}\!(\Src a) p\ssub L(\Tgt a | \Src a)\big) \in K_{\exp} \\
...
We have now found a minimizer of $\Inc$ that does not require storing a full joint distribution.
...
% \def\Pash{\mathit{P\!S}}
\def\Pash{\mathit{V\mskip-4muC\mskip-3muP}}
...
the set of $\mathbf v$ariables that $\mathbf c$luster $C$ has in common with its $\mathbf p$arent cluster.
...
\begin{align*}
    ...
    &= \sum_{C \in \cal C}
        \sum_{c \in \V(C)} \mu_C(C{=}c) \log \frac
            {\mu_C(C{=}c)}{\mu_C(\Pash_C{=}
            \Pash_C(c) )}
\end{align*}
%
% This means that
...
\cref{prop:cluster+idef} shows that finding a compact representation of that optimize the PDG scoring function \eqref{eqn:scoring-fn} can be written as a convex optimization problem with a
