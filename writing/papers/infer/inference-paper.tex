...
% I have no problem using \cdot, and have changed it below. 
...
For graphs with bounded tree-width, many problems 
%oli4: elaborating on the citation.
(in fact, any expressible in a certain second-order logic \parencite{courcelle1990})
can be solved in
linear time,
 % \parencite{courcelle1990}
including inference in standard graphical models.
...
% $\bbeta = [\beta_a]_{a \in \Ar} \in \Rext^\Ar$
    ...
    \begin{itemize}[itemsep=0pt]
        ...
        \item a weight $\beta_a \in \Rext$ indicating
            the modeler's confidence in the cpd $p\ssub a(\Tgt a | \Src a)$, and
...
% \TODO[intuition]
%oli4: added your paragraph from the email:
...
information, we have two loss functions, which associate with a measure
...
%oli4: I think your version is clunkier than mine. It's not a distance, so you need quotes, while "discrepancy" is accurate. The words "can be viewed as" also are an unnecessary hedge in my opinion. Quite literally, it measures the discrepancy (even if there are other ways to measure it).  I'm reinstating my version.
...
The second scoring function measures a more structural kind
of discrepancy between $\mu$ and $\dg M$:
...
%oli4: originally added example to clarify, but I think it clogged things up a little,
% so I'm leaving it commented out.
\discard{\color{rewrite} 
For example, a causal picture $\Ar = \{ \to X, Y \gets\}$ with one unit of $\alpha$ for each edge cannot result in a distribution $\mu$ where $X=Y$ and both are uniformly distributed; such a distribution is missing ....
% the only way two independent mechanisms can both generate the same variable $X$ unconditionally is if $X$ is deterministic, 
% and so a PDG consisting of a single edge $\ed{}{}{X}$, with $\alpha = 2$ cannot give rise to .
}
%
...
indexed by a trade-off parameter $\gamma \ge 0$ that controls the importance of
...
%joe4
% The notation 
$\bbr{\dg M}^*_\gamma := \argmin_\mu \bbr{\dg M}_\gamma(\mu)$ denotes the set of optimal distributions at a particular value $\gamma$.
...
{\color{purple!80!black}
    ...
    $\gamma$-inference for the specific case of PDGs satisfying $\bbeta \ge \gamma \balpha$, which is sufficient to ensure strict convexity of \eqref{eqn:scoring-fn}, and hence a unique optimal distribution.}
%
...
%oli4*: The secondary thread of our story is that we can compute << M >>, the inconsistency.  I think this is sucn an important is an important concept that it demands definition here; proposition 1 seems like an inappropriate to me, because one might skip over proposition 1, and really this is core background material about PDGs. 
% As for being a loss function, I'll grant that, we've already said as
% much about why we care about it, but I thought it would be good to follow back up
% on this now that we can talk about it more formally.  
...
which allow more expressive constraints,
...
%oli4: I can't find where you put it!  I'm keeping it here for now so it doesn't get lost.
% But also I can't imagine where it would fit better. We've just introduced the exponential cone and I'm giving you the intuition you keep asking for!
...
\end{equation}
where $\mat c \in \Rext^{3n}$ is some cost vector, and $\mat b \in \Rext^m$, $\mat A \in \Rext^{m \times 3n}$ describe linear constraints.
...
%oli4: that doesn't mean we shouldn't include history. This content is what Chris told
% me to write here, so we might want to check with him before deleting it. 
%
...
The reason for doing so is that a dcp program can be efficiently compiled to a
...
Because all of our convex programs are linear programs with dcp exponential cone constraints, they need not be in exactly the form of \eqref{eq:exp-conic-program}.
...
we can calculate $\Pr_{\cal M}(\X)$ if we grant ourselves
 % the luxury of storing a joint distribution, as we do in this section.
...
When $\bbeta < \balpha$, we still have a way to find \emph{some} local optimizer via the concave-convex procedure, which we implement and discuss at length in \cref{sec:larger-gamma}---although this alone is insufficient for inference, as we have defined it.
...
every summand of the last term is a negative conditional entropy, and can be easily converted to an exponential cone.
...
but we are particularly interested in the distribution $\bbr{\dg M}^*$, which, uniquely among them, best minimizes $\IDef{\dg M}$.
...
% More concretely
Our approach to inference computes the inconsistency $\aar{\dg M}_\gamma$ as a side effect of finding distributions of interest---but suppose we were only interested in calculating $\aar{\dg M}$.
