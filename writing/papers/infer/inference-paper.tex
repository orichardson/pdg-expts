...
% "fix it" here;  Based on our conversation, it seems I should rename it name to
% "structural score" or "structural loss", since structural incompatibility means
% something else to us intuitively. 
%oli5: but 
...
\begin{align*}
    ...
    % \bbr{\dg M}_\zogamma(\mu) 
        ...
        % \right] - \gamma \H(\mu)
        %  % \right]
        % =&(1-\zogamma) \OInc_{\dg M}(\mu) + \zogamma \, \CDef_{\dg M}(\mu)        
        .
\end{align*}
...
\text{\sf subject to:}&\quad
    ...
    % \forall C \in \mathdcal C.& ~~(-\mat v_C, \mu_C, \mu_C(\Pash_C)) \in K_{\exp}^{\V C}
...
%oli5: I'm not a fan of how the grammar works out here. Reverting to something more neutral until we can agree on the notation.
...
\TODO[
\hspace{-3em} REVIEW: the result \cref{prop:consistent-NP-hard} has nothing to do with tree width; a proper lower bound in this section ought to say we can't do better with bounded tree width, right? So is this the right place for it? ]
