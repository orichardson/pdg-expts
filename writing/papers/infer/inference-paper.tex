...
% minimizing the inconsistency of a PDG 
    ...
    It follows that the training process in machine learning can largely be conceptualized as inconsistency minimization.
...
Under a standard interpretation of relative entropy $\kldiv{\mu}{p} := \Ex_{\mu}[\log \frac\mu p]$, 
...
The second scoring function measures a more structural kind of discrepency between $\mu$ and $\dg M$: 
...
% a situation in which each arc $a \in \Ar$ 
% $\alpha_a$
...
a situation in which each unit of $\alpha_a$ corresponds to an independent causal mechanism that determines $\Tgt a$ based on $\Src a$. 
