...
%  \aistatstitle{Inference in Probabilistic Dependency Graphs,\\
  ...
  %oli3:
    % \aistatstitle{Inference in Probabilistic Dependency Graphs}
    \aistatstitle{Inference in Probabilistic Dependency Graphs,
        via Exponential Conic Programs}
...
Indeed, the best we could hope for would be tractability on the restricted
...
While the many approaches to exact inference in standard graphical models have largely turned out to be different perspectives on the same mathematics \parencite[\S9-11]{koller2009probabilistic},
...
Thus, [...] is not only a theoretical result, 
but we also get efficiency out of the box. 
...
%oli2: a compromise? 
If the same algorithms are na{\"i}vely applied to graphs with cycles (such as loopy belief propagation), 
...
% For belief propagation in particular, is possible to save time by distilling the answers to all possible queries via a data-structure called a \emph{clique tree}
...
exponential cones: an exponential cone constraint is dcp 
...
\begin{align*}
    f(\mu_\tplus1) \!+\! g(\mu_\tplus1) &\le  f(\mu_\tplus1) \!+\! (\mu_\tplus1 \!-\! \mu_t)^{\sf T} \nabla g(\mu_t) \!+\! g(\mu_t)
        % &\text{(concavity of $g$)}
        \\
