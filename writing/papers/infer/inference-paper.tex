...
% As a consequence, 
%
% By analogy to other graphical models,
% To put the question on the same footing as other graphical models, we make the following definition.
% To allow for both answers, we only 
Because both notions are defensible, 
% we show in 
we provide an algorithm that solves the marginal inference task. 

% By inference in a PDG, we mean respoding to probabilisty qureies $Q(Y|X)$ for (sets of) variables $X,Y \subset \X$
\begin{defn}
    A PDG inference algorithm is a procedure 
    that takes as in put (1) a PDG $\dg M$ over variables $\X$,
    % a non-negative number $\gamma \ge 0$ or $0^+$,
    % either a non-negative number $\gamma \ge 0$ or $\gamma = $``$0^+$'',
    (2) a value for $\gamma \in \mathbb R_{\ge 0} \cup \{0^+\}$, 
    and (3) a probabilistic query of the form $Q(Y|X{=}x)$, 
    ...
    which is the conditional marginal
    of some $\mu \in \bbr{\dg M}^*_\gamma$  (if $\gamma$ is a number), 
    or $\bbr{\dg M}^*(Y|X{=}x)$ (if $\gamma = 0^+$).
\end{defn}

% If we want  as other graphical models, we make the following definition.


To get an algorithm that 
