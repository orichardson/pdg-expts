...
{\color{gray} If $\mat u = [u_a]_{a \in A}$ is a vector over $A$ and $\mat v = [v_b]_{b \in B}$ is a vector over $B$, then $\mat u \mathbin{\otimes} \mat v := [ u_a \cdot v_b ]_{a \in A, b \in B}$ is a vector over $A \times B$.}
...
A distribution $\mu \in \Delta S$ is often identified with its simplex representation, which is the vector $[\mu(S{=}s)]_{s \in S}$.
...
%
The dirac distribution $\delta_x$ denotes the degenerate distribution over $X$ which places all mass on the value $x \in \V X$.
%
...
%oli3: it can be a set, but only if the elements are not litterally
% subsets of V, because it needs to have duplicates.  I guess maybe ``corresponds to''
% is weak enough that I can say set here... but collection is the more generic term
% that allows us to avoid focusing on this irrelevant aspect, while being a little more technically precise. I'm happy to change it to "set" for the next draft if you insist, but it was intentional.
...
\begin{defn}
    ...
    is associated with a set $\Src a \subseteq N$ of source nodes, and a set $\Tgt a \subseteq N$ target nodes.
    %
    We often write $\ed aST$ to indicate an arc $a \in \mathcal A$ with $\Src a = S$ and $\Tgt a = T$.
\end{defn}
...
% tree decompositions intelligently.
    ...
    %     \item $\bbeta$
    \end{itemize}
    In aggregate form, $\balpha = [\alpha_a]_{a \in \Ar} \in \Rext^\Ar$ and $\bbeta = [\beta_a]_{a \in \Ar} \in \Rext^\Ar$ are weight vectors, and
    % $\mathcal P = \big[ p\ssub a(t|s) \big]{}^{a \in \Ar}_{(s,t) \in \V(\Src a,\Tgt a)}$
    % $\mathcal P = \big[ p\ssub a(t|s) \big]{}^{a \in \Ar}_{(s,t) \in \V(\Src a,\Tgt a)} \in \Rext^{\V\!\Ar}$
    $\mathcal P = [ p\ssub a(t|s)]_{(a,s,t) \in \V\!\Ar} \in \Rext^{\V\!\Ar}$
    % $\mathcal P = \{ p\ssub a\}_{a \in \Ar}$
    contains the cpds.
\end{defn}
...
We identify the event $X{=}x$ with the PDG containing a single
edge whose target is $X$, source is empty, and $p\ssub a(X) = \delta_x$.%
\footnote{for deterministic cpds such as this one, every choice of weights $(\alpha, \beta > 0)$ is semantically identical.}
...
\]
where $c \in \Rext^n$, $A\in \Rext^{m \times n}$,
and $b \in \Rext^m$.
 % describe a linear constraint.
...
% PDGs make a distinction between
the PDG formalism distinguishes between constraining so that $X{=}x$,
and returning the conditional distribution; this means that answering
\emph{conditional} probability queries are
...
\begin{example}
	...
	But some calculations reveal that $\bbr{\dg M^{+x}}^*$ only has support where $X{=}x$, it is proportional to $p(Y) q(x|Y)^{s/r}$.
...
% So even though a PDG
So, adding an event to a PDG is a different form of conditioning,
which is sensitive to confidence.
This has big implications for responding to conditional probability queries, because it means there can be a big difference between restricting to worlds where $Y{=}y$, and conditioning on $Y{=}y$.
% \end{enumerate}
% As a consequence,
...
approach to probabilistic modeling with PDGs: represent each piece of relevant information (including any data, priors, and also a current hypothesis model $h$) as a cpd with the appropriate confidence, and then alter $h(Y|X)$ by minimizing the overall inconsistency of the PDG (say, by gradient descent).
 % particular, this was suggested \textcite{pdg-aaai}.
...
that inference in PDGs is NP hard as well.
% One might imagine that \emph{minimizing} the inconsistency is the hard part, as opposed to noticing it.
% Might it easier to simply determine whether or not a PDG is inconsistent?
% It is not.
It turns out that
...
Recall that $\mat m \otimes 1 = [ \mat m, \cdots, \mat m] \in \Rext^{n \times k}$.
%
...
\subsection{%
    ...
    (small \texorpdfstring{$\boldsymbol\gamma \boldsymbol> \mat0$}{gamma > 0})
...
\textbf{Capacity.}
The black-box optimization approaches have an edge in that they can handle larger models (see the cutoffs in \cref{fig:resources,fig:gap-resource-fine}). 
