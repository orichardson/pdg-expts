...
% there has been no practical way to use a PDG to compute answers to probabilistic queries.
...
An inference algorithm is a way of answering probabilistic queries.
For general parameter settings, a PDG may not indicate just one distribution.
In these cases, inference amounts to describing properties of a given distribution that are true of 
...
\parencite{lubin}.
Our contribution here is to show that the problem of inference in a PDG 
...
First compute an explicit representation of the joint distribution $\Pr_{\cal M}(\X)$ over all variables; then marginalize to $\Pr_{\cal M}(X,Y)$, and condition on $X\!=\!x$.
%
% For other graphical models,
For a factor graph or BN, both steps are very straightforward;
...
Then in \cref{sec:small-gamma} we give a more complex but still straightforward translation of $\bbr{\dg M}_\gamma$ to a convex program when $\gamma$ is small. 
...
We use this to get a treewidth-based analogue of \cref{sec:small-gamma,sec:larger-gamma,sec:empirical-limit}.
