...
% \begin{align*}
$
    % \CInc_{\dg M}(\mu) := - \H(\mu) + \sum_{L \in \Ed} \alpha\ssub L\, \H_\mu(\Tgt L | \Src L),
    \CInc_{\dg M}(\mu) := - \H(\mu) + \sum_{a \in \Ar} \alpha_a\, \H_\mu(\Tgt a | \Src a).
$
...
Note that $\CInc$ does not depend on the cpds
...
When the observational and structural information conflict, then the distribution(s)
 which best represents a PDG will depend on the relative importance of observation and structure (see \Cref{sec:example-gamma}).
...
\begin{align*}
    \bbr{\dg M}_\gamma(\mu) &:= \OInc_{\dg M}(\mu) + \gamma \, \CInc_{\dg M}(\mu)
...
Supposing that $\bbeta > \mat 0$,%
    \footnote{or more generally, that $\bbeta \gg \balpha$,
        (i.e., if $\alpha_a>0$ then $\beta_a>0$), 
        which is weaker than $\bbeta > \gamma \balpha$ for every $\gamma > 0$}
...
%oli4: this might be a bit much but ok.
since it coincides with $\gamma$-inference with $\gamma$ equal to an infinitessimal number $\epsilon$.
...
{\color{gray!50!white} It is also sometimes called the relative entropy cone, because if $\mat m, \mat p \in \Delta^{n-1} \subset \mathbb R^n$ are points on a probability simplex, then $(-\mat u, \mat m, \mat p) \in K_{\exp}^n$ if and only if $\mat u$ is an upper bound on $\mat m \log \frac{\mat m}{\mat p}$, the pointwise contribution to relative entropy at each outcome.}
%
%oli4: being a little more explicit.
As one might expect, 
$K_{\exp}^n \subset \mathbb \Rext^{3n}$ denotes the cartesian product
...
Techniques have only recently been developed to solve such programs in polynomial time \parencite{badenbroek2021algorithm,dahl2022primal}.
...
\[
    \mathop{\text{\sf minimize}}\limits_\mu\quad
        % \OInc_{\dg M}(\mu) + \gamma \CInc_{\dg M}(\mu)
        \bbr{\dg M}_\gamma(\mu)
...
\footnote{
    %oli4: missing extra gamma. 
    When $\bbeta < \gamma \balpha$, we still have a way to find \emph{some} local optimizer via the concave-convex procedure, which we implement and discuss at length in \cref{sec:larger-gamma}---although this alone is insufficient for inference, as we have defined it.
    }
...
% This is a start, but what we were really after was the unique distribution $\bbr{\dg M}^*$ that also minimizes $\CInc_{\dg M}$.
...
%  M}^*$, which, uniquely among them, best minimizes $\CInc{\dg M}$.
%oli4: added \epsilon
but to do $\epsilon$-inference, we need to find the distribution $\bbr{\dg M}^*$ that, uniquely among them, best minimizes $\CInc_{\dg M}$.
It turns out that a solution to \eqref{prob:joint-inc} can be used to construct a second optimization problem of a similar size, that also optimizes $\CInc$ subject to these constraints.
...
%joe4
%conditional marginals along the edges, and now optimize $\CInc$.
%We now run into a second issue: $\CInc$ is not convex.
%Fortunately, when we constrain to distributions that minimize $\OInc$, it is.
conditional marginals along the edges, and optimize $\CInc$.
%oli4: I don't think the "in attempting to do so" adds anything.
But in attepting to do so, we run into a second issue: $\CInc$
%oli4:
...
\begin{equation}
    % \CInc_{\dg M}(\mu) =
    \underset{{\dg M}}{\CInc_{}}(\mskip-0.5mu\mu\mskip-1mu) \!=\!
...
the denominator of \eqref{eq:idef-alt-constr} does not depend on $\mu$ and so is a constant vector in our search for $\mu$ that minimize $\CInc_{}$.
...
% $
    ...
    %upper-case letter.
    %hmm. what about lower case bold $k$?  It's a vector after all.  I called it 
    % \psi because it's the product of all of the cpds, i.e., the unnormalized probability
...
\end{align*}
then $\bbr{\dg M}^* = \mu$
and $\mat 1^{\sf T} \mat u = \CInc_{\dg M}(\mu)$.
\end{prop}
...
%Running \eqref{eqn:joint+idef} on a convex solver like that described
%in \cite{??} gives rise to the
Running \eqref{eqn:joint+idef} through a convex solver gives rise to the first known algorithm
...
% However, the same cannot be said of $\CInc_{}$, because it depends on the joint entropy $\H(\Pr_{\bmu})$ of the entire distribution.
It is not so clear that $\CInc_{}$ can be dealt with in the same way, because it depends on the joint entropy $\H(\Pr_{\bmu})$ of the entire distribution.
...
\begin{figure}
    ...
    \caption{
        ...
        The ExpCone methods \texttt{cvx\_opt\_joint} are split into two variants, depending on whether or not it also computed the second step described in \cref{sec:also-idef} to account for $\CInc_{}$.
...
\begin{figure}
    ...
    \caption{An illustration of the trade-off between $\OInc$ and $\CInc_{}$. Darker collors correspond to larger $\gamma$.}\label{fig:inc-idef}
\end{figure}
...
% \begin{lproof}\label{proof:sharp-p-hard}
	...
	\[ \aar{\dg M_\varphi}_\gamma = \inf_\mu \Big[ \OInc_{\dg M_\varphi}(\mu) + \gamma \CInc_{\dg M_\varphi}(\mu) \Big]. \]
	...
	Because $\CInc_{}$ is bounded, it follows immediately that
	...
	\[
		\aar{\dg M_\varphi}_\gamma =
			\inf\nolimits_{\mu \in \Delta \big[\bbr{\varphi} \times \{\mat 1\}\big]} \gamma\; \CInc_{\dg M_\varphi}(\mu) .
	\]
	A priori, by the definition of $\CInc_{\dg M_\varphi}$, we have
	\[
		\CInc_{\dg M_\varphi}(\mu) =
	...
	$$
	Therefore, $\CInc_{\dg M_\varphi}(\mu)$ reduces to the negative entropy of $\mu$.
...
larger, meaning that the structural term $(\CInc_{})$,
becomes more important.
Because $\CInc_{}$ can express features of a distribution like conditional indepenedence,
it cannot be convex.%
