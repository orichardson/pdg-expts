...
to answer such questions with a PDG.
...
%oli5: I still don't see the problem --- this just means the set of variables \X is closed under unions. 
%oli5: in any case, this fix won't quite address the problem, 
{\color{red}
%oli5: maybe this softens the problem
We can regard
sets of variables $\mat X$ are variables themselves,}
with
...
%it.   You first desxcribe what the objects are, then explain them.
    \voli{In aggregate, $\balpha = [\alpha_a]_{a \in \Ar} \in \Rext^\Ar$ and $\bbeta = [\beta_a]_{a \in \Ar} \in \Rext^\Ar$ are the vector forms of the weights, and
...
hyperedge).  
This is captured by the \emph{causal incompatibility}
...
\mathop{\text{\sf minimize}}\limits_{\mu, \mat u, \mat v} & ~~
    ...
    \\[-0.2ex]
    &\qquad
    - \sum_{\mathrlap{\!\!\!(a,s,t) \in \smash{\V\!\Ar^+}}} \beta_a \, \mu(\Src a{=}s,\Tgt a {=} t) \log p\ssub a (t|s)
\\[0.2ex]
...
\mathop{\text{\sf minimize}}
    ...
    - \sum_{\mathrlap{\!\!\!(a,s,t) \in \smash{\V\!\Ar^+}}}
        \beta_a\, \mu_{C\!_a}\!(\Src a{=}s,\Tgt a {=} t) \log p\ssub a (t|s)
    ...
    \item constraints as follows:
        \begin{itemize}[label=\textbullet]
            ...
            \begin{align*}
                ...
                \qquad\text{and}\qquad
                w_{a,s,t} &= \mu_{C_{\!a}}\!(\Src a{=}s)\, p\ssub a(\Tgt a{=}t\mid\Src a{=}s)
                    \qquad\Big(~= p\ssub a(\Tgt a{=}t\mid\Src a{=}s) \sum_{\bar c \in \V(C_a \setminus\{\Src a\})}
...
\begin{lproof}
    ...
    \begin{align*}
        O\left( |\V \!\Ar + \V \mathdcal C|^{1+\omega} 
            \left(\log |\V \!\Ar + \V \mathdcal C| + \log\left(1 + \max_{(a,s,t) \in \V\!\Ar} \beta_a \log \frac{1}{p\ssub a(t|s)} \right) \right)  \right) \\
        \subset 
        O\left( |\V \!\Ar + \V \mathdcal C|^{1+ \omega} 
