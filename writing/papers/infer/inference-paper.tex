...
% A PDG can contain inconsistent beliefs, and there is a natural way measuring its degree of inconsistency.
% \voli{They can also capture inconsistent beliefs, and moreover provide a natural measurement of the degree of this inconsistency. }
% \vjoe{
% They can also capture inconsistent beliefs; moreover, we can measure how inconsistent a probability distribution is with a PDG. This allows us to define the degree of inconsistenty of a PDG to essentially be that of the probability measure that is least inconsistent with it.}
%oli5*: adopting Joe's version, but it requires some tweaks, for two reasons:
...
%% Fortunately, by moving the punctuation in the clever way, we can fix this.
...
%oli5: again, our innovation isn't the algorithm; we're using an off-the-shelf algorithm.
...
%oli5*: I already added this material later in the document, where I think it belongs. In my view this is too much detail and doesn't make much sense here: we haven't explained what a PDG is, for instance, so a lot of this can't be understood properly until reading section 2 or the previous papers.
%oli5: at a higher level, I'm also not happy with the picture laid out in this paragraph 
\vjoe{
%joe5*: The issue isn't correctness but how we define inferfence.
%oli5: I don't see why you're making a distinction. Defining inference means defining what it means to answer probabilistic queries correctly. 
%joe5*: I rewrote this pareagraph completely.  We need to tell the "right"
%  story, and tell it early.  
%oli5*: I really dislike this paragraph; to me it seems obvious, wordy, and several steps  of background removed from anything we actually do. The second paragraph is valuable  real estate and this strikes me as a very poor use for it. Only the last sentence is interesting, and the rest is all well-known background and repetition of the definition of probabilistic inference that we already gave. 
...
%oli5: we didn't define these either.
...
%oli5: I suppose we can make that happen, although it unnecessarily complicates the 
% math in some places and looks like the two are more equal than they actually are. 
% Keep in mind that this second loss function can be negative or positive but is bounded,
% while the first is only positive but can be unbounded.  I think the relationship is 
% clearer when presented like a free energy. 
...
% is a lower bar than "maximally" consistent, which is what IDef represesents. 
consistent with
...
%oli5: no longer helpful the way I wrote it.
...
\discard{\color{red} 
%oli5*: This is on very sketchy ground.
% For example: if \alpha=0, we were imagining that every
% distribution was "qualitatively consistent" with this structure,
% but in fact only the uniform distribution is.  So, in that case, this 
% sentence amounts to saying "if the uniform distribution is consistent with
% observational info, then all inference is done on the uniform distribution". 
...
% Otherwise, there may not be a unique distribution. This story is too nice. 
    that is, 
    there is a
...
that best represent a PDG will depend on the relative importance of observation and structure (see \Cref{sec:example-gamma}).
