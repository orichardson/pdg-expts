...
\begin{align*}
    ...
    - \sum_{\mathrlap{\!\!\!(a,s,t) \in \smash{\V\!\Ar^+}}}
        % \beta_a\, 
        \alpha_a\gamma\,
        \mu_{C\!_a}\!(\Src a{=}s,\Tgt a {=} t) \log \p_a (t|s)
...
It therefore
follows from \cref{prop:cluster+idef,prop:cluster-small-gamma-correct}
that
%oli5:
$\zogamma$-inference
%joe5:
...
\begin{lemma}
    ...
    Then for $t \in [0,1]$, the function
    \[
    % t \mapsto (1-t) \mu_0 + t \mu_1
    t \mapsto \kldiv[\Big]{ (1-t) \mu_0(X,Y) + t \mu_1(X,Y) }
        {((1-t) \mu_0(X) + t \mu_1(X)) p(Y|X)}
...
\begin{lproof}
    ...
    Note also that $|\delta|(x)^2 = (\sum_{y} |\delta(x,y)|)^2 \ge (\sum_{y} \delta(x,y))^2$, and strictly so if there are any $y$ such that $\delta(x,y) < 0$. Meanwhile, $|\delta|(x,y)^2 = \delta(x,y)^2$ is unchanged.
    ...
    The second condition, however, is somewhat uninformative; we've lost information at this point.  
    We now return to the expression we had in \eqref{line:beforabs} before considering $|\delta|$. 
    We've already shown that the contribution to the sum at each value of $x$ is non-negative, so if $\bdelta^{\sf T} \mat H(\mu) \bdelta$ is to equal zero, each summand which depends on $x$ must be zero as well.  Now if $x$ is a value for which $\delta(x) = 0$, then
    \begin{align*}
    ...
    \[
        \bdelta^{\sf T} \mat H(\mu) \bdelta = 0
            \quad\iff\quad \delta \ge 0 \quad\text{and}\quad \forall (x,y) \in \V(X,Y).~~
    ...
    At $\mu_t = (1-t)\, \mu_0 + t\, \mu_1$, the nullspace of the Hessian $\mat H(\mu_t)$
    consists of
    \begin{align*}
        &\Big\{ \bdelta \in \mathbb R^{\V(\!X\!,Y\!)} : 
            \bdelta^{\sf T} \mat H(\mu_t) \bdelta = 0 \Big\} \\
        &= \Big\{ \bdelta \in \mathbb R^{\V(\!X\!,Y\!)} : 
            \delta(X,Y) \mu_t(X) = \delta(X) \mu_t(X,Y) \Big\} \\
        &= \Big\{ \bdelta \in \mathbb R^{\V(\!X\!,Y\!)} : 
            (1-t) \delta(X,Y) \mu_0(X) + t \delta(X,Y) \mu_1(X) =
            (1-t) \delta(X) \mu_0(X,Y) + t \delta(X) \mu_1(X,Y) \Big\} \\
        &=
    \end{align*} 
\end{lproof}
...
\begin{lproof}\label{proof:marginonly}
    ...
    % \def\pt 
    there is some $(\hat a, \hat s, \hat t) \in \V\!\Ar$ such that 
    \[
...
\begin{lproof}\label{proof:can-use-cliquetree}
    ...
    Let $G := (\X, E := \{ (X{-}Y) :  \exists C \in \C.~\{X,Y\} \subseteq C\})$ be the undirected graph whose vertex set is $\X$, and that has edges connecting every pair of variables that share a cluster. We will regard $G$ as a Markov Network.
    Note that $G$ is chordal: 
    % if $(X, Y) \in E$ and $(Y, Z) \in E$, then there exist $C, C' \in \C$ with $\{X, Y\} \subseteq C$ and $\{Y, Z\} \subseteq C'$, meaning 
    if $G$ contains a loop $X{-}Y{-}Z{-}W{-}X$
    
    
    Suppose that $\mat X, \mat Y, \mat Z \subseteq \X$ are sets of variables
    %TODO: add explicit separation undirected graphs
    such that $X$ and $Y$ are separated by $\mat Z$ according to $G$---that is,
    ...
    % This is because 
    Suppose, for contradiction, that this were not true. Then for each edge $e = (C{-}D) \in \cal T$, there would be at least one variable $X_e \in C \cap D \setminus \mat Z$. 
    % So, if $C_{X_0}$ is a cluster containing $X_0$ and $C_{Y_0}$ is a cluster containing $Y_0$, 
    Every variable is a member of at least one cluster; say $X_0 \in C_{X_0}$ and
    $Y_0 \in C_{Y_0}$.  As $\cal T$ is a tree, there is a (unique) path (in $\cal T$) from $C_{X_0}$ to $C_{Y_0}$; let's label it as follows:
    \[
    ...
    Next, since $(\C, \cal T)$ is a tree decomposition of $\Ar$, each hyperarc $a \in \Ar$ can be assigned to some cluster $C_a$ that contains all of its variables, which allows us to lift the cluster partition $\C = \C_1 \sqcup \C_2$ to a partition $\Ar = \Ar_1 \sqcup \Ar_2$ of edges, and consequently, a partition of PDGs $\dg M = \dg M_1 \bundle \dg M_2$.
    Concretely: let $\dg M_1$ be the sub-PDG of $\dg M$ induced by variables $\X_1 \subseteq \X$ of variables and arcs $\Ar_1 = \{ a \in \Ar : C_a \in \C_1\}\subseteq \Ar$; define $\dg M_2$ symmetrically. (To be explicit: the other data of $\dg M_1$ and $\dg M_2$ are given by restricting each of $\{\mathbb P,\balpha,\bbeta\}$ to $\Ar_1$ and $\Ar_2$, respectively.)
    % Thus, $\dg M = \dg M_1 \bundle \dg M_2$. 

    Finally, suppose for some $\gamma > 0$ that $\mu \in \bbr{\dg M}^*_\gamma = \bbr{\dg M_1 \bundle \dg M_2}^*_\gamma$.
    We can then apply \cref{prop:markov-property}, to find that
    $\X_1$ and $\X_2$ are independent given $\X_1 \cap \X_2$
     % = C_1 \cap C_2$. 
...
\begin{lproof}\label{proof:cluster+idef-correct}
    ...
    \begin{align*}
        ...
        &= -\H(\Pr\nolimits_{\bmu}) - \sum_{a \in \Ar} \alpha_a \sum_{c \in \V C_a}
    ...
    Since we know that there is a unique distribution that minimizes $\SDef_{\dg M}$ among the elements $\bbr{\dg M}_0^*$, and also that this distribution can be represented by a clique tree (by \Cref{coro:can-use-cliquetree}), we conclude that $\bmu$ must represent this distribution. Thus, $\Pr_{\bmu} = \bbr{\dg M}^*$ as desired. 
\end{lproof}
