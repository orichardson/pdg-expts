...
(2) a construction that allows us to express these problems compactly
for PDGs of boundeed treewidth,
%joe7: in what way did we develop the theory of PDGs
%for which we needed to further develop the theory of PDGs, and
and
%joe7
%(3) an appeal to interior point methods that can solve such problems
(3) an appeal to interior-point methods that can solve such problems
in polynomial time. 
%joe7*: Saying "cursory analysis" is terrible!  You don't want to put down
%your own paper in the abstract!  I don't see what this adds in any
%case, once you've said that we verify correctness and complexity.
%What other kind of analysis do we provide?
%We verify the correctness and complexity of our approach, provide an
%implementation, and a cursory analysis.
We verify the correctness and complexity of our approach, and provide an
implementation.
\end{abstract}
...
%oli5: added
%joe6
%joe7: reihstated.  I think it is grammatcal, and sounds better
%Unlike in a BN, the two can conflict in a PDG.
Unlike a BN, the two can conflict in a PDG.
%oli6: I think the change is not grammatical; I'm reverting it (to the
%above). 
%Unlike a BN, the two can conflict in a PDG. 
...
a PDG is also of interest for of its degree of inconsistency---% 
...
%oli6: merging and compressing; there was also a missing "as" that
%I've added to the next line.  
...
%oli6: you really think we should include the laundry list of
%algorithms in the comment above?  I'm happy to do this if the paper
%gets accepted; for now, we're very pressed for space.
%joe7: Yes, I do
%joe7
...
viewed as running on a tree
\parencite[\S9-11]{koller2009probabilistic}. 
...
%joe7: there's no notion of choice!
...
%
% Most computer scientists are familiar with linear programming (LP),
% where the constraints are also linear, and likely also the variants
% in which the contstraints can quadratic (QP) or that a matrix be
...
Interior-point methods provide an iterative way of approximately
%joe7
solving linear programs \parencite{karmarkar1984new} in polynomial time. 
olving linear programs in polynomial time \parencite{karmarkar1984new}. 
%joe4
...
The algorithm that seems to display the best empirical performance
\parencite{dahl2022primal}, however, was only recently shown to
%joe7: I still don't understand what "converge" means in this context
converge in polynomial time \parencite{badenbroek2021algorithm}. 
...
the more complicated ones, we begin simply by finding minimizers of
...
Observe that
\begin{align*}
%joe2: you still haven't defined \otimes.  I'd be happy if you could  get rid of it altogether.
    &&(- \mat U,~
        % \mat m \otimes \mat 1
        [\mat m,\,...\,, \mat m]
        % \mat M
...
%joe7
%some $\p_a(t\mid s) = 0$, in which case the linear term explodes.
some $\p_a(t\mid s) = 0$, in which case the linear term is undefined.
%joe4
...
%oli6: kind of agree altogether, but I want to emphasize somehow that
%this is the first thing we know works (I've been trying to find
%anything that works for some time). It's also not efficient until
%next section, so your edit doesn't make sense. For now, just making
%it "first algorithm" which has less posturing. 
% first known algorithm
first algorithm
...
%reader, who can't read your mind
    ...
    O\pqty[\Big]{  (N\!+\!A)^4 V^{4T} \pqty[\Big]{ T \log V + \log \frac{N\!+\! A}{\epsilon} 
    ...
    \unskip\daggerfootnote{ at the cost of substantial overhead and
      engineering effort, the exponent $4$ can be reduced the exponent
      to 2.872, by appeal to \textcite{nesterov1996infeasible} and the
...
%myhsterious. 
      current best matrix multiplication algorithm to invert linear
      systems. } 
    where
    % $
    % time, where     
...
%joe7
%But suppose we were interested in calculating only this
But suppose that we were interested in calculating only this
inconsistency. 
...
our approach compares favorably (both in terms of time and in terms of
accuracy) to black-box optimization baselines such as using LBFGS or
Adam optimizers on \cref{eqn:scoring-fn} directly. 
...
%joe7
%Assymptotically, there may be much for improvement. Our implementation 
...
$\tilde O(n^{2.9})$.  But BNs, Factor Graphs, and nearly all graph
problems can be solved in $O(n^1)$ time, if we assume bounded
tree-width. Can we do better? 

%joe7*: don't undercut your result!
%More importantly, we have only shown how to do inference in a small
subset of cases.
%joe7
%Namely, we have only cleanly demonstrated how to do
%$\zogamma$-inference in cases where $\zogamma$-semantics is a
%singleton and moreover $\bbr{\dg M}_{\gamma}$ is convex, and
%moreover, that convexity follows from a point-wise ``observational
Specifically, we have shown how to do
$\zogamma$-inference when $\zogamma$-semantics is a
singleton and $\bbr{\dg M}_{\gamma}$ is convex;
moreover, we have that convexity follows from point-wise ``observational
...
%joe7: we don't know that it's much more difficult
...
In some cases, this might actually allow us to do inference---say, if
%joe7: I don't know what "external reasons" are
%we happen to know for external reasons that $\bbr{\dg M}^*_\gamma$ is
we happen to know that $\bbr{\dg M}^*_\gamma$ is
%joe7
%pseudo-convex (although we loose polynomial time guarantees and have 
%no ability to automatically recognize such situations).
pseudo-convex (although we currently cannot
recognize such situations automatically).
In any case,
we have implemented this, and describe it in \cref{sec:larger-gamma}. 
...
%joe7*: I can't parse this.  Did you mean to remove the "it" on the
%next line?  Why is the equation in parens?  what is the "formally"
%doing there?
...
%joe7    
%    Now, consider another discrete measure $|\delta|$, whose value at
    Now consider another discrete measure $|\delta|$, whose value at
    each component is the absolute value of the value of $\delta$ at
    that component, i.e., $|\delta|(x,y) := |\delta(x,y)|$. 
...
%joe7: What does it mean that it's a fluke?  Why even bother saying this?
    ...
    We've already shown that the contribution to the sum at each value
    of $x$ is non-negative, so if $\bdelta^{\sf T} \mat H(\mu)
%joe7
%    \bdelta$ is to equal zero, each summand which depends on $x$ must
    \bdelta = 0$, then each summand which depends on $x$ must
    be zero as well.   
...
%    which is only possible if $\delta(x,y) = 0$ for all $y$. 
    which is possible only if $\delta(x,y) = 0$ for all $y$. 
    ...
    \begin{align*}
        \bdelta^{\sf T} \mat H(\mu) \bdelta = 0
        \quad&\iff\quad 
            (\forall x .~  \bdelta_x \ge \mat 0 \text{ or } \bdelta_x
        \le \mat 0)
            \qquad\text{and}\qquad 
            \forall (x,y) \in \V(X,Y).~~
%joe7
...
%joe7: Why is there a line here?  It's highly nonstandard.
    \medskip
    \hrule
    \medskip
    
    Finally, we are in a position to prove the lemma.  
%joe7*: please write \Delta (X), not \Delta X.  Make life easier for
%the reader!
...
\begin{lproof}\label{proof:marginonly}
    ...
    Therefore, it must be the case that all distributions in $\bbr{\dg
%joe7
      %      M}_0^*$ have the same conditional marginals, as promised.
            M}_0^*$ have the same conditional marginals, as claimed. 
\end{lproof}
...
%joe7
    %    We can then apply \cref{prop:markov-property}, to find that
        We can apply \cref{prop:markov-property} to find that
    $\XX$ and $\XNX$ are independent given $\XX \cap \XNX$.
%joe7
%    Then, we use standard standard properties of random variable
We then use standard properties of random-variable
        independence 
...
%joe7
%        ~=~ - \H(\Pr\nolimits_{\bmu})\quad\text{by
        %          \cref{eq:cluster-ent-decomp}}.  \]
        ~=~ - \H(\Pr\nolimits_{\bmu})\]
by \cref{eq:cluster-ent-decomp}}. 

%joe7
%Now, the objective of our problem \eqref{prob:cluster-small-gamma} is
The objective of our problem \eqref{prob:cluster-small-gamma} is
essentially the same as that of \eqref{prob:joint-small-gamma}, so the
analysis in \hyperref[proof:joint-small-gamma-correct]{the proof of}
\cref{prop:joint-small-gamma-correct} applies with only a handful of
superficial modifications.  
%joe7: in what sense is it a shortcut.  This should be rewritten, but
%I'm not quite sure how.  Perhaps "By an argument similar to that of
%ther proof of ...", 
Using that proof to take a shortcut, the objective of
    \eqref{prob:cluster-small-gamma} must equal 
    \begin{align*}
    % \textit{(\ref{prob:joint-small-gamma}.obj)} =
...
\begin{lemma} \label{lem:mainlemma}
 ...
 strictly feasible (i.e., if there exists $\mat x \in \mathrm{int}\,
%joe7
 % K$  such that $A \mat x = \mat b$ )  is its dual problem
  K$  such that $A \mat x = \mat b$)  is its dual problem
...
%joe7: I still don't know what this means
    ...
    %joe7*: what is MOSEK?  You can't introduce it out of the blue here!
    %
...
\begin{lproof}
    ...
    Generally speaking, 
%joe7
    %    the idea behind primal-dual interior point methods
        the idea behind primal-dual interior-point methods
    \parencite{nesterov1994book} such as the one behind MOSEK, is  
    ...
    After adding the correction step $\Delta z_+^{\text{cor}}$ to $z$,
%joe7
    %    we repeat the entire process. The full algorithm, then, is
        we repeat the entire process. The full algorithm can be
...
%joe7
%    Their main result \parencite[Theorem 3]{badenbroek2021algorithm},
    \parencite[Theorem 3]{badenbroek2021algorithm}
    states that for every $\epsilon \in (0,1)$, 
    ...
    \medskip
%joe7: Again, having a line here is quite nonstandard
    \hrule
...
%joe7
%    Then Theroem 1, part 1 of \citeauthor*{nesterov1996infeasible},
    Then part 1 of Theorem 1 of \citeauthor*{nesterov1996infeasible},
...
%joe7: again, I don't know what "to machine precision" means
...
%joe7: you need to define "1-strongly convex"
    Now, (conditional) relative entropy is 1-strongly convex, and each
    relative entropy term is scaled by $\beta_a$. 
    Furthermore, we're only considering marginal conditional entropy, so this convexity may not hold in all directions.
    Still, if the next step direction $\delta$ is not far from the
%joe7
%    gradient, as is the case if the interior point method has nearly
    gradient, as is the case if the interior-point method has nearly
    converged, then in that direction, the objective will be at least
    ($\min_a \{ \beta_a : \beta_a > 0\}$)-strongly convex.  
...
%joe7: and again
  Problem \eqref{prob:cluster-small-gamma} is solved to machine
    precision in time 
    \[
...
%joe7
%    The same arguments made in \cref{lem:cluster-inc-polytime} show
    The same arguments as those made in \cref{lem:cluster-inc-polytime} show
    that both $n,m \in O(|\V \!\Ar + \V \C|)$. 
...
\begin{lproof} \label{proof:consistent-NP-hard}
	...
	First, note that the number of nodes, edges, and non-zero
%joe7
%        entries in the cpds are polynomial in the $|\mathcal J|, |\mat
        entries in the cpds is polynomial in $|\mathcal J|$ and $|\mat
        X|$, and the total number of parameters in a simple matrix
        representation of the cpds is also polynomial if $\mathcal I$
        is bounded (e.g., if $\varphi$ is a 3-CNF formula). 
	A satisfying assignment $\mat x \models \varphi$ of the
        variables $\mat X$ can be regarded as a degenerate joint
        distribution $\delta_{\mat X = \mat x}$ on $\mat X$, and
        extends uniquely to a full joint distribution $\mu_{\mat x}
        \in \Delta \V(\dg M_\varphi)$ consistent with all of the
        edges, by
%joe7: you haven't defined \otimes        
...
%joe7
%    \textbf{PART (b).}
%    We prove this by reduction to \#SAT. Again, let $\varphi$ be some
    We prove part (b) by reduction to \#SAT. Again, let $\varphi$ be some
    CNF formula over $\mat X$, and construct 
...
%joe7
        %	We start with a claim about first term.
        	We start with a claim about the first term.
...
%joe7*: I would cut this.  You state a bunch of results and don't
%prove them, and it does not advance the main story of the paper at all.
...
%joe7*: We've defined inference.  Why are you repeating the fact that
%it's subtle and then undercutting what we said in the main text.
%This is a *terrible* idea.  The story here *must* be changed.
...
\begin{example}
    ...
    % p(X)\delta_{y}(Y)$.
    p(Y) \delta_x(X)$.
%joe7*; NO!!!  It's not a "better" answer, but the answer to a
%different question.  We need to give much more intuition and
%discussion of this point.
    And perhaps that is even a better answer than conditioning, if
    indeed we are far more confident in $p$ than in $q$. 
\end{example}
...
We now 
%Consider, for example, consider a PDG $\dg M$ containing three hyperarcs\end{figure}

%joe7
%We now 
%Consider, for example, consider a PDG $\dg M$ containing three hyperarcs
Consider, for example, a PDG $\dg M$ containing three hyperarcs
...
%joe7: we don't have observational facets any more.  HYou're going to
%confuse the reader.
...
%joe7*: I'm not even sure what this is a proof of.
