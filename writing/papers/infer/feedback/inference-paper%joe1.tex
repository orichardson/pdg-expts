...
%  \aistatstitle{Inference in Probabilistic Dependency Graphs,\\
  %    via Exponential Cones and Otherwise}
    \aistatstitle{Inference in Probabilistic Dependency Graps}
...
p%other probabilistic graphical models.
...
%joe1
%This paper may be viewed as providing partial answers---both
...
\empb{Probabilistic dependency graphs}, or \emph{pdgs} \parencite{pdg-aaai},
are a particularly flexible class of probabilistic graphical
%joe1
%models, which subsumes Bayesian Networks (BNs) and Markov Random Fields 
models that which subsume Bayesian networks (BNs) and Markov random fields 
(MRFs).
%joe1: much too wordy
...
% It seems that in a  minimizing inconsistency is a generically useful
 % modeling task.
 %joe1:
Unfortunately, the earlier work on pdgs does not provide any
...
This allows us to use a powerful recent interior-point method
...
%joe1
%We write $\Delta S$ to denote the set of proability distributions over
We write $\Delta S$ to denote the set of probability distributions over
a finite set $S$. 
...
we write $\mu(X)$ for its marginal distribution on $X$, and $\mu(Y|X)$
for the cpd obtained by first conditioning on $X$ and then
marginalizing to $Y$.  
...
\begin{defn}
    ...
    \begin{itemize}[nosep]
        ...
        \qedhere
    %     % \item $\mathcal P = \{ p\ssub L (\mat T_L | \mat S_L)
    %     \}_{L \in \Ed}$ is an indexed set of cpds    
    %     \item $\bbeta$ 
    \end{itemize}
\end{defn}
...
The incompatibility of a joint distribution $\mu(\N)$ over all
variables, with such a PDG is given by a weighted sum of relative
entropies: 
\begin{align*}
    \Inc_{\dg M}(\mu) :=
...
One should be careful to distinguish this joint distribution $\bbr{\dg
  M}^* \in \Delta\V \N$, which arises in the limit as $\gamma \to 0$,
from $\bbr{\dg M}^*_0$, the set of distributions that minimize
%joe1
%$\Inc_{\dg M}$ which contains $\bbr{\dg M}^*$, and possibly many others.
$\Inc_{\dg M}$; the latter set can be shown to contain $\bbr{\dg
  M}^*$ (see \parencite{pdg-aaai}), but may also contain other distributions.
...
\begin{enumerate}[nosep, label=\textbf{D\arabic*.}]
    ...
    \item Generally speaking, optimization is computationally difficult.
      Even our most powerful optimization techniques
%joe1
%      only provably find optima in certain special cases. 
      only find optima in certain special cases. 
...
%      (more-or-less; see \cref{sec:expts}), the standard theoretical
      ...
      tools do not apply in our setting.  
        ...
        in general $\bbr{\dg M}_\gamma$ is neither Lipshitz nor
...
%joe1: I have no idea what you mean by "throwing all the relevant
%pieces in a PDG"
in which we throw all of the relevant pieces in a PDG, and try to
minimize inconsistency. 
Well, we have a PDG $\dg M$, and we also have a candidate joint
distribution $\mu$.
%joe1: YOu have to give some intujition about how you
What if we put them both on the same footing, in a new PDG, and
measure its inconsistency? 
%joe1
...
\begin{linked}{prop}{optimalYgivenX}
	...
	$$\displaystyle
		...
		\argmin_{p(Y|X)}\,
%joe1: You haven't defined +.  Are you expecting the reader to know
%what it is?
                \aar{\dg M + p}_\gamma =
...
%into the PDG"     
which includes a conditional probability on $Y$ given $X$ into the PDG
$\dg M$.  
\end{linked}
...
Consequently, if we are only interested in querying the marginal
...
One might worry that this reformulation could increase the difficulty
%joe1: I have no idea what nice properties are you talking about?  
of optimization (\textbf{D2})---that we might lose the several nice
properties we do have---but fortunately, this concern is not
substantiated.  
...
%don't know what the "quantitative limit" is.
...
Such unconditional probabilities may be identified with unit vectors
$\mat p_j \in \mathbb R^n$, and all $k$ of them may conjoined to form
%joe1: what isn't this just a matrix?  What makes is "stochastic"?
a stochastic matrix $\mat P = [\,p_{ij}] \in [0,1]^{n \times k}$. 
...
and observe that:
\begin{align*}
%joe1*: I'm lost.  I don't know what \otimes means in this context,
%and I have no idea what hyou're trying to do.  You have to SLOW DOWN
%and add more intjition.
...
For each $L \in \Ed$, let $N_L := |\V(\Src L, \Tgt L)|$ be the
dimension of joint settings of the source and target values of $L$,
%joe1
%i.e., the dimension of the cpd $p \ssub L$,
that is, the dimension of the cpd $p \ssub L$, 
%joe1: before you used K for the exponential cone; you should probably
...
%joe1*: I'm totally lost here.  I know that S_L and T_L have something
%to do with what you called the source and target (although hyou never
%defined the terms), but I don't know exactly wehat they are.  You're
%wrote \mj(S_L) which was last defined as the marginal of \mu on S_L,
%but that doesn't  look like what you mean here.  And since I have no
%idea what exponential cones are or how you're usiong them, I don't
%know what it means for a tuple like this to be in K.  SLOW DOWN.
%GIVE MORE INTUTION.  Help the poor struggling reader.
    \forall L \in \Ed.~&(-\mat u^L, \mu(\Src L, \Tgt L), \mu(\Src
        L) p\ssub L(\Tgt L | \Src L)) \in K_{\exp}. 
        % (-t^L, \Pi_{(\Src L \Tgt L)}\mu, \Pi_{(\Src L)} (\mu) p\ssub L(\Tgt L | \Src L)) 
        % \big(\!\shortminus\! \mat u\ssub L,~ \Pi_{\Src L\Tgt L}\!(\mat m),~
            % \mat P\!\ssub L (\Pi_{\Src L}\!(\mat m) \otimes \mat 1) \big) 
        % (-t^L_{xy}, \mu(x,y), )
...
%jhoe1: YOu really need a lot more intjition.
...
%joe1*: Again, I have no idea what's going on here.
...
\begin{linked}[Markov Property for PDGs]{prop}{markov-property}
	...
	% $\mat X_2 := \N^{\dg M_2}$.
%joe1: I couldn't quite parse the next sentence.  I wrote what I
%thought you intended
%  Suppose $\dg M_1$ and $\dg M_2$, over respective sets of
  %        variables $\mat X_1$ and $\mat X_2$.
    Suppose that $\dg M_1$ and $\dg M_2$, are PDGS over the sets of
%        variables $\mat X_1$ and $\mat X_2$, respectively.
	 % and let $\mathbf X$ denote the variables they have in common.
	Then for all $\gamma > 0$, we have that
	\[
%joe1: You have defined neither + nor \mnodels
	\bbr{\dg M_1 \bundle \dg M_2}^*_\gamma
			% \subset
			~\models~
		% \mathrm I( \N_1 ; \N_2 \mid \mathbf X)
...
The major consequence of \cref{prop:markov-property} is that we
%joe1: I have no idea what calibrated clique trees are, and I very
%much doubt that most of the AIStat community will have a better idea
%only have to consider distributions $\mu$ that come from calibrated clique
have to consider only distributions $\mu$ that come from calibrated clique
trees.  
...
%joe1
%After all, as we saw in \cref{sec:inf-via-inc}, it seems an oracle
As we saw in \cref{sec:inf-via-inc}, it seems an oracle
that tells you your degree of inconsistency can be combined with a
convex optimization algorithm to determine marginals in a way that
does not scale with the rest of the graph.  

%joe1*: remove the conjectures; this is the wrong place for them.  You
%canperhaps mention them in the conclusion, if you haven't solved them.
\begin{conj}
  %joe1*: Is this what you mean byh "inference in a PDG"?  Domputing
  %conditional marginals of [[\dg M]]^*?  If so, you should have said
  %that a long time ago, and provided much more INTUITION about why
  %this should e what we care about.  This is the first that I've
  %heard that this is the key problem you're trying to solve
    Inference in a PDG---that is, computing conditional marginals of
    $\bbr{\dg M}^*$---% 
...
%joe1: how did morality get into this?  It either does apply, or
%doesn't.  Hyou should be clear what you mean.
Morally, everything we've said here already applies to other graphical
models. 
...
%joe1: I haveno idea what the "residual norm" is.  I doubt most AIStat
%readeres will either.
and finds a point $\epsilon$-close in residual norm to the optimal
distribution 
...
%joe1: Again, what is the PDG objective?  I've never seen that
%terminology before this paper.
...
%joe1: one might indeed wonder.  Unfortuantely, one will be kept
%wondering after one has read this section ...  We need mjuch more
%here, including a discussion of why you chose what you chose to do
%the comparison
Since PDGs generalize other graphical models, one might wonder how our
method stacks up against them.  
...
%joe1
%We start by focusing on emperical properties of the optimization over
We start by focusing on empirical properties of the optimization over
joint distributions. 
