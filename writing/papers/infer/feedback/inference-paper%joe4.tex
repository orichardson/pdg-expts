...
%joe4: I'm missing the next two files.  Can you send them to me?
\input{inference-preamble.tex}
\input{pdg-preamble-v2.tex}

\newcommand\discard[1]{}

\newcommand\obslimit{observational surrogate} %(distribution)
\newcommand\ObsLimit{Observational Surrogate} %(distribution)
...
\begin{abstract}
  %joe4: you need to tell the rader what PDGs are; I pulled the first
  %sentence from the intro
  Probabilistic Dependency Graphs (PDGs) \parencite{pdg-aaai}
...
They can also
capture inconsistent beliefs, and have an associated measure
%joe4
%of ``inconsistency'' that quantifies how far the probabilities in
%a PDG are from being self-consistent.
...
% How much computation is necessary to synthesize our beliefs into a
% single joint probability distribution?
...
which subsume Bayesian Networks (BNs)
...
of `inconsistency that quantifies how far a probabiolity measure is
from satisfying the constraints of a PDG.
% Beyond its role in undergirding the semantics of pdgs,
% Beyond its role in providing the semantics of pdgs,
% Beyond its central role in the development of PDG semantics, this
% inconsistency measurement also captures many standard loss functions
% and statistical divergences \parencite{one-true-loss}.
%joe4:this isclunky
...
Nevertheless, under very mild assumptions a PDG does single out a
...
%PDG M)".
%particular distribution which we call its ``\obslimit''.
particular distribution that we call its ``\obslimit''.
Our primary goal is to answer probabilistic queries with respect to
this distribution.
%joe4*: isn't it actually a limit, rather than an "endpoint".  I don't
%know what "endpoint" means.  After rewriting this sentence, I decided
...
%story?  To mu mind, it just clutters it up.    I left in my rewrite
%so you can see it.
\commentout{
%Still, the \obslimit\ is just a special endpoint of a continuous
%family of  distributions that the PDG represents.
...
Under stricter assumptions,
 ...
 the same techniques enable inference with respect to some of these
 other distributions as well.}
%joe4: added.  We need to argue that this distribution is reasonable.
For the PDG that corresponds to a BN or FG, the \obslimit\ is just the
distribution represented by the BN (resp., FG).
...
%we're doing inference with respect to the same distributions, which I
%nhow did.
...
Section~\ref{??} for formal definitions).
...
largely turned out to be distinct perspectives
...
% our approach is quite distinct in nature.
...
%joe4
%Our ability to do PDG inference in polynomial time is based on a line
The algorithm is based on a line
of recent work in convex programming that establishes polynomial-time
convergence
...
%joe4
%can be efficiently converted to a (series of) exponential conic
...
The direct appeal to a commercial solver
%joe4: what does "efficiency out of the box" mean?  That we can use
%off-the-shelf techniques?  If so, it's redundant.
%gives us efficiency out of the box, and also allows us to benefit from future
allows us to easily take advantage of future
improvements in
...
%joe4
%Beyond their role as a probabilsitic model, PDGs
%    are also separately of interest because of their inconsistency measure.
...
%    largely be conceptualized as inconsistency minimization.
    largely be conceptualized as  minimizing the inconsistency of a PDG.
...
interface between such solvers and the standard PDG Python library.
Finally, we evaluate this implementation empirically, and show that it
is both more accurate and faster than other optimization baselines.
%
% Moreover, the approach makes use of , giving us efficiency out-of-the box.

%joe1: It is *very* strange to combine both preliminaries and related
%work in one section.  This strikes me as a bad idea.
...
can be added (+), partially ordered ($\le$), or multiplied ($\odot$)
pointwise as usual.   $\mat 1$ denotes an all-ones vector, of a shape
implied by context.
%joe4: cut this; why not use the standard . for inner product.  Why
%use nonstandard notation.  (I understand it's not completelhy
%nonstandard, but I'm sure that the overwhelming majority of
%books/papers denote inner product using a dot.  Why
%different?
%For the inner product between $\mat u$ and $\mat v$ of the same shape,
%we write $\mat u^{\sf T} \mat v$.

\commentout{
We will sometimes use superscripts as well, especially when
    indices depend on one another. For example, if $\dg S$ is a finite
    set of finite sets, then
...
%joe4
%
%Given a joint distribution $\mu$ over many variables including both
Given a joint distribution $\mu$ over (the values of) a set of
variables including both $X$ and $Y$,
...
%joe4*: But who write \mu(X,Y) to denot a distribution on X and Y?
%That seems equally nonstandard (and is *horrible* notation), because
%it makes it hard to distinguish that probabiity of X=x from a
%probability on X.
...
% There is also a natural bijection between directed hypergraphs and
...
In fact, for graphs with tree-width, many problems can be solved in
linear time \parencite{courcelle1990}, including inference in standard
graphical models.
...
$\boldsymbol\phi = \{ \phi_E : \V E \to \mathbb R_{\ge 0} \}_{E \in \Ed}$
...
\begin{defn}
%joe4; why not {\bf p} rather than \mathcal P; this would be more
%consistent notation.
  %  A PDG $\dg M = (\X, \Ar, \mathcal P, \balpha, \bbeta )
    A PDG $\dg M = (\X, \Ar, {\math p}, \balpha, \bbeta )
...
%joe4
%    is a directed hypergraph  $(\X, \Ar)$, whose nodes correspond to
%    variables, and each
    is a directed hypergraph  $(\X, \Ar)$, whose nodes correspond to
    variables, ${\bf p} = [p]_{a \in \Ar}$,
$\balpha = [\alpha_a]_{a \in \Ar} \in \Rext^\Ar$,
    $\bbeta = [\beta_a]_{a \in \Ar} \in \Rext^\Ar$, and  each hyperarc
...
%joe4: typo, I assume
%          the modeler's confidence in the cpd $p\ssub L(\Tgt a |
          the modeler's confidence in the cpd $p\ssub a(\Tgt a |
          \Src a)$, and
    ...
    %     \item $\bbeta$
    \end{itemize}
%joe4: now unnecsesary
    %    In aggregate, $\balpha = [\alpha_a]_{a \in \Ar} \in \Rext^\Ar$ and
%    $\bbeta = [\beta_a]_{a \in \Ar} \in \Rext^\Ar$ are the vector
...
can be viewed as measuring the ``distance''  between $\mu$ and the
cpds of $\dg M$,
...
%joe4
%Under a standard interpretation of relative entropy $\kldiv{\mu}{p} :=
Under a standard interpretation of the relative entropy $\kldiv{\mu}{p} :=
\Ex_{\mu}[\log \frac\mu p]$,
...
% There is a second kind of discrepency between $\mu$ and $\dg M$, of
% a more structural flavor:
The second scoring function measures a more structural kind of
%joe4
%discrepency between $\mu$ and $\dg M$:
discrepancy between $\mu$ and $\dg M$:
...
%the extent to which $\mu$ fails to arise in
%a situation in which each unit of $\alpha_a$ corresponds to an
the extent to which $\mu$ enforces the fact a hyperarc from $\Src$ to
$\tgt$ respreseents an
independent causal mechanism that determines $\Tgt a$ based on $\Src
%pa$.
...
%
%joe4
%The notation $\bbr{\dg M}^*_\gamma := \argmin_\mu \bbr{\dg
$\bbr{\dg M}^*_\gamma := \argmin_\mu \bbr{\dg
  M}_\gamma(\mu)$ denotes the set of optimal distributions at a
particular value $\gamma$.
% Of particular interest is the ``quantitative limit'' as $\gamma \to 0$,
% The inference task for PDGs, then is

%joe4*: Thix paragraph needs to be rewritten to tell a crisp story.
...
{\color{purple!80!black}
    One natural conception of inference in PDGs is parameterized by $\gamma$:
    $\gamma$-inference for $\dg M$ means responding to probabilistic
    queries in a way that is sound with respect to every $\mu \in
    \bbr{\dg M}^*_\gamma$.
    This paper demonstrates the tractability of
    $\gamma$-inference for the specific case of PDGs satisfying
    $\bbeta \ge \gamma \balpha$, which is sufficient to ensure strict
    convexity of \eqref{eqn:scoring-fn}, and hence a unique optimal
    distribution.}
...
%
% The case of $\gamma \to 0$
% On the other hand, the case of $\gamma = \epsilon$,
% The limit as $\gamma \to 0$, however, which intuitively means
% The latter
% The limit as $\gamma \to 0$,
Interestingly, the limiting behavior as $\gamma \to 0$ is mathematically distinct,
giving rise to another conception of PDG inference which is not parameterized.
...
% (i.e.,$\forall a \in \Ar.~ \alpha_a  0  \Rightarrow \beta_a \ge 0$).
...
%     \footnote{$\bbeta \gg \balpha$ iff $\forall a \in \Ar.~ \alpha_a \ge 0  \Rightarrow \beta_a \ge 0$})
there is still a unique optimal distribution $\bbr{\dg M}^*$, which we call the \emph{\obslimit\
% (distribution)
of $\dg M$}.
Intuitively, it corresponds to an extreme emperical
...
% when we omit the subscript,
% When $\gamma$ is not given
...
%joe4
%\TODO[TODO: Fix up this historical perspective. Chris might be in a
%  better position to write.]
...
%joe4
%In the 1990s, these methods were extende to handle second-order cone
%programs (SOCPs) and semidefinite programs (SDPs) in which one can add
%more expressive constraints,
In the 1990s, these methods were extended to handle second-order cone
programs (SOCPs)  and semidefinite programs (SDPs),
which allow more expressie constraints,
...
%But some---those involving probabilities and divergences---have
%historically resisted expression in these terms.
%For such problems,
...
our purposes.  We need what have been called \emph{exponential cone
constraints}.
...
% Recent work in interior point methods has provided a way to solve
% linear optimization problems with such constraints in polynomial
% time \parencite{dahl2022primal}.
...
%have polynomial time algorithms to solve such problems, which is what
%I now say.
...
Recently, techniques have been developed to solve such programs in
polynomial time \parencite{badenbroek2021algorithm,dahl2022primal}.
...
%joe4
    %    a program that satsfies these conventions is said to be dcp.
        ...
        \emph{discipline convext perofgram (dcp)}.
...
% the
%joe4
%constraint $(x,y,z) \in K_{\exp}$ is dcp, iff $x$, $y$, and $z$ are
constraint $(x,y,z) \in K_{\exp}$ is a dcp iff $x$, $y$, and $z$ are
affine transformations of the variables being optimized.
...
%joe4
%So in a sense, the main point of an inference algorithm is to avoid
The key feature of inference algorithms for BNs and factor graphs is
that they do this calculation without expicitly representing $\Pr_{\cal M}(\X)$.
...
we can calculate $\Pr_{\cal M}(\X)$, even if we ignore ths space
required to reprsent it (as we do in this section).
%joe4: talking about luxuries and indulgencdes muddles the picture
%With this indulgence,
Note that
$\gamma$-inference is already an optimization problem by definition:
...
For sufficiently small $\gamma$, this is a convex one.
%joe4*: why do we care whether it is a disciplined conves program
But is it a \emph{disciplined} convex program? Can it be solved in
polynomial time, relative to the dimension of $\mu$?
...
\Cref{sec:small-gamma}.
% When $\gamma$ is large, we still have a way to find \emph{some}
% local optimizer via the concave-convex procedure, which we implement
% and discuss at length in \cref{sec:larger-gamma}---although
% ultimately we conclude that our ability to find one such minimizer
% of many falls short of what we would like from an inference
...
\commentout{
When $\bbeta < \balpha$, we still have a way to find \emph{some} local
optimizer via the concave-convex procedure, which we implement and
discuss at length in \cref{sec:larger-gamma}---although this alone is
insufficient for inference, as we have defined it.
...
%joe4
%When $\gamma = 0$, we have $\bbr{\dg M}_0(\mu)= \Inc(\mu)$ which is a
When $\gamma = 0$, $\bbr{\dg M}_0(\mu)= \Inc(\mu)$, which is a
...
There is a straightforward connection bween the exponential cone and
relative entropy:
...
\begin{align*}
    ...
    % \end{cases},
\end{align*}
then (a) the inconsistency $\aar{\dg M}$ equals the optimal objective
value $\mat 1^{\sf T} \mat U \bbeta$, and (b) $\mu \in \bbr{\dg
%joe4
%  M}^*_0$, which is to say that $\mu$ minimizes $\Inc_{\dg M}$.
  M}^*_0$, so $\mu$ minimizes $\Inc_{\dg M}$.
...
% Now that we have a taste for how to approach the problem in explicit
% matrix terms,
%joe4*: I have no idea what the "simplex represenation" is.  I cu t
%this sentence
%{\color{gray}Now that we have a taste for how this works in terms of
%  matrices,
...
%joe4
%Then, for each $a \in \Ar$ we can regard $\mu(\Tgt a, \Src a)$ and
For each $a \in \Ar$, we can regard $\mu(\Tgt a, \Src a)$ and
$\mu(\Src a)p\ssub a(\Tgt a | \Src a)$, both distributions over
%joe4*: please put in parens.  Be kind to the reader!
%$\{\Src a,\Tgt a\}$, as vectors of shape $\V a$.
$\{\Src a,\Tgt a\}$, as vectors with structure $\V a$.
% \in \Rext^{m \times n}
...
%joe4
%As before, we now introduce an optimization variable $\mat u$ which
As before, we  introduce an optimization variable $\mat u$ that
packages together
...
%Separately, we are still interested in the optimizers $\bbr{\dg
This is a start, but to do inference, among the minimizers of $\Inc$,
we must find the special distribution $\bbr{\dg M}^*$,
while for $\gamma$-inference, we need the optimizers $\bbr{\dg
  M}^*_\gamma$ for $\gamma > 0$.
...
When $\gamma > 0$ is sufficiently small,
% Take $\gamma = 1$.
...
To see this, note that by  \parencite[Proposition 4.6]{pdg-aaai},
\eqref{eqn:scoring-fn}
 can be rewritten as:
% \begin{align*}
\begin{equation}
...
%joe4
%translation into an exponential cone constraint, and the second one
translation into an exponential cone constraint; the second term is
linear in $\mu$.
Now,
...
The only wrinkle is that it is possible for a user to specify that
some $p\ssub a(t\mid s) = 0$, in which case the linear term explodes.
%joe4
...
we need to divide $\V\!\Ar$ into two parts:
...
\begin{prop}\label{prop:joint-small-gamma}
    ...
    % $\bbr{\dg M}^*_\gamma$,
%joe4*: why are we considering only ]]M}}_1 rather than [[M]]_\gamma
    $\bbr{\dg M}^*_1$,
...
already saw a way of finding a distribution that minimizes $\Inc_{\dg M}$,
%joe4*: it's not just that we're "particularly interested"; it's
%necessary given how we'fe defined inference.  We need to tell a clear
%story here!
%but we are particularly interested in the distribution $\bbr{\dg
%  M}^*$, which, uniquely among them, best minimizes $\IDef{\dg M}$.
but to do inference, we need to find the distribution $\bbr{\dg
  M}^*$ that, uniquely among them, best minimizes $\IDef{\dg M}$.
...
% First, a characterization of the set $\bbr{\dg M}^*_0$ of distributions that minimize $\Inc_{\dg M}$.
First, a characterization of the minimizers of $\Inc_{\dg M}$.
...
\begin{prop}\label{prop:marginonly}
    ...
    the distributions $\mu$ minimizing $\Inc_{\dg M}$ all have the same conditional
        marginals along $\Ar$.
...
%joe4
%conditional marginals along the edges, and now optimize $\IDef{}$.
%We now run into a second issue: $\IDef{}$ is not convex.
%Fortunately, when we constrain to distributions that minimize $\Inc$, it is.
conditional marginals along the edges, and optimize $\IDef{}$.
But in attepting to do so, we run into a second issue: $\IDef{}$ is
not convex iin general
Fortunately, when we restrict to distributions that minimize $\Inc$, it is.
...
%joe4
%Feeding \eqref{eqn:joint+idef} to a convex solver gives rise to the
Running \eqref{eqn:joint+idef} ono a convex solver lime that described
...
We will now show how the joint distribution $\mu$ in our optimization
%joe4
%may be replaced
can be replaced
...
For the remainder of this section, fix a PDG $\dg M$ and a tree decomposition $(\cal C, T)$ of $\dg M$'s underlying hypergraph.
...
% $[\mu_C(c)]_{c \in C \in \mathcal C}$.
% $[\mu_C(c)]_{(C,c) \in \sqcup \mathcal C}$.
...
that the source and target of every arc $a$ lie entirely within at
least one cluster.
%joe4
...
%joe4
%Problem \eqref{prob:cluster-inc} is very similar to
Problem \eqref{prob:cluster-inc} is similar to
\eqref{prob:joint-inc}, except
that it also includes local marginal constraints along the edges of the clique tree,
so that we are optimizing over \emph{calibrated} clique trees. In some
ways, it
is analogous to
...
%joe4: I didn't think problems had "objectives".  Perhaps "solution"?
    \item the objective of \eqref{prob:cluster-inc} evaluated at $\mat
...
% We now have a way of computing the inconsistency of $\Inc$ that does not require us to store a joint distribution.
...
we can express \eqref{eq:bethe-entropy} in a more useful form:
%
\begin{align*}
    \H(\Pr\nolimits_{\bmu}) &=
    % \H(\Pr_{\bmu}) &=
...
%another?  I'm lost.
Note that the denominator $\mu_C(\Pash_C)$ is an affine transformation
of $\mu_C$.
...
For vectors $\mat v = [v_{C,c}]_{C \in \mathcal C\!,\,c \in \V C}$,
$\mat u = [u_{a,s,t}]_{(a,s,t) \in \V\!\Ar}$,
and $\bmu = [ \mu_{C}(c) ]_{C \in \mathcal C\!,\,c \in \V C}$
...
\textbf{The \ObsLimit.} Suppose that $\boldsymbol\nu = \{\nu_C : C \in
\mathcal C\}$ is a calibrated clique tree over the tree decomposition
$(\mathcal C, \mathcal T)$ representing a distribution
$\Pr_{\boldsymbol\nu} \in \bbr{\dg M}^*_0$, say obtained by solving
\eqref{prob:cluster-inc}.
 % representing
%
...
Once again, let
% $\mat u := [ u^C_c ]^{C \in \mathcal C}_{c \in \V(C)}$,
...
\text{\sf subject to} &\quad
    ...
    \forall C \in \mathcal C.&~~
        (-\mat u_C,  \mu_C(C), \boldsymbol\psi_C \odot
...
At this point, standard algorithms that operate on
clique trees
...
% More concretely
Our approach to inference computes the inconsistency $\aar{\dg
  M}_\gamma$ as a side effect of finding distributions of
%joe4
...
In fact, calculating
$\aar{\dg M}_\gamma$
and $\gamma$-inference appear to be equally difficult;
%joe4*: What about inference wrt [[M]], which is supposed to be what
%thue paper is about.
%see \cref{sec:inf-via-inc} for futher discussion.
...
% \begin{lproof}\label{proof:sharp-p-hard}
    ...
    Then, because this new edge's contribution to $\Inc_{\dg M}$
...
\begin{example}
    ...
    Intuitively, both the distribution of $Y$ and the conditional
    distribution of $X$ given $Y$ can be bent to satisfy $X{=}x$, and
    in $\bbr{\dg M^{+x}}^*$ the cpd with lower confidence will be bent
    further than the one of higher confidence.
