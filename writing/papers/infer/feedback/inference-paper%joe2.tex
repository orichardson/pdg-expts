...
%joe2: see my comment immediately above.
...
%joe2
%We will sometimes use superscripts as well, especially when indicies
...
%$\mat u := [u^X_x]^{X \in \mathcal X}_{x \in X}$ defines a vector
%whose indicies range over the disjiont union $\sqcup \mathcal X$.
...
%It is equivalent to $\mat u := [u_{(x,X)}]_{x \in X, X \in \mathcal
%  X}$, but more compact.
...
%Supplying just the upper index, $\mat u^{X}$ is the projection of
%$\mat u^{X}$ is the projection of
%$\mat u$ onto the subspace where the upper index is $X$.  
%joe2
%Vectors over the same set can be added pointwise as usual, and
...
%joe2: explain (and motivate) relative entropy where you first use it.
...
%$\Ed$ of ``hyperedges'', which correspond to subsets of $V$.
$\Ed$ of \emph{hyperedges}, which correspond to subsets of $V$.  
%joe2
%A graph may be regarded as the special case in which every hyper-edge
...
%joe2
%    nodes, and a collection $\mathcal A$ of ``hyperarcs''; each $a \in 
...
%joe2: you need to be consistent about whether you're putting in the hyphen
  %\item Every vertex $v \in V$ and every hyper-edge $E \in \Ed$ is
...
% \begin{align*}
$
    % \IDef{\dg M}(\mu) := - \H(\mu) + \sum_{L \in \Ed} \alpha\ssub
    % L\, \H_\mu(\Tgt L | \Src L), 
...
%joe2*: this should have come in the introduction, and should be
%related to inconsistency in the introduction
%An inference algorithm for a probabilistic model $\mathcal M$ one that
...
%If this seems reasonable, you might suspect that this reformulation
%could increase the difficulty of optimization (\textbf{D2})---that we
...
%joe2: You haven't defined "exponential conic programs".  At best,
%you've defined exponential-cone constraints.  You have to define the
%notion of you really want to use it.
...
% Chris has been calling it that and I adopted it. But ``scoring
% function'' works too. 
...
% $\mat p\ssub L \in \mathbb R^n$.  Similarly a candidate (``joint'')
% distribution $\mu(X)$ 
...
% Fortunately, this can be done by simply running a second
% optimization problem,
%joe2: I can't parse the next sentence
...
%joe2
        %	That is to say, if there is an edge $\ed aXY \in \Ar$, and
...
%joe2: What does it mean for a representation to be less expressive?
...
%joe2: 
  %  Suppose $\dg M_1$ and $\dg M_2$ are PDGs
	...
	\]
%joe2
%	That is to say: in every optimal distribution $\mu^* \in
...
%joe2: I basically stopped reading carefully here
...
%joe2: it seems like something is missing here
...
%joe2: Sigh ... I have no idea what Bethe entropy (as opposed to
%entropy) is or why you want to compute it.
...
%joe2: again, you need to be consistent about PDG vs. pdg.  Please
%check all occurences.  I have a slight preference for PDG (which is
%what we did in the AAAI paper), but could go either way.  It's more
...
\begin{linked}{theorem}{main}
    % For   O( 3 n * ((3n+m+1)^3 ) * log(√n / ϵ) )  =   O( n^4 log(n)
  % log(1 / ϵ) )
%joe2: I have no idea what residual norm is
%There is an algorithm that takes $O(n^4 \log n  \log \nf1\epsilon )$ time, 
%and finds a point $\epsilon$-close in residual norm to the optimal
%distribution 
There is an algorithm that finds a point $\epsilon$-close in residual
norm to the optimal distribution $\bbr{\dg M}^*$ in time $O(n^4 \log n
\log \nf1\epsilon )$,
where $n$ is the total number of parameters in a clique tree.
\end{linked}
...
%joe2: you're back to "PDG objective".  Please don't introduce new
%terminology out of the blue 
...
%joe2
%Once we sort the terms in to convex terms $f(\mu)$ and concave terms
...
%joe2
%hyper-edges, with 1-2 target nodes and 0-3 source nodes. The cpds are
...
\begin{lproof} \label{proof:consistent-NP-hard}
	...
	\begin{itemize}
%joe2
          %	\item a hyper-edge $\{X_{j,i} : i \in \mathcal I(j)\}
          	\item a hyperedge $\{X_{j,i} : i \in \mathcal I(j)\}
                  \tto C_j$, together with a degenerate cpd 
