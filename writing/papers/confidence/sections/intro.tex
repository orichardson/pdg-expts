...
Therefore, if we update $\theta$ with a labeled training example $\phi = (x,y)$ to obtain new weights $\theta'$, there is no guarantee that $f_{\theta'}(x)$ gives high probability to $y$.
...
the weights do eventually converge.%
	\commentout{\footnote{Note for Joe: this is a white lie; the truth depends a bit on the architecture, and we may require that the space is compact. But this can be easily achieved by considering weights taking extended real values.}}
...
Furthermore, if we train on $\phi$ but stop before convergence,
the number of training iterations a natural notion of how to handle intermediate levels of confidence, as it interpolates
% the sequence of itermediate settings of weights 
%oli1: I'll save this verbage for later, per your request
% describes a path
% provides.
 between no confidence (zero iterations) to full confidence (infinite iterations).
...
has
an advantage: it has a clear interpretation as the ``fraction of the
way incorporated'', while he number of training iterations $n$ used in
\cref{ex:classifier} only really has meaning relative to
% the algorithm that performs the incremental updates.
% other values of $n$. 
other values of $n$ performed with the same training algorithm.
...
On the other hand, the number-of-iterations description in
\cref{ex:classifier} is at least as general, because we can use it
capture fractional confidence $\alpha \in [0,1]$ as follows.
% 
...
then sequentially performing $t$ updates of confidence $\iota$ is 
equivalent to a single one of confidence $\alpha= 1-(0.99)^t$.
Thus, confidence $\alpha$ corresponds
exactly to a sequence of 
% $t = \log (1 - \alpha ) / \log(1-\iota)$
\begin{equation} t = \log (1 - \alpha ) / \log(1-\iota) \end{equation}
updates of confidence $\iota$.
...
So, rougly speaking, the two confidence domains
are equivalent once we have a clear
picture of what a ``unit update'' means in the second case.
%
% This general idea can be cleaned by appeal to differential geometry.
% Specifically, we can take a limit
%
To illustrate this, we turn to a third example
...
what we have been calling confidence, in his preferred 
...
\parencite{shafer1976mathematical}, or equivalently, a
\emph{basic probability assignment}: a function $m : 2^W \to [0,1]$
satisfying $\sum_{B \subseteq W} m(B) = 1$ and $m(\emptyset) = 0$.
%
Now, suppose we come accross evidence that supports an event $A
\subseteq W$ to a degree $\alpha \in [0,1]$. This evidence summarized
with a second basic probability assignment
$s = \alpha\, A + (1-\alpha)\,  W$ called a 
\emph{simple support function} which 
intuitively lends $\alpha$ support to $A$, but otherwise
says nothing, by placing the rest of its mass on the trivial event.
We can then combine our prior belief $m$ with the new evidence $s_A$
according to Dempster's rule of combination, 
% to obtain a posterior belief
which in this case yields posterior belief $m'$ whose value 
at each 
% $\emptyset \ne$
non-empty
$B \subseteq W$ is:
...
\begin{align*}
	...
	\alpha \sum_{\substack{\mathclap{U \subseteq W} \\ \mathllap{U \cap A} = \mathrlap{B}}} m(U) 
		\Big).
\end{align*}
As before, when $\alpha = 0$, the posterior beliefs are the same as the
prior ones, and when $\alpha = 1$, this has the effect of removing all 
mass from sets $W$ that are not subsets of $A$ and renormalizing.
\[
	m'(B) =\begin{cases}
		& \text{ if } B \subseteq A \\
		& \text{otherwise}
...
% want to describe 
If $m$ is a also simple support function on $A$, $m'$ is as well.
No suppose we wanted a property of simple support functions on $A$,
that combines additively when we combned them with $\oplus$. 
Shafer defines that to be the \emph{weight of evidence},
and proves that, up to a positive scaling constant, it is equal to
% of $s_A$ to be 
$- \log (1-\alpha)$  
\parencite[pg 78]{shafer1976mathematical}. 
Note that this is precisely the expression for $t$, up to the choice
$\iota$ of unit above. This aspect of simple support functions turns
out not only to be useful conceptually, but also integral to the
theory of evidence he is looking to develop, and so the entirety of
Chapter 5 is devoted to it.
\end{example}
...
\begin{scope}[every node/.style={align=left,rounded corners=3,draw,thick,inner sep=5pt,anchor=center}]
	...
	\node at (0, 1.4)(path) {Smoooth Paths\\ 
		...
		\small\hfill\color{gray}(diffble)\hfill};
...
\end{scope}
	...
	\draw[arr] (vfield) to[out=-95,in=-2] node[above,pos=0.8]{$\int \mathrm dt$} (flow);
	% \draw[arr1] (vfield) to[out=90,in=0] node[below right]{$\int \mathrm dt$} (path);
	\draw[arr] (path) to[out=0,in=90] node[above right]{$\frac{\partial}{\partial t}$} (vfield);
	\draw[arr] (flow) to[out=-7,in=-85] node[below right,pos=0.8]{$\frac{\partial}{\partial t}$} (vfield);
