...
%oli3: added to address your comment that we can't use ``analogues''
% in the definition. Therefore, we have to introduce all of the
% axioms with upper bound \top so that.
...
% the interval $[\bot,\top] := [0,1]$,
$\bot = 0$ and $\top = 1$,
yet still write $\bot$ and $\top$ in stating our axioms, 
so that they apply more generally.
% $\Phi$ for a set of possible inputs,
% and $[\bot,\top]$ for the set of confidences, where $\b$. 
...
\begin{CFaxioms}
	...
	% \[
		...
		% F :  \Phi \times [0,\top] \times \Theta \to \Theta
		F :  \Phi \times [\bot,\top] \times \Theta \to \Theta
	% \]
	$
	which, given input $\phi \in \Phi$, confidence
	% $\chi \in [0,1]$,
	% $\chi \in [0,\top]$,
	$\chi \in [\bot,\top]$,
	and a prior belief state $\theta \in \Theta$, 
...
% that characterize the important features shared among
...
% This interpolation becomes more useful if it is not just continuous, but differentiable as well.
Confidence interpolates between ignoring and fully incorporating information---
...
\begin{CFaxioms}
	...
	is differentiable
	for all $\phi$ and $\theta$.
	% \unskip\footnote{in the presence of continuity (\cref{ax:cont}), 
	% 	this is equivalent to differentiability on all of $[0,1]$.}
\end{CFaxioms}
...
\begin{example}\label{ex:dupl}
	...
	% $c \notin\{ 0, 1 \}$.
	$0 < c < 1$. 
	So, in accordance with \Cref{ax:funcform}, we use $F$ to update your beliefs, partially incorporating $\phi$ arrive at some belief state $\theta_1 := F^c_\phi(\theta_0)$.
	Immediately afterwards, the friend repeats herself: $\phi$ is true.
	% Your
	Supposing our
 	confidence remains the same,
	then by \Cref{ax:funcform}
