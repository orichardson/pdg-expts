...
\textbf{Differentiability}. Suppose that the space $\Theta$ is actually a differentiable manifold. 
...
Concretely, let $\mathrm{Aut}(\Theta, \Phi)$ be the set of automorphisms $\sigma : \Theta \to \Theta$ (say, rotations of the simplex of distributions), that also have an associated action on assertions, so that $\sigma\phi \in \Phi$ is the corresponding relabeling of assertions.  The symmetry condition can now be captured by: 
...
\textbf{Sequences.}
A $\Theta$-update rule $F$ on $\Phi$ also suggests an extension to a more 
general update rule $\bar F$ on the set 
\[ 
\Phi^{\mathbb R *} := \Big\{ 
