...
As graphics is concerned, what matters in the end are pixel colors. 
...
Just as higher-level programming languages enable programmers to 
    build larger projects by giving larger-scale structure, 
    the notion of layers in graphics porgrams enables artists to create more detailed
    creations. 
For an image, what makes this possible is transparency. 
Transparency 


As statistical models are concerned, what matters in the end are probabilities. 
...
A \emph{update rule} for $\Phi$ on $\X$ is a function
\[
    F: (\text{Confidence} : \mathbb R) \times (\text{Statement} : \Phi) \to (\Delta\X  \to \Delta \X)
\]
