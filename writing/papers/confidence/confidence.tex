...
\begin{examplex}{a}{b}\label{ex:colorballs}
    \def\red{{\mathsf{red}}}
...
\begin{URaxioms}
    ...
    \item If $\Theta$ parameterizes a family of probabilities over $(\X, \mathcal A)$, 
        ...
        %(in pairs $(\beta,\Pr)$).
            ...
            \label{ax:diffble}
    \item If $\Theta$ itself has differentiable structure:
...
Some update rules, such as the one in \cref{ex:b} have a particularly conveneint property: the result of applying many updates does not depend on the order in which the information arives.
