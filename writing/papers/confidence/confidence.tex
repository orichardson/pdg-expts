...
Of course there are, already many well-established ways of representing uncertainty,
...
Indeed, the concept is so dominant that an informal poll of our colleagues suggests that most computer scientists view ``confidence'' as a synonym for probability.
...
While probability ranges from absurd (0) to undeniable (1),
...
Here, confidence can be viewed as a measure of how seriously to take an input, as one updates beliefs.
...
\begin{itemize}
    ...
    \begin{itemize}
        ...
        Each nano-bot has some type $\phi \in \Phi$, and has the effect of reconfiguring other bots to be of type $\phi$.
        Each bot type $\phi$ has a concentration $\beta_\phi$, say measured in number of bots per liter of solution, is in some sense, a measure of how much ``confidence'' the mixture has in $\phi$---if the concentration is zero, then that bot type may be ignored, and if all particles are of type $\phi$, then 
...
\begin{CFaxioms}
    ...
    \begin{enumerate}
    \item $\Theta$ itself has differentiable structure, and
...
% \end{examplex}
\begin{wip}
Consider a neural network with a final softmax layer, the policy network:
it takes observations to impuses over actions.
Such a network is said to be ``boltzmann rational'' at inverse temperature $\beta$ if $\pi(A|Z) \propto \exp(- Q^\pi())$
\end{wip}
