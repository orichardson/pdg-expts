...
\begin{example}
    ...
    Imagine a ball has been drawn from an urn, and we are uncertain about its color, which we know to be either red, green, or blue.
    Let the space $X$ be the discrete set $\{ \red,\green,\blue \}$ corresponding to possible shoe sizes, and $\mathcal A = 2^X$ be all possible subsets of it, so that a probability $\mu \in \Delta \X$ is just a 3-dimensional vector $\mat p$.
    For the assertion language, suppose $\Phi = \{$``the ball is $\mathsf c$''$ : \mathsf c \in X\}$, so that you can make assertions such as ``the ball is red''.
    ...
    As mentioned above, you can compose udpates. For instance, suppose we had a prior $\mat p_0$ over shoe sizes, and then three people come up to us and make assertions about the ball color---%
    ...
    \begin{enumerate}
        ...
        $\boldsymbol\beta^{\mat q \to \mat p} = (\beta_{\red}^{\mat q \to \mat p}, \beta_{\green}^{\mat q \to \mat p}, \beta_{\blue}^{\mat q \to \mat p})$
        such that applying the updates 
        $F^{\beta_{\red}^{\mat q \to \mat p}}_{\red}$,
        $F^{\beta_{\green}^{\mat q \to \mat p}}_{\green}$, and
        $F^{\beta_{\blue}^{\mat q \to \mat p}}_{\blue}$,
        to $\mat q$, in any order, yields $\mat p$. Moreover, this vector is unique.
        
        In general, it will turn out that if $\mu \ll \nu$ (that is, if $\forall A \in \mathcal A.~\nu(A) = 0 \implies \mu(A) = 0$), there is a sequence of updates that will take you take you from $\nu$ to $\mu$. 
...
\begin{prop}[closure under rescaling]
    If $F^{\beta}_\phi$ is an update rule for $\X$ on $\Phi$, then so is
    $G^{\beta}_\phi := F^{k \beta}_\phi$, for any real $k > 0$.
\end{prop}
...
One further requirement that we could well have added to our set 
\begin{URaxioms}
    \item For all $\phi_1, \phi_2 \in \Phi$ and $\beta_1, \beta_2 \in \mathbb R$, and $\mu \in \Delta\X$,
    we have that 
    $
        F^{\beta_1}_{\phi_1} ( F^{\beta_2}_{\phi_2}(\mu)) = 
            F^{\beta_2}_{\phi_2} ( F^{\beta_1}_{\phi_1}(\mu)).
    $
...
\begin{defn}
    An update rule is linear if its vector field can be written as
    $F'_\phi(\mu) = \mathrm{V}_\phi \mu$ for some linear operator $V_\phi$.
\end{defn}

\begin{prop}
    A linear update rule $F$ is commutative if and only if, for every pair of formulas  $\phi, \phi' \in \Phi$, the
    matrices $V^F_\phi$ and $V^F_{\phi'}$ commute.
\end{prop}

\subsection{Invertable Update Rules}
\begin{URaxioms}
    \item For all $\phi\in\Phi$, and $\beta \in \mathbb R$, the update
    $F^{\beta}_{\phi}$ is invertable.
    \hfill\textbf{(Invertability)} \label{ax:invert}
\end{URaxioms}


\begin{prop}
    If $F$ satisfies \cref{ax:invert,ax:diffble}, then for all $\beta \in \mathbb R$, $\phi \in \Phi$, the function
    $F^\beta_\phi : \Delta\X \to \Delta\X$ is a diffeomorphism, and its inverse is given by $F^{-\beta}_\phi$, in the sense that
    \[
...
So in some sense, the two are equally valid parameterizations of the space.
