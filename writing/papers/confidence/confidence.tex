...
% \begin{minipage}{0.48\linewidth}
    ...
    And yet, in the standard representation of pixel color, only three of the four bytes actually represent a color; the last byte encodes opacity---in some sense, a \emph{confidence} in the color.
    ...
    This enables much more structured and intricate digital art, via layers. 
...
\[
    \overset{*}F^{\beta}_{\boldsymbol \varphi = (\beta_i \phi_i)_{i =1, \ldots n}}
        (\mu) := F^{\beta \beta_1}_{\phi_1} \circ F^{\beta \beta_2}_{\phi_2}
            \circ \cdots\circ F^{\beta \beta_n}_{\phi_n}(\mu).
...
For a smooth manifold $M$ (such as the space $\Delta X$ of distributions over $X$), and a point $p \in M$, let $T_p M$ be the tangent space to $M$ at point $p$ \parencite{lee2013smooth}, and let
