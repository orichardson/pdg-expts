...
By contrast, zero confidence represents a truly neutral stance: 
...
Variance is also commonly used to measure (the inverse of) confidence. 
...
\cref{ax:funcform} is not as anodyne as it looks. 
Some might object that it's not possible to write such a function that is appropriate in all circumstances.
For instance, Shafer argues for Dempster's rule of combination as a way of incorporating information, but is very careful in emphasizing that only \emph{independent} information should be incorporated in this way. 
\begin{example}\label{ex:dupl}
    You have initial belief state $\theta_0$.
    Now, person $A$, comes up to you and tells you that $\phi$ is true, and
        you have some degree of confidence $c$ in them. 
    ...
    Immediately afterwards, $A$ repeats what they just said: that $\phi$ is true. 
    ...
    Unless you already know that $\phi$ is true, or $c \in \{\bot,\top\}$, it will typically it not be the case that $\theta_1 = \theta_2$. 
    And yet, it seems your belief state should be no different after $A$ repeats themself.
\end{example}
...
With this issue in mind, here are three ways to proceed.
...
\begin{enumerate}
    ...
    This would be a severe limiation to the theory.
    Imagine that we are writing code that describes how a synthetic agent updates its beliefs.  
    Shafer's approach is to package such code with a warning against running it unless assured that observations will always be independent. 
    ...
    Another feature of \cref{ex:dupl} was that we could identify the information as being the same.
    So, the relevant description of the information was not just $\phi$, but a pair $(\phi, \mathit{src})$ that also describes its source.
    
    \begin{align*}
        \Phi' &:= \Phi \times \Big\{ \text{possible sources }~ \mathit{src}~ \Big\}\\
        \Theta' &:= \Theta \times 
            \Big\{ \text{histories } L = [(\phi_1, \mathit{src}_1, c_1), \ldots (\phi_n, \mathit{src}_n, c_n)] \Big\} \\
         G^{c}_{(\phi,\mathit{src})}(\theta, L) & := 
            \left( F^{c}_{\phi}(\theta),
            \begin{cases}
                 L :: (\phi,\mathit{src},c) &\text{ if } c \ne \bot \\
                 L & \text{ if } c = \bot
        \end{cases} \right)
    \end{align*} 
    ...
    \item \textbf{Incremental Interpretation of Confidence.}
\end{enumerate}
...
% We are primarily interested in the case where confidence can be measured as a real number,
...
\begin{CFaxioms}
    ...
    \item $\Theta$ has a manifold structure, and
        ...
        is continuously differentiable at $\beta = 0$. %\label{ax:diffble}
    \item 
...
\begin{lproof}
    ...
    % \TODO[TODO: this can be done without leaning so hard on degenerate distributions]
\end{lproof}
