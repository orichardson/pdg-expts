...
\begin{wip}
    ...
    If we were working in $\mathbb R^n$, we would simply collect them 
    To convert between them, we'll need a metric tensor.
    % we have 
...
\begin{prop}
    ...
    % \begin{align*}
    $
        (\mathrm{Gibbs}'_\varphi p)_{x} = p(x) (\Ex_p[U(X,\varphi)] - U(x,\varphi) )
    $.
    % \end{align*}
\end{prop}
\begin{proof}
