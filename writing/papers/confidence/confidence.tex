...
A conservative vector field is one that is ``path independent'' --- that integrating the value of the field along path depends only on the path endpoints. 
% In three dimensions, they are called ``curl-free''. 
It is well-known that such vector fields are equivalently characterized as those arising as the gradient of a potential function. 
