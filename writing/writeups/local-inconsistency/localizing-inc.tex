...
\begin{document}
    ...
    If $\dg M$ is an inconsistent PDG, one  representation of its inconsistency if every edge ``participates'' in the inconsistency. That is, for all edges $L \in \Ed^{\dg M}$, we have that $\aar{ \dg M \setminus L} < \aar{\dg M}$ for all $L \in \dg M$. 
