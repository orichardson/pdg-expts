...
%oli11: to visually set captions apart
\usepackage[format=plain,
%oli11: is this usually cheating? The Neurips Guidelines are silent on figure caption size.
            labelfont={sl},
...
%\declaretheoremstyle[ 
%	headfont=\normalfont\slshape\color{subfiglabelcolor}\itshape,
...
\newcommand{\note}[1]{{\color{subfiglabelcolor}\ \!\Large\smash{\textbf{[}}{\normalsize\textsc{note:} #1}\ \!\smash{\textbf{]}}}}
...
%joe9*: I really don't like fraktur. There's a reason that you hardly
%ever see it. I have typically used italic M for models.  Why not do
%so here?  It's much less forbidding that fraktur to most people (ask
%your friends!)
%\newcommand{\dg}[1]{\mathfrak{#1}}
%oli11: I've actually seen fraktur more in model theory books than anywhere else. If you really dislike fraktur though lets not do it. I wanted to reserve italic fonts for functions though, especially now that they're mnemonic. Also I believe \mathit{M} is indistinguishable from $M$ for the NeurIPS font. 
% What about script?
...
%\newcommand\IBal{\mathit{Extra}}
%oli10: changed name of command, and what it results in.
%joe9
%\newcommand\IBal{\mathit{Gib}}
%oli11:
% \newcommand\IBal{\mathit{IF}}
%oli11: I'm now realizing I unintentionally reverted the naming choice (I was looking for something new and changed my name a lot). I don't care all that much what you change it to but ideally it would retain the G- in front (the subscripts / superscripts don't look as good, and we're trying to emphasize fixed model), not start with I (because Inc does), and have some lower case letters, for stylistic coherence.
...
%\newcommand\extrainfo[2][\dg M]{\IBal(#2;#1)}
...
% If you have a collection of BNs, I would agree that naming the associated PDG \dg M_{B} is the right thing to do, because it emphasizes the dependence, and is defined in a scope where it won't contaminate the meaning of "$\dg M$"" in general. 
% Your notation takes this to an extreme I find unpalatable. You use this subscript notation for the general function which returns (by analogy) the PDG associated to an arbitrary, as of yet undefined PDG, which could be stored in any symbol. Therefore, M_{<symbol>} is bound to the conversion function, but M_1, M_2 are also used, and so is just normal M. I find this difficult to read. I also know for a fact that more than one student in your RaU class has been confused that the function that transforms things is the conflated with its output and other unrelated variables, and that a mere notational change improved things. It is just a mathematical fact that by naming things this way, the grammar you define ceases to be context-free, which makes it harder to parse. It also is now almost impossible to refer to the mathematical object corresponding to the funtion (e.g., which takes a factor graph and returns a PDG: the standard use of placeholders would give [ - ]_[.], which is certainly useless). The subscript also prevents you from then having indexed collections of models in general, because M_i now means transform i (and the indexed set could be indexed by a collection of bayesian networks...), and doesn't typecheck. IF you use M(X) instead, you have similar issues, and the paranetheses keep you from being able to use a model as a function (I woulld like to be able  to write {\dg M) (Y \mid X)). 
%
% Two more, even scarier effects. First: what do you do if you have {\dg M} in scope, and want to convert a factor graph? Shouldn't {\dg M}(\Phi) or {\dg M}_{\Phi} have some dependence on {\dg M}? It sure looks like it does. 
% Second: you also have to be MUCH more careful to verify coherence properties. Every time you define $M_X$ for a new class of objects $X$, you have to be very careful that every detail of the conversion exactly lines up; you otherwise have to write . Now \Gamma, \Gamma' might not be super readable and I am sympathetic to that complaint, but it is always clear what computation is occuring, and which function is being used. If you are good about structuring your document and defining things, \Gamma will quickly become a part of the reader's langauge just as quickly (I argue more quickly if you use it often, because there are fewer symbols and no subtleties) as M_B.
%
%I find this particular bit of notation, which I acknowledge that you have been using consistently, to be very poor for more reason than just aesthetics, and on principle I would like to avoid adopting it if I can.
% I admit that fraktur font is intense, but I think that using a wide variety of visual symbols makes it WAY easier to track.
% Finally, you might be surprised to learn that when I show people my work, I often get a surprising number of complements /specificlaly/ regarding my notation. I can think of instances of this happening spontaneously, coming from category theorists, physicists, ML people, and even one of your students. Regularly my friends go so far as to accuse graders of giving me higher scores just because my assignment is prettier. I know I sometimes make dumb decisions, and my notation can become inconsistent especially when I write something over a long period of time (though I always plan on making it uniform eventually), but I believe that I'm actually way better than average at coming up with notation that (at least certain people) really like. 
% Bottom line: We seem to be on the opposite ends of some notation spectrum, but I also care a lot about notation. I am bad at many things, including focusing, and telling stories in straight lines, but I want you to know that in general, choosing notation is not one of my weaknesses.
%
...
%oli11: Because this is the function and not its output, and  {\dg M}. is already used for PDGs, I want this to be called something {dg M}. Some more suggestions. 
%oli11: Option 1. use mathit "pdg" (with kearning adjustment so it's more compact and the g does not appear offset)
...
%joe9: this will get the numbering to work out right for theorems in
%the approndix
%oli11: it doesn't update the staements properly though, and I think what I had before worked (except I was lazy and had a few broken labels).
% Also, I don't get why you don't just add the code in the \newcommands to the environment definition directly, and just define one of these...
...
%joe7
			...
			\end{subfigure}%			
%oli11: margins too small for new caption, need to squeeze some more space out of this...
			...
			\begin{subfigure}[b]{0.57\textwidth}
				...
				\draw[very thick, |-|, color=orange!50!black,text=black] (2.7, 1.35) --node[above=0.5em]{\small Restricted PDG in 
				%oli11
					% Examples~\ref{ex:grok-ablate}~and~\ref{ex:grok-union}}
					\Cref{ex:grok-ablate,ex:grok-union}}
					(7.1,1.35);
...
%            restriction} 
          ...
          \caption{The \MN\
		   	$\PDGof{{\mathcal B}}$ corresponding to ${\mathcal B}$, and a 
            restriction of it.} 
...
%oli11: Quickly hint about why this works syntactically, so we can better summarize this contribution
We have stressed that edges are interpreted differently in PDGs than in Bayesian Networks. Readers should be aware that this approach is closely related to the notion of a \emph{commutative diagram}, a common representation in pure mathematics. For those familiar with them, the potential failure of a PDG to be consistent is analogous to the possibility that a diagram could fail to commute.
...
%oli11: rewriting the semantics intro. There's a lot of redundancy here, the emphasis is a bit strange, the story is dry, and space is precious
%TODO
\commentout{
Although the meaning of an individual cpd is clear, the .
We consider three ways of imbuing a PDG with global meaning. 
}
...
%joe9: doing it more consistently
%       $\mu(Y\sheq \c \mid X\sheq x)$ is the $x,y$ entry in the cpd given by
                %                $\bp$.
   $\mu(Y = \cdot \mid X\sheq x) = \bp(x)$.
...
%oli7: oops...
    ...
    %joe7
    %oli11: you say expected below, no expectation yet.
%        expected 
%joe8
%      		divergence from $\bp$ to $\mu(Y\mid X=x)$,
%oli10: it's not symmetric, so linguistically the direction
%matters. (and unfortunately it's written backwards, so "from the
%second argument to the first argument" is correct.) 
%      		relative entropy between  $\bp$ and $\mu(Y\mid X=x)$,
%joe9
%		relative entropy from $\bp$ to $\mu(Y\mid X=x)$,
%oli11: add spacing
		% relative entropy from $\bp(x)$ to $\mu(Y = \cdot \mid X=x)$,
		relative entropy from $\bp(x)$ to $\mu(Y \!= \cdot\mid X=x)$,
...
%oli11: shorten, remove linguistic repetititon
% then take the expectation  with respect to $\mu_X$ (that is, the
%                 marginal of $\mu$ on $X$). 
take the expectation over $\mu_X$ (that is, the
                marginal of $\mu$ on $X$). 
...
% So what's the verdict? Incompatibility or inconsistency? We talk
%about inconsistency a lot, so if we don't include the definition of
%inconsistency of a PDG (without a distribution) below, we probably
%should call this inconsistency -- even though I agree that this feels
          %slightly wrong.
%joe9: Either one is fine, just not both.
	The \emph{incompatibility} of a PDG $\dg M = \mnvars[]$ with
	a joint distribution $\mu$, denoted $\Inc(\dg M,\mu)$, is  
    \[
		...
		\!\!\!\sum
			...
			\beta_L
                    ...
                    \left[\kldiv[\Big]{ \mu(Y\!= \cdot\mid X \sheq
                                                    x) }{\bp(x) } \right] ,
	\]
%oli2: I suppose I also need to define relative entropy?
%joe3: yes
%oli11*: I would like to put the definitions of relative and conditional together, at the beginning of the section. They appear everywhere in the semantics, and defining standard symbols you use everywhere in the middle of just one definition seems like poor organizational structure. My impression is that peole skip over things quickly, and then when trying to understand something they look first in {the related area, the begining of the paper, and google}. If someone skips to the middle or end or is reading the appendix and sees this D symbol, the only way they'll know what it means is if they go back through the paper carefully until it stops appearing. 
%oli11:  it in a standard place.
...
%oli10: We now use it a lot in our proofs. We can either uncomment it
%        here (benefit: we deliver on describing inconsistent PDGS,
%        not just with respect to distributions), or in the appendix
%        (which will give us space for higher value things. For now
%        I've uncommented it but either resolution is ok with me.) 
%joe9: you dind't uncomment it, which is good; it belongs in the appendix
...
%        $\SD{\dg M}$ and $\Inc(\dg M;-)$ distinguish only
        $\SD{\dg M}$ and $\Inc(\dg M, \cdot)$ distinguish only
...
%oli11: Commenting it all out and trying again.
\commentout{
		Suppose an agent has a PDG $\dg M$ in mind, and
                imagines that all sample variation in a joint
                distribution $\mu$ over $\V(\dg M)$ arises as a result
%joe9*: In the next line, did you mean ``some'' or ``each''? See my
%next joe9*
%oli11: Both words are ambiguous because of the sentence structure, but I meant to say that we imagine the distribution is generated by rolling the dice in each link. What we do is clear from the formula. I think this is why I prefer putting the formula before the explanation --- people read the explanation already knowing what you're going to do, and then learn why you do it.
                of sampling the value of a target variable $Y$ of some
%joe9
%    link $\ed LXY$, given the value already at its source. If this
%                were the case, one would expect the 
                edge $\ed LXY$, given the value of $X$. If this
                is the case, one would expect the 
                total amount of information required to communicate a
                sample of $\mu$ to be the same as the total amount of
                the information required to separately encode, for
%joe9: why do you need to use the word ``randomness''.  Also, while I
%prefer ``edge'' to ``link'', we should be consistent
%                each edge $\ed LXY$, the the randomness $Y$ given $X$.   
%                each edge $\ed LXY$, the probability of $Y$ given $X$.   
                each edge $\ed LXY$, the probability of $Y$ given $X$.
%oli11: This is not what I mean. You're encoding the identity of a sample,
% drawn randomly from \mu. You're not encoding the problabaility; that's assumed 
% to be known.   
%joe9*: I'm starting to lose it.  You start talking about samping the
%value of Y for *some* link X-> Y.  Then you switched to talkiing
%about all links.  
%oli11: I said *all* randomness was specified by *some* link, by which I meant that collectively, *all* links together would be compared with the total randomness.
		% If all variables are constant according to $\mu$, or
                % $\dg M$ corresponds to the BN that  $\mu$, the two
                % quantities will be identical. 

		For an arbitrary PDG and $\mu$, these two quantities
                will differ; if the amount of uncertainty in the
                distribution is lower than one would expect from the
%joe9: note that it's ``deficit''
%                links, the distribution has an information defecit,
                edges, the distribution has an \emph{information deficit},
                indicating that there are additional correlations
%joe9
%                between variables, that are not suggested by the
                between variables that are not captured by the
                graph. The higher the information deficit, the 
%joe9
                %  worse the qualitative fit of the distribution.
worse the qualitative fit between the PDG and $\mu$.
%joe9*: Now you lost me.  What is ``surplus uncertainty''? This needs
%tob e given *much* more intuition.
		On the other hand, if a distribution has surplus
                uncertainty, using it confers a benefit.
                For instance, a PDG containing only the variable $X$
...
%mentioned. 
%oli11: I think this is false. While to express "safe" in general, you would need a utility function, I claim that there is not *any* utility function that, over the joint space of random variables here, would change this calculus at all (except to make it so that you don't care). 
% there's 
               and no edges encodes awareness about $X$ but no
               beliefs about it. Given that $X$ could be distributed
               in any way (even chosen adversarially), it is safest
               to act as though you believed $X$ to be uniform.  
%oli11: irrelevant because commented out anyway, but I think what I wrote is more coherent than the below.
                % and no edges leading to $X$ means that the agent is
                % aware of $X$, but has no  information regarding the
                % probability of various values of $X$ but no
		We measure the surplus/deficit with a quantity we call
%joe9: I don't like this terminology
%                the \emph{Graph Information Balance} ($\IBal$) 
                the \emph{information fit} ($\IBal$) (between ${\dg M}$
                and $\mu$):
}
%oli11: end comment. The summary at the end of our meeting was,
%" Given a distribution, compare the cost of specifying \mu directly to the cost of specifying the information on the edges for \mu.". You also said not to say anything about samples, but it's information required to specify a sample of the distribution, given the distribution. I'll try a slightly different appraoch, but I don't have a good idea about what's going to stick.


%Let $G$ be a (multi-)graph whose nodes correspond to random variables. 
Each edge $\ed LXY$ of a PDG $\dg M$ represents a qualitative claim that the value of $Y$ can be (noisily) computed from $X$ alone.
	%I'm intentionally not saying anything about randomness here, to keep things simple, and make the flow better. I think the word "qualitative" lets me get away with this, because it is morally correct.
%
A joint distribution that qualitatively matches $\mu$ will intuitively have as much variation as possible, except where we have claimed that one variable depends on another. 
%
To formalize this, we require only the underlying multigraph $G := (\N^{\dg M}, \Ed^{\dg M})$ of $\dg M$.
For an arbitrary  $G$ and $\mu$ are known, contrast the amount of information required to
\begin{enumerate}[label=(\alph*)]
	\item directly describe a joint outcome  $\mat w ~ \sim \mu$ drawn from $\mu$, and
	\item separately specify, for each edge $\ed LXY$, the value $\mat w_Y$  given the value $\mat w_X$, in expectation.
\end{enumerate}
% (a)  and (b) 
When these two quantities are equal, an agent that is prepared to specify (b) is expecting a description of exactly the same length as the one required to describe the world. In particular, this is true if $\mu$ is a degenerate distribution, in which case the agent, who knows $\mu$, requires no descriptions at all. 
If (b) $>$ (a), then there are extra correlations in $\mu$ that are not suggested by the graph structure, which makes a $\mu$ a worse fit if you think the graph is correct.
If (a) $<$ (b), then there is something in the distribution that $G$ is silent on; distributions that are more uncertain in these places fit the graph better. We consider now the difference between (a) and (b).
%oli11 option1: shorter, better focus, but not
	% $y \sim \mu(Y \mid X)$.
%oli11 option2: more technically correct, but longer and distracts from focus of story with details that we've mentioned elsewhere
%	$y \sim \mu(Y \mid X\sheqx)$ in expectation over $\mu_X$.

Formally, for an arbitrary multi-graph $G$ on a set of variables $\N$, we define the the \emph{$G$-information balance} ($\IBal[G]$), to be the function which computes this difference for an arbitrary distribution $\mu$.  
\begin{equation}
%joe7
%          \IBal'(\dg M; \mu) := \sum\alle \H_\mu(Y\mid X) - \H_\mu.
%oli11: no more M in parentheses.
% \IBal(\dg M; \mu) := \sum_{\ed{L}XY \in \Ed^{\dg M}} \H_\mu(Y\mid X) - \H(\mu),
 \IBal[G](\mu) := \sum_{\ed{L}XY \in \Ed^{\dg M}} \H_\mu(Y\mid X) - \H(\mu),
  \label{eqn:alt-extra}
\end{equation}
...
where the notions of ``information"' and ``uncertainty'' are measured via entropy.
%oli11: this gets in the way of the flow for me. I don't know if a footnote is acceptable. 
\footnote{
...
\definecolor{subfiglabelcolor}{RGB}{0,0,0}
		% \tikzset{venn/.style={opacity=0.5, draw=black,thick}}
		% \tikzstyle{isbn}=[background rectangle/.style={fill=black!35},show background rectangle}]
...
\begin{example}
	...
	\caption{\itshape Illustrations of example graph information functions $\{ \IBal[G_i] \}$, drawn underneath their associated multigraphs $\{ G_i\}$. Each circle represents a varaible; an area in the intersection of circles $\{C_j\}$ but outside of circles $\{D_k\}$ corresponds to information that is shared between all $C_j$'s, but not in any $D_k$. Variation of a candidate distribution $\mu$ in a green area makes its qualitative fit better (according to $\IBal$), while variation in a red area makes its qualitative fit worse; grey is neutral. Only the boxed structures in blue, whose graph information functions can be seen as assertions of (conditional) independence, are expressible as BNs.}
...
\end{prop}
        ...
        While we focus on this particular scoring function in the
        paper, largely it leads to interesting results and has deep
...
%leaving it.
%oli11*: oh right! it's definitely also true that if it was convex before adding a set of deterministic edges, it continues to be convex afterwards. Is this actionable? 
...
% We then define
Let
%oli11: line shave with \smash
% $\bbr{M}^*$ be the unique element of $\lim_{\gamma \rightarrow 0} \bbr{M}_\gamma^*$.
$\bbr{M}^*$ be the unique element of $\smash{\lim\limits_{\gamma \rightarrow 0}} \bbr{M}_\gamma^*$.
%
%oli10: added footnote, equantion* environment
%oli10*: I'm now convinced that this proposition is (a) true, and (b)
%not trivial to prove.
%joe9: It's not that hard. Sorry that my note didn't go out lat
%night.  I'm having real problems with Thunderbird.  In any case, the
%limit is always well defined.  I defined it in the previous sentence.
%	\footnote{The limit is well-defined by \Cref{prop:lim-exist}} 
%oli11: no display mode, moved above.
% \begin{equation*}
...
%oli11: stating the more general fact now, There are actually two things going on here, which is why I had two propositions....
\begin{prop}\label{prop:consist}
%	If $\dg M$ is a consistent PDG, then $\UD{\dg M} \in \SD{\dg
  %M} = \{ \mu : \bbr{\dg M}_0(\mu) = 0 \}$
%oli11
% If $\dg M$ is consistent, 
% then $\bbr{\dg M}^*$ is a singleton $\{\mu\}$, where $\mu \in \SD{\dg  M}$.
$\bbr{\dg M}^* \in \bbr{\dg M}_0^*$; in particular, if $\dg M$ is consistent, then $\bbr{\dg M}^* \in \SD{\dg  M}$.
\end{prop}
...
\commentout{        
	\begin{remark}
		...
		$\bbr{\dg M}'_* = \bbr{\dg M}_*$
	 	where $\bbr{\dg M}'_*$ is the variant of \eqref{eq:uniqdist} which uses the alternate formulation $\IBal'$ of the extra information in place of $\IBal$.
	\end{remark}
}
%joe6
...
%oli10:
	...
	are strictly more general than BNs, and can emulate factor
        graphs for a particular value of $\gamma$. 
...
The construction described in \Cref{ex:smoking} can be used more generally to convert arbitrary Bayesian Networks into PDGs. Because the former does not encode confidences, $\beta$ can be supplied independently.
\begin{defn}[BN to PDG]
	Given a BN $\mathcal B$, and a positive number $\beta_X$ for each variable $X$ of $\cal B$, let $\PDGof{\mathcal B, \beta}$ be the PDG containing of the cpds of $\cal B$, 
%oli11*: we should discuss how to set \beta for projection edges. I'm going to sweep this bit under the rug and deal with it in the appendix. Note that it is an aesthetic choice and may affect belief revision, but with relative entropy it does not change the semantics because failing to follow this constraint always gives you an infinite score.
	plus the additional logical structure described in \Cref{ex:smoking}.
	% setting each projection $\{\pi_j\}$ with $\beta_{\pi_j} = 1$ together with the confidences
	The formal details can be found in the appendix (\Cref{def:bn2PDG}).
\end{defn}
...
%oli11:
 	...
 	If $\cal B$ is a Bayesian network (specifying a distribution $\Pr_{\cal B}$), then
%oli11: insert \betas, and reword because one semantics distribution is provably unique.
	...
	%             ${\mathcal B}$.
	for all $\gamma > 0$ and all vectors $\beta$ over the nodes of $\cal B$,
	$\bbr{\PDGof{\mathcal B, \beta}_\gamma^*}$ contains a unique element, equal to $\bbr{\PDGof{\mathcal B, \beta }}^* = \Pr_{\cal B}$.
%oli11 line shave by moving to top
%	where $\Pr_{\cal B}$ is the distribution specified by
%	            ${\mathcal B}$.
%joe8
%%oli9
...
%joe8: that might be true, but you can't introduce the notation Pr_B
%out of the blue           
...
%                the conditional independencies of $\cal B$.	 
	...
	\begin{vfull}
		\begin{theorem}
			If $\mathcal B_1, \mathcal B_2, \ldots$ are a conditional Bayesian networks containing whose sets of variables they condition on are pairwise disjoint, then they can be combined into one conditional BN $\cal B$, and  PDG union
			\[ \dg M} := \bigoplus_i \mathcal \PDGof{\mathcal B_i} \] 
			satisfies
...
%joe9: I don't know what ``external'' means.
%\footnote{This parameterization is generally external to the factor
%  graph, and is the associated exponential family; to describe what is
%  genreally called a ``factor graph'', one would take each
%  $\theta_L=1$.}
\footnote{Typically, a factor graph is taken to be the special case
  where all the parameters $\theta_J$ are set to 1, while the more general
...
(as in \Cref{def:bn2PDG}), and edges $\sf 1 \to X_J$ with
...
%joe9: it's not the operations that have the same semantics
...
%	By using the alternate formulation $\IBal'$ of the extra
...
%oli10: By this, you don't know how it got to be this way? (I now have
%a proof in the appendix which I would reference) 
...
happen when every $\beta_L=\gamma$. 
%This view also clarifies additional properties. 
}
...
%oli10: there's nothing special going on here. It's just proportional
%to \phi(x)^2 instead of to \phi(x), so big things get bigger and
%small things get smaller. 
...
%joe9: I can live with this for the abstract, although it should go in
%the full paper. I didn't understand what ``more deterministic'' meant
%at first.
\begin{vfull}
...
increasing $\beta_L$ makes an agent more certain about the
...
% Also I'm not convinced by the argument in any setting: just because the ones that give probaility 1 and 0 are the ones that minimize \IBal, doesn't mean that all intermediate distributions score the same. Actually the scores definitely will not cancel.
...
% minimize $\IBal(\dg M; \mu)$, the ones that give $x_1$ either
...
%oli11: trying again to integrate some of what I wrote above in clearer terms. Note that now the commutative diagram hint comes in the syntax section.
Although PDGs are visually similar to a BNs and equivalent when both are rooted trees, a slight change in the way edges are interpreted brings them in line with commutative diagrams,
and makes them
...
%oli11: I want something simpler than "representational capacity" but I can't think of what to say instead
and representational capacity (nodes)
%oli11
% regarding nodes and edges
without affecting previously-received information.
%oli11: I like what you have written, but my paragraph above says a lot more. I'm trying to merge them to make it clearer.
%oli11: This local meaning is super important. It was in the equation, but I keep not being able to show it directly; I'll try to re-word. 
In stark contrast to factor graphs, which are equally flexible, restrictions of PDGs continue to
retain the same local commitment to the meanings of cpds in the restriction; this is 
As a result, PDGs exhibit both flexibility and locality, making them uniquely modular.
...
\item They allow for a clean separation between quantitiatve
  ...
  the graph struture; this is captured by the terms $\Inc$ and $\IBal$
  in our scoring function.
\item PDGs have (several) natural semantics%
%oli11 added
, each of which paints a global probabilistic picture with constraints, making a PDG more useful than a mere collection of data.
\end{itemize}
We showed that PDGs
...
some of the arguably unnatural properties of factor graphs (a pont
...
.
%oli11: added
Moreover, it seems that the setups to a very large class of standard problems (e.g., variational approximation, belief updating) are naturally encoded as PDGs, while their standard resolutions (resp., variational inference, Bayesian updating) can be regarded as reductions of its inconsistency.
%
...
%don't need it.
%oli11: I actually asked Bobby for a reference and he said it was so standard that everyone just says it. He even looked through a couple standard convex analysis books and says it's not there. I proved it because you asked for a result I couldn't find one.
%oli11: It may be worth keeping some of the strong convexity stuff around though; strong convexity is a _lot_ more useful for finding the minimum than strict convexity, and ML people will immediately know that this means it is efficient. 
...
%oli11
				 ...
				 $\mu(X=x)>0$. By Gibbs inequality \cite{mackay2003information},
                 $\kldiv{\mu(Y|x)}{\bp(x)} = 0$. Since this is true
                 for all edges, we must have $\Inc(\dg M, \mu) =
...
%joe9
                 ...
                 edges must be positive as well, and so $\Inc(\dg M,
...
%oli11: aaahhh it took me an hour to edit this, and I don't think anything even changed.
...
%oli11 removing "and so" breaks flow of equantions; replace with \implies
            % \intertext{and so,}
		\implies\qquad
			\Inc(\dg M, \lambda p_1) + (1-\lambda)p_2)
%joe9
%                        &\leq \lambda \Inc(\dg M,p_1) + (1-\lambda)
                        %                        \Inc(\dg M,p_2)
...
%joe9: where did the w come from, and how does it relate to x and y?
%We need to explain the notation before we start.
%oli11: I've added a sentence.
	The scoring function for PDGs, given \Cref{eqn:full-score} can be expanded as
	  \[ \bbr{\dg M}_\gamma(\mu)= \E_{\mat w \sim \mu} \Bigg\{   \sum_{ X \xrightarrow{\!\!L} Y  } \left[
			\beta_L \log \frac{1}{\bp(y\mid x)} + (\gamma
					...
					\Bigg\},  \] 
%oli11
		where $x, y$ are the the respective values of the variables $X$ and $Y$, at the world $\mat w \in \V(\dg M)$. 
%oli11: optional
%drawn from $\mu$.
	\end{prop}
	\begin{proof}
	  \begin{align*}
			\bbr{\dg M}_\gamma(\mu) &:= \Inc(\dg M, \mu) + \gamma \IBal(\dg M, \mu) \\
...
%oli11: updated
% If $\dg M$ is consistent,
% then $\bbr{\dg M}^*$ is a singleton $\{\mu\}$,
% where $\mu \in \SD{\dg  M}$.
$\bbr{\dg M}^* \in \bbr{\dg M}_0^*$; in particular, if $\dg M$ is consistent, then $\bbr{\dg M}^* \in \SD{\dg  M}$.
\eopro
...
%		By \Cref{prop:convex-if-gamma-small}, for any $\gamma
%< \gamma_0$, $\bbr{\dg M}_\gamma$ is a strictly convex function of
%$\mu$ and hence has a unique global minimum.  
%
%		It remains to show that the function $f : (0,
%                \gamma_0] \to \Delta(\V(\dg M))$ given by $\gamma
%                \mapsto \argmin_{\mu} \bbr{\dg M}_\gamma(\mu)$,
%                converges as $\gamma \to 0^+$. 
%%joe9: I'll leave it to you to slot in my proof.
%		\todo{}

%joe9*: simplified proof
Now suppose that $\bbr{\dg M}^* = \{\mu\}$. We want to show that $\mu$
is consistent with ${\dg M}$.  Suppose that $\bbr{\dg M}^* = 
\{\mu_\gamma\}$.  By the argument above, $\mu_\gamma \rightarrow
\mu$.
Since $\Inc$ is clearly continuous, it follows that $\Inc(\mu_\gamma)
\rightarrow \Inc(\mu)$.  
%	\begin{prop}\label{prop:consist}
...
%joe9
%		First, note that $\IBal$ is a finite sum of entropies
%                and conditional entropies, over the variables $\N^{\dg
%                  M}$, which have finite support --- and therefore is
%                bounded. As a result, there exist real constants $\lb$
                Since $\IBal$ is a finite sum of entropies
                and conditional entropies over the variables $\N^{\dg
                  M}$, which have finite support; thus, it is
                bounded.
%joe9*: the lower bound is irrelevant
%                As a result, there exist real constants $\lb$
%                and $\ub$ depending only on $\N^{\dg M}$ and $\V^{\dg
%                  M}$ such that $\lb \leq \IBal(\dg M, \mu) \leq \ub$
                %                for all $\mu$.
%oli11: Not true, it can be negative.
% Thus, there exists some bound $K$ depending only on $\N^{\dg M}$ and
Thus, there exist real constants $\lb,\ub$ depending only on $\N^{\dg M}$ and
$\V^{\dg M}$ such that $\lb \leq \IBal(\dg M, \mu) \leq \ub$ for all $\mu$.
%oli11: we don't assume M is consistent anymore, we need the lower bound, and I prefer my version anyway, so I'm cutting this and reinstating my version below. I don't think it's any more complex, and the reference to the other version
\commentout{
Since $\dg M$ is consistent, there is some distribution $\mu^*$ such that
$\Inc(\dg M, \mu^*) = 0$.  Thus, $\bbr{\dg M}_\gamma(\mu^*) \le \gamma
\ub$.  Since $\bbr{\dg M}_\gamma(\mu_\gamma) \le \bbr{\dg
                  M}_\gamma(\mu^*)$, we must have
$\Inc(\dg M, \mu_\gamma) \le \bbr{\dg M}_\gamma(\mu_\gamma) \le \gamma
                \ub$.  It follows that $\Inc(\dg M, \mu) = 0$, as desired.
}%\oli11 end commentout
%oli11: reinstate
Therefore, for any distribution $\mu \in \Delta(\V(\dg M))$, we have
%joe9: this seems like overkill
%oli11: But I already did the overkill and it's super clear what's going on. It takes me a lot of energy to read what you've written. I thought the alignment was really easy to visually track and spent some time making sure it looked nice.  I can understand you not wanting me to waste my time doing things like this, but I really do not understand you changing it. 
%oli11: reinstating because I'd rather do this than fix the lower bound in yours
% \commentout{                
		\begin{alignat*}{4}\relax
			&\forall\gamma,\mu.~&\gamma\lb &~\leq~& \gamma\IBal&(\dg M, \mu)  &~\leq~&  \gamma\ub \\
		% \intertext{\centering Adding $\Inc(\dg M, \mu)$ to each quantity}
               % \implies
			&\forall\gamma,\mu.~&
			\Inc(\dg M, \mu) + \gamma\lb &~\leq~& \Inc(\dg M, \mu) +& \gamma\IBal(\dg M, \mu)  &~\leq~&  \Inc(\dg M, \mu) + \gamma\ub \\
			&\forall\gamma,\mu.~&
...
%                variables, subject to the sole constraint of being
%          compatible with $\PDGof{\cal B}$ (which again means that
...
%joe9
%                X_n$ be any ordering of the variables in $\mathcal B$,
...
%joe9*: this feels like overkill.  Isn't there a simpler argument?
%		We now put the following facts in equational form for
                %                ease of use:
...
%joe9
%                        node. Recall from Definition~\ref{def:bn2PDG},
                        node. Recall from Definition~\ref{def:bn2PDG}
                        that $\PDGof{\cal B}$ contains all of these
                        cpds, and possibly also degenerate cpds
                        $\pi_{i,j}$, which, for a setting $(y_1, y_2,
                        \ldots, y_m)$ of the new joint variable
                        $\Pa(X_i)$, determines a (degenerate)
                        distribution over a particular parent $Y_j \in
                        \Pa(X_i)$, by putting all mass on $y_j$. This
                        cpd is equal to $\delta_{y, y_j}$, and
                        $\H(\delta_{y, y_j}) = 0$. Although the value
                        of $y_j$ may change between settings of
                        $\Pa(X_i)$, in each case it is a point mass,
                        which always has no entropy.  
...
%	\todo{Write out $\Inc$, proofs of theorems}
	...
	\begin{figure*}[t]
		\centering
		\tikzset{attn/.style={draw, fill=magenta, fill opacity=0.3, font=\Large\slshape\color{subfiglabelcolor}, inner sep=4pt}}
		\scalebox{0.75}{
			\begin{tikzpicture}
			\begin{scope}[every node/.style={ellipse, fill, fill opacity=0.05,text opacity=1,
					outer sep=3pt,font=\slshape\color{subfiglabelcolor}}, xscale=2.5,yscale=1.2]
				\node (KB) at (-3, 0.5) {KB};
				\node (CG) at (-3, 1.5) {CG};
