...
\begin{proof}
    ...
    For any $\omega \in \V\!\X$, we can apply condition(d) in the case where $s_i := \Src i(\omega)$ and $t_i := \Tgt i(\omega)$ to get that
    ...
    \begin{align*}
        ...
        &= \prod_{i = 1}^n \nu(X_i \mid \Src i) 
            &\text{by (b)}
    \end{align*}
    Therefore $\nu(\X) = \mu(\X)$ factors as required by the BN $G$, meaning that $\mu$ has the independencies specified by $G$.
\end{proof}
