...
%oli1: what do you think of the point in this footnote? Do you think
        %it's worth bringing up somewhere?
...
%oli1: I've taken most of your suggestions and merged some of my old
%points back in
...
%joe1: removed paragraph berak and rewerote. this seems rather ciruitous
%
%In particular, we wish to highlight \MNs\ substantially improved
%modularity.
%As we will see, we can efficiently both add new information to a graph
In particular, as we shall see, $\MN\s are significantly more modular
than other appraoches.
...
%computational costs'', and we don't discuss this issue.  I also don't
%know what it would mean for a representation to be more dramatic.
...
%you mean semantics. 
%The semantics is discussed later.  If you mean something else, you
%have to make it clear.  I don't know what it means to ``promise
%deeper parallels''.  Let the examples show how PDGS are a natural
%etension, rather than saying it.  We really don't want to talk about
...
the (unconditional) marginals of $F$ and $G$, or we choose a
...
%joe2
%		But then what should we do with the original prior
		But then what should we do with the original 
                probability we had 
...
%                assume $F$ and $G$ to be independent;% (but will later
                is like our earlier BN, except that we no longer
                assume that $F$ and $G$ are independent;% (but will later
                                % if necessary) 
...
%them.  Moreover, it's notthe ability to represent
%inconsistency/over-constrained states that's useful when there's no
%inconsistency, but the modularity.  Reverting back.  In general, 
...
%reader first.  
	...
	%oli1: this is not part of the cpt on the right, and we also
        %say it below. 
...
%distraction.  
%oli1: it is necessary to distinguish the observed nodes for this
%diagram to make any sense.There is a very clear standard for how to
%do this: to make them darker. Not disitinguishing them could be
%confusing for people who use conditional graphical models
%joe2: I stand by what I said.  What in the discussion below
%uses/needs the fact that they're colored.  Why introduce coloring if
%you don't need it?  If you are really concerned, you can add a
%footnote saying that, in the standard representation of conditonal
%Bayesian networks, the observed nodes are typically colored.  (I
%don't think that this footnote adds anything, but if you really think
%that people are going to be confused, feel free to add it.)
...
%Ignoring the faint ``water'' node for now, the diagram
     		Ignoring the faint node labeled $W$ for now, the diagram
                on the left 
...
%at the reader.  
%oli1: From the modeler's perspective, you have to know for sure what
%the inputs are to your conditional BN, which is intended to present
%the inputs as an abstraction, but here it's a liability: what are the
%observed nodes if you remove and add some edges? graph operations
%will change the type of the object you're working with, which
%illustrates the lack of modularity. It turns out that this type-level
%tracking, which I imagine make implementations and formal
%manipulations of graph operations trickier, is unnecessary.
...
%Idon't understand in what sense a BN ``is intended to present the
%inputs as an abstraction''.  I don't even know what it means to
%present inputs as an abstraction.                  
% I assume the objectionable part of this is the word `type level'? 
...
%oli1: using the word astrobiologist multiple times is space
%consuming. by using 1st person plural, it's shorter because we can
%unambiguously use pronouns, and avoid gender. Is using 1st person
%plural (or 2nd person singular) like I did in example 1, taboo? 
...
%(which strikes me as exceeingly strange).  
		Now suppose that the astrobiologist is reminded by a
                biologist that life 
		...
		$L$ (as clearly seems appropriate, given that life
                depends on water), we
%joe2                
...
%oli1: ah, you're right. I will fix---but I do think the rewrite made
%the problem worse, as before the diagram above was clearly a
%conditional BN, and we haven't shown what the PDG looks like yet (and
%there's no precedent to think they're the same because the one in
%example 1 looks very different from the BN counterpart). 
...
Consider the classic example used to introduce Bayesian nets, with
four binary variables indicating whether a person ($C$) develops
cancer, ($S$) smokes, ($SH$) is exposed to second hand smoke, and
($PS$) has parents who smoke, presented graphically in
...
%joe2: cut, and rewrote                
%		The BN is a compact representation of a joint
%distribution over all four variables, which achieves compactness by
%taking advantage of independence between variables. It encodes an
%assumption that every node is independent of its non-descendants
%given its parents. 
%		Most of the time, we do not make the independence
%assumption because we know for certain that the variables are
%independent; rather, we just suspect that the identified links are by
%much more important than the others. Determining for sure that
%smoking  and second hand smoke are independent, controlling for
%parents' smoking habits, would extremely difficult, and would require
%empiricism to validate.
According to the semantics of this BN, $S$ and $C$ are independent,
conditional on $PS$. While this seems plausible, checking that this is
in fact the case would be nontrivial. 
The \MN, on the other hand, represents merely the set of constraints
on marginals given in the tables.
%joe2: slw=owing down to tell the story
%Depending on our chosen semantics
%(we offer several in section \ref{sec:semantics}), we can further
%interpret the constraints. For instance, by looking at the maximum
%entropy distribution consistent with the constraints, we get back the
%independence assumptions from the BN, and can thereby view
%\Cref{subfig:smoking-pdg} as representing the same distribution as the
%BN.
One obvious semantics for ths graph viewed as a PDG is to consider all
distributions with these conditional probabilities.  While the
distribution that makes $S$ and $C$ independent conditional on $PS$ is
one of these distributions, there are many others.  However, as we
show (Theorem~\ref{??}), if we consider the distribution that
maximizes entropy among those consistent with the PDG, we recover the
distribution encoded by the graph viewed as a BN.  Thus, in a sense,
we can enforce the independence assumptions while still allowing for
the possibility of non-independence, if further evidence comes in to
suggest that it does not hold.
...
%joe2: I have no idea what this sentence is saying.  I object to
%adding sentences I don't understand
%\begin{vfull}
%			On might worry that we've merely constructed
%                        some object we have no computational access
%                        to, but note that in this case, sampling is no
%                        harder in the \MN, We discuss this problem in
...
Suppose that you acquire additional information: you read a 
thorough empirical study 
demonstrating that people who use tanning beds
		...
		The \MN\, once again has no trouble: we can simply
%joe2: You can't talk about merging graphs when you haven't given the
%graphs that are supposed to be merged!
%                merge the graphs, as in \Cref{subfig:smoking-pdg}.
add a node $T$ (for tanning) with an edge pointing to $C$, and associate
the approrpiate cpt with the edge, as in \Cref{subfig:smoking-pdg. 
  %joe2: added
  (Again, we have a node labeled $\mathsf 1$, which plays the same role
  as in Figure~\ref{fig:gun-floomp-diagram}.)
  %		\begin{center}
%			\scalebox{0.9}{
%			\begin{tikzpicture}
%			\node[dpadded] (1) at (0,0) {$\mathsf 1$};
...
%			\end{tikzpicture}}
  %		\end{center}
  %joe2
%		As before, simply adding $T$ to the parents of $C$
%  makes it impossible to use the old table, and require
  As before, in a BN, simply adding $T$ as a parent of $C$ would 
  make it impossible to use the old table, and would require
                us to guess at the interactions between tanning beds
                and smoking. 
...
sections, we formalize \MNs and relate them to other approches.
...
%joe2: I cut this.  What is the point of the example?  Why is it here?
        %What does it tell me that other examples haven't already told me.
        \commentout{
	We have seen some examples of \MNs; we now provide the formal definitions.
%	We have seen some examples of \MNs, but elided the details; we now begin a more careful treatment of \MNs\ as mathematical objects. 
%	\moveme{Rather than representing a probability distribution, \MNs\ can be thought of as \emph{constraints} on distributions.}
	Compared to a Bayesian Network, a \MN\ still consists of a
        directed graph, and the edges are associated  with cpts.
        probability tables, but now each edge is interpreted
        individually. Consider the graph 
	...
	which would be interpreted as three tables $\Pr(C\mid A, B), \Pr(A), \Pr(B)$ in a BN. Interpreting it as a \MNs, there are no distributions on $A$ or $B$, and the arrows into $C$ would be split into two separate tables $\Pr(C \mid A)$ and $\Pr(B \mid A)$, rather than a joint one. 	
        ...
        %joe2: reverted back to what I had before
We have seen some examples of \MNs; we now provide the formal
definitions.
	...
	\begin{defn}[\MN]\label{def:model}
		...
		%oli1: the union doesn't work out if they're not
		%labeled. There are definitely examples where you have
		%parallel edges (two candidate probabilities b/c
		%you've deleted a node and haven't resolved it, or two
		%conflicting observations...). Having parallel edges
		%will make the formalism cleaner as well, because we
		          %will definitely have parallel paths.
%joe2: I don't want union in any case, so this is not loss.  If you
%think parallel edges make things cleaner, give me an
%example.			\item $\Ed \notation{\subseteq \N
%\times \N \times \mathit{Label}}$~~is a set of directed edges, each
%of which has a source and target in $\N$\notation{, and
                          %$\mathit{Label}$ is a set of possible labels}.
%joe2: CUT! graphs don't have ``parallel edges''  Multigraphs (which
%we are not considering) do except that they're not called parallel edges.				\footnote{Though likely to be
...
%oli: I did introduce them, just below---but I have now removed them.
%joe2: I would still prefer a symbol other than \mu ere, but since
%you're using a macro, we can worry about that later
	                \item $\bmu\notation{\colon \big(\!({A,B,\ell})\colon \! \Ed \big) \to \V(A) \to \Delta\V(B)}$
			% HYPERGRAPH \bmu TYPE: $\colon\!\big(\!({\bf
                        % A,B})\colon \! \Ed \big) \to
                        % \prod\limits_{A\in \bf A} \!\! \V(A) \to
...
%	  associates, for each edge $(X,Y, \ell) \in
	  associates with each edge $(X,Y) \in
...
% You've addressed my more general concerns in the email.
	...
	%oli1: here's where I defined it but it's removed now.
...
%	we will often use two additional bits of syntax to represent
	we will often use two additional syntactic conventions to represent
        common constraints:  
	\begin{itemize}
		\item A special variable $\sf 1$ that always takes its
                  unique value $\star$. It is used to represent
...
%                indicating the degenerate special case of a cpt
	      \item Double-headed edges $A \doubleheadrightarrow B$
                indicate are associated with the degenerate special
                case of a cpt 
                  corresponding to a deterministic function $f : A \to
%joe2: no more \none
%                  B$, the distribution assigning a point mass to
                  %                  $f(a)\neq\none$ for each $a \in A$.
       B$, the distribution assigning a point mass to 
                  $f(a)$ for each $a \in A$. 
	%oli1: this second one in particular is useful. People don't
        %usually do this visually in a BN, even if it's the standard
        %construction and it might help people see how we're going to
        %use these. 
...
%	While it is possible to employ 	
%	the answer that this notation is roughly shorthand for a new
%        node, at the junction, with projections. For instance, the
...
\commentout{In the factor graph literature, especially with
          regard to loopy belief propagation
          \parencite{wainwright2008graphical}, it is common to call a
          collection of marginals that are not necessarily all
          compatible with a distribution \emph{pseudomarginals},
          making a \MN\ in some sense a collection of `conditional'
          pseudomarginals. This gives an alternate, more technically
          precise expansion of \MN\ as ``Pseudomarginal Dependency
          Graph''.} 
...
%oli1: sorry, should be way clearer now.
...
% I just don't think there's enough information there to be careful with labels without it.
	In each of the examples in the introduction, something has happened causing us to combine pieces of diagrams, or make additions. The formal underpinning of this process is a \emph{union} of \modelnames.
	...
	\begin{example}[continues=ex:guns-and-floomps]
		In reference to \Cref{fig:gun-floomp-diagram}, let $\sfM$ be the initial knowledge \MN, represented by variables $\mathsf 1, F, G$ and the edges $1\to F,1\to G$, all in black. Let $\mathsf E$ be the \MN\ consisting of the two variables $F$ and $G$, a edge $e = (F, G,)$, interpreted by $\bmu_{e} = T$ where $T$ is the cpt as before. The entire (inconsistent) diagram with three edges is then
		$ \sfM \cup \mathsf E$.
	\end{example}
	...
	%oli1: The below is better written, but it does not mention the point that I was trying to make: that 
	% I also am
	There is more than one way of giving semantics to a \MN.  We discuss three approaches below.  The first is perhaps the most natural: associating with a \MN\ a set of distributions. This set will be empty if the \MN\ is inconsistent. 
	The second approach also associates with a \MN\ a set of distributions, but associates with each one a \emph{weight} (cf.~\parencite{halpern2015weighted}), which intuitively can be thought of as a measure of the ``goodness'' of that distribution.  For an inconsistent \MN, some distributions may still be better than others. 
	...
	Our third approach chooses a particular distribution from the
        weighted set of distributions defined by the second approach,
%joe2
%        this associating with a PDG a unique distribution. As we shall
        thus associating with a PDG a unique distribution. As we shall
        show, it is this third approach that allows us to recover BNs
        from PDGs. 
	...
	\todo{}
	\subsection{\MNs\ as sets of distributions}\label{sec:set-of-distribution-semantics}
	If we think of a \MN\ as kind of like a BN except without independence assumptions, then just as a BN represents a single distribution on joint settings of its variables, a \MN\ might be thought of as representing the set of all distributions that are compatible with it exactly, with or without the independence assumptions.

	\begin{defn} \label{def:set-semantics}
		If $\sfM\sheq\mnvars[]$ is a \MN, let $\llbracket \sfM \rrbracket_\Set$ be the set of distributions over the variables in $\sfM$ consistent with $\boldsymbol\mu$ on every edge. Formally,		
	...
	For strict \MNs, \Cref{def:set-semantics} precisely defines
        the set of distributions whose marginals exactly match; in
        general, we only require that $\mu(b \mid A)$ exceed $\bmu(b
        \mid a)$ --- intuitively, if an agent can only set upper and
        lower bounds, and $\mu$ is between them, then $\mu$ is
	...
	\begin{defn} \label{def:weak-consistent}
		$\sfM$ is consistent if there is some $\mu \in \bbr{\sfM}_\Set$, and strictly consistent if there is some $\mu \in \bbr{\sfM}_\Set$ that assigns no mass to any world where a variable takes $\none$.
	\end{defn}
	
	\note{By contrast, $\sfM$ is over-constrained if there were \emph{some $\bmu$} of the same type as $\bmu^\sfM$ such that $\bbr{\N^\sfM, \Ed^\sfM, \V^\sfM, \bmu}_\Set = \emptyset$, and under-constrained if every $\bmu$ of this type yields $\big| \bbr{\N^\sfM, \Ed^\sfM, \V^\sfM, \bmu}_\Set \big| > 1$.
	}
...
%need union at all.  This is a distraction
	...
	From the two special cases of $\dcap$ discussed above, one can see that adding new edges, (which we will see correspond to observations in \cref{sec:belief-update}), cuts down the set of possible distributions, just like conditioning, and adding new variables to a consistent model freely increases the number of valid distributions like one would expect. Note that all of this is done by combining \MNs, which is only possible because of their extreme modularity.
...
%should cut Theorem 3.6
	\subsection{\MNs\ as distributions}
...
%Our first appraoch treats all inconsistent \MNs\ as equivalent,  which is unsatisfying.
	The semantics in \cref{def:set-semantics} inconsistent \MNs\ are equivalent, which is unsatisfying. For
	instance, there's a difference between thinking a 60\% head-biased coin will land heads 63\% of the time, as opposed to 0\% of the time. %and estimates of distributions from samples are necessarily going to be a little off. 	
...
%prefer to use \mu or \Pr for for probabilities, not \nu        
	...
	%oli1: 
 	\todo{rewrite}
 	We only want to score distributions based on the edges $\Ed$ that an agent actually has expressed, and we will measure similarity by relative entropy: the number of bits required to encode the $\bmu^\sfM$ given an optimal code for $\nu$.  Perhaps one might think that this seems backwards: we're giving a \todo{}
 		
	\begin{defn}\label{def:zeta-score}
		The (strict) inconsistency, $\zeta$, of a \MN\ $\sfM = \mnvars[]$ with respect to a distribution $p \in \Delta[\V_{\none}(\sfM)]$ (or $\Delta \V(\sfM)$ if strict) is given by
		\[
...
%far afield ...	
	...
	\begin{defn}
		...
		$$\bigcap_{p \in P}\ \bbr*{\vphantom{\Big|}p^0\circ \cdots\circ p^k}_\Set \neq \varnothing$$
	\end{defn}
...
%	\vspace{0.5em}
	...
	\[ \bbr{\sfM}\MaxEnt := \Big\llbracket M \Big\rrbracket_\Set^{-H(\cdot)} \]
...
%		\end{tikzpicture}}
	...
	\thmsetconvex*
	\begin{proof}
		Choose any two distributions $p, q \in \bbr{M}_\Set$ consistent with $M$, any mixture coefficient $\alpha \in [0,1]$, and any edge $(A,B) \in \Ed$.
