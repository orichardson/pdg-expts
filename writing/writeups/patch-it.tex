...
%	\moveme{Not exactly related, but I've been meaning to point out for a while that this expansion can be viewed as the reason d-separation has a special case for paths with ``colliders'': $A \times B$ is specifically constructed to be the unique parent that never shares information between $A$ and $B$}
	...
	\textbf{My Thoughts.}
	This is pretty clearly enough to get around the problem in section 0. However, it is still a stronger extra assumption than required: all of the cases in \ref{sec:cases-works} are covered by maximum entropy alone, without this extra assumption that brings in a lot of causal information.

	\subsubsection\
	There is also a related  maximum-entropy formulation of factor graphs (which in particular includes BNs), that says that the distribution that maximizes entropy given mean paramters is exactly the one represented by the factorization.  It's not clear exactly how the mean paramters work out for BNs, and I still can't find any existing work that does this with the conditional distributions specifically in mind.
	
	\subsubsection\
	Dawn Holmes%
		\footnote{\url{http://holmes.faculty.pstat.ucsb.edu/RESEARCH.htm}}
	in particular seems to have done a lot of stuff along these lines.%
		\footnote{\url{https://aip.scitation.org/doi/pdf/10.1063/1.1835247}}%
		\footnote{\url{https://aip.scitation.org/doi/pdf/10.1063/1.1381865}}
	``The approach is to show that the maximum entropy solution that satisfies only the causal constraints, possesses the independence relationships embodied in the causal network''
