...
% - and their inconsistency (which is superlinear / concave)
    ...
    \begin{defn}[Markov Kernels]
        If $\mathcal X  = (X, \mathcal F)$, and $\mathcal Y = (Y, \mathcal G)$ are two measurable spaces, a Markov Kernel $\kappa : \mathcal X \to \mathcal Y$, often written as ``$\kappa(Y|X)$'', is a function $\kappa : X \times \mathcal G \to \mathbb R$, such that
        \begin{enumerate}
    ...
    \begin{defn}[category of measurable spaces with base measure]
        ...
        \end{align*}
            ...
            %     & \Big\}    
            % \end{align*}
        and the morphisms $\BaseMeas((\mathcal X, \lambda), (\mathcal Y, \mu))$ from $(\mathcal X, \lambda)$ to $(\mathcal X, \mu)$
    \end{defn}
    ...
    \begin{defn}
        An \emph{unweighted PDG} over a structure $(\N, \Ar, \src,\tgt, \balpha)$ is a functor $\mathrm{Paths}(\N,\Ar)$
        \[ 
            {\dg M} = (\V, \mat p)  : \mathrm{Free}(\N, \Ar) \to  \BaseMeas
        \] 
    \end{defn}    
