...
% - and their inconsistency (which is superlinear / concave)
    ...
    \begin{prop} 
    There is a natural isomorphism between hypergraphs and bipartite graphs:
    % \def\bitgr{\mathit{BG}}
    ...
    \begin{defn}
        ...
        source variables and a target $T_a \subset N$ of target variables.        
    \end{defn}
    
    Equivalently, a directed hypergraph $(N, \mathcal A)$ is a regular directed graph 
    $(2^N, \Ar)$ whose set of vertices is a powerset. 
    ...
    \begin{defn}
        ...
        \begin{enumerate}
            ...
            \item If $\mathcal T$ is a topology on $X$, and 
                $\lambda(U) > 0$ for every non-empty open set $U \in \mathcal F \cap \mathcal T$, then $\lambda$ is \emph{strictly positive} (wrt $\cal T$).
    ...
    \section{PDGs}
    \begin{defn}
        A \emph{dependency structure} is a directed hypergraph
    ...
    \begin{defn}
        A \emph{weighted dependency graph structure} $(\N, \Ar, \balpha, \mathcal F_{\!\Ar})$ is a depencency structure $(\N, \Ar)$ together with a $\sigma$-algebra $\mathcal F_{\!\Ar}$ on $\Ar$, and a (signed) measure $\alpha$ on $\Ar$.
