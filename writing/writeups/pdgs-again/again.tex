...
%
    ...
    \begin{defn}
        A \emph{PDG structure}, or qualitative PDG, is a weighted directed graph $(\N, \A, \src, \tgt, \balpha)$, where $\N$ is the set of nodes, $\A$ is a set of arrows, the maps  $\src, \tgt: \A \to \N$ give the respective sources and targets of each arrow,  and $\balpha : \A \to \mathbb R$ is a real-valued weight for each arrow.
    \end{defn}
    ...
    \begin{defn}
        A PDG over a structure $(\N, \A, \src,\tgt, \balpha)$ is a functor $\mathrm{Paths}(\N,\A)$
    \end{defn}    
