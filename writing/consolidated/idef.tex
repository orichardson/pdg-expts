...
%   \subsection{}
    ...
    In a sense, probabilistic is uncertain only about an outcome.
	% You see,
	By definition, there is some (known) set $\Omega$ of possible outcomes, and uncertainty takes the form of a probability distribution $\mu : \Delta\Omega$.
	One may then consider \emph{random variables} present, but in this telling, these are (deterministic) functions of an outcome $\omega \in \Omega$. 
	 % to some set of possible values (which we denote by $\V(X)$, for a variable $X$).
	A given probability $\mu$ may result in lots of uncertainty in some variable $X$ and none at at all for another variable $Y$ while another distribution on $\Omega$ does the opposite,
    but at the end of the day, uncertainty is about $\Omega$, and it is merely filtered through the variables.

    While we share this , the characterization of $\Omega$ as prior to variables may be misleading; it is common to define $\Omega$ ``at the last minute'', as the set of all realizable joint settings of the relevant variables, which can only be done after the rest of the modeling.
