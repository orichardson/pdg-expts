...
Since each matrix $\bp[X]$ is positive, so is the total transition matrix $\mat P$, obtained by sequentially applying each $p_X$ in order. As a result, the Markov chain $\mat X^{(0)} \to \mat X^{(1)} \to \ldots$, whose transition matrix is $\mat P$, is ergodic,
...
The probability distribution $\Pr^\prec_{\mathcal D}$ specified by the dependency network $\mathcal D$ is defined to be this stationary distribution.
\end{defn}
