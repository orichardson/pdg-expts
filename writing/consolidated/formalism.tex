...
\begin{document}
	...
	\begin{defn}[PDG]\label{def:sPDG}
		A PDG is a tuple $\pdgvars[]$ where
...
\end{constr}
	...
	\begin{defn}[set-of-distribution semantics] \label{def:set-semantics} 
		...
		$\dg M$ is \emph{consistent} if $\SD{\dg M}$ is inhabited (non-empty), and \emph{inconsistent} if $\SD{\dg M}$ is empty.
	\end{defn}
