...
%oli12: Do you mean the way I use it, the way it looks in my pdf, or difficulty in editing / compiling?
...
%oli12: Introduce our protagonist earlier. Changes to pronouns have been made throughout example.
...
%oli12*: it seems like we should add a sentence fragment here, along the lines of "but we will be able to easily recover them".  Also, the above is kind of redundant, so I keep looking at it trying to figure out how to re-word, but it's so well written that I can't figure out what I want to do to it.
...
%    , and show that, among all distributions consistent with $\PDGof{{\mathcal B}}$,
...
%oli12: I want to keep some of the material  underneath it for the full paper though.
...
%oli12*: Are you serious that you don't see the connection?? We should talk about this.
% I really do think it's a massive oversight not to at least mention this. I can see a world where mentioning this is the most important thing we do in the paper. I also think you hugely underestimate how many people care about commutative diagrams, what this buys you, and the implications for mathematical reasonsing.
\begin{vfull}
...
%oli12: I dislike the sentence below. Primarily, it seems like cheating to come up
% with totally different semantics depending on what you want to capture, and 
% we're really not doing that --- our three semantics are intimately related, and
...
%oli12: while I still prefer and would prefer to promote 'inhabited' (no negatives), I think we can agree that a single negative is easier to follow single negative than tripple negative.
...
%oli12: You seem insistant on this particular heading; but here's why it bugs me:
...
%oli12: we've used this transition several times now. I think it flows fine without it. Removing
...
%oli12: Do you still think (as below) that referencing an example is appropriate?
...
% I do not care at all what markers are used (though squares mean QED to me, and hence seem weird for examples / definitions) --- you can change both markers to whatever you want by searching for "\declaretheorem" and then changint the "qed=" field. 
\end{defn}
...
%would change this calculus at all (except to make it so that you
                %don't care).
%joe11I I dsagree; you're too wedded to maxent.  But this is not the
%time to have this battle                
% there's 
and no edges encodes awareness about $X$ but no beliefs about it. Given that $X$
could be distributed in any way (even chosen adversarially), it is safest to act
as though you believed $X$ to be uniform.  
%oli11: irrelevant because commented out anyway, but I think what I wrote is more coherent than the below.  and no edges leading to $X$ means that the agent is  aware of $X$, but has no  information regarding the probability of various values of $X$ but no
...
% The former is correct, because Entropy measures uncertainty, not dependence.
...
% Balance at least indicates that it could be negative, and Graph information I suspect already has a meaning elsewhere. We also want to emphasize its polarity (positive is bad), just like we did with inconsistency. I therefore propose going back to
...
%% between ${\dg M}$ and a distribution $\mu$, denoted $\IBal{\dg M}(\mu)$,
...
%you wrote didn't really help my intuition at all (besides being too long)
...
% (i.e., $\SD{\dg M} \ne \emptyset$), 
%oli12: This is in the full paper, where this notation actually is defined. Reverting.
($i.e., \Inc(\dg   M) = 0$)
...
%oli12: Low priority. The issue is that our consturction must assign \beta to the projection edges, and choosing one, though unimportant for semantics, may be relevant.  
%joe10*: yuck!        
...
%joe10
          %          (specifying a distribution $\Pr_{\cal B}$), then
...
%    for all $\gamma > 0$ and all vectors $\beta$ over the nodes of $\cal B$, 
    for all $\gamma > 0$ and all vectors $\beta$,
                    %joe10: far too complex
%	$\bbr{\PDGof{\mathcal B, \beta}_\gamma^*}$
    %%        contains a unique   element, equal to $\bbr{\PDGof{\mathcal
    $\bbr{\PDGof{\mathcal B, \beta}}_\gamma^* = \{Pr_{\cal B}\}$. 
        %        B, \beta }}^* =
%        	$\bbr{\PDGof{\mathcal B, \beta}_\gamma^*}
%         = \{\bbr{\PDGof{\mathcal B, \beta }}^* =
%        \Pr_{\cal B}$. 
...
%oli12: He did, but this is confusing because my bib files are poorly named. I'm citing some notes on free energy in factor graphs (including the bethe free energy, and the pdf file is called bethe.pdf). This is is the source that has the notation I use. 
...
%oli12: does something strange happen when you try to TeX it without braces? These two are equivalent for me and I don't know why you bother to change it.
...
%oli12: folding in the next paragraph: 
...
\Cref{eq:semantics-breakdown} also makes it clear that fixing $\gamma$ on the same scale as $\beta$ is
...
%oli12: we have now explained the point. I'm inserting a part of this again.
...
%joe10: I don't understand this
  ...
  %  constraints,
%oli12: this was clarified in the semantics introduction now. edited and reinstated.
...
%joe7: I already saved your bib files as oliver1, 2, 3,
                ...
                %joe1:send me allrefs again and I'll revert back
...
Thus, $\mu^* \in \bbr{\dg M}_0^*$, as desired.
    \commentout{
    		...
    		% \intertext{\centering Adding $\Inc_{\dg M}( \mu)$ to each quantity}
                   % \implies
    			&\forall\gamma,\mu.~&
    			\Inc_{\dg M}( \mu) + \gamma\lb &~\leq~& \Inc_{\dg M}( \mu) +& \gamma\IBal{\dg M}(\mu)  &~\leq~&  \Inc_{\dg M}( \mu) + \gamma\ub \\
    			&\forall\gamma,\mu.~&
...
In this section, we prove Theorem~\ref{thm:bns-are-pdgs}.  We start by
formalizing the transformation of a BN to a PDG.
...
%		We now put the following facts in equational form for
		...
		\begin{description}
			...
			%
		      ...
		      \item[Fact 2]
%joe10: this is way too wordy, and technically correct on a number of
%minor grounds.  The cpds that you say don't have entroyp certainly
%have entropy; their entropy is 9.
%(Only $\cal B$'s cpds have entropy).
