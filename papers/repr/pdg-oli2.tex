...
\newcommand\extrsymb{\mathit{IDist}}
...
\newcommand\extrainfo[2][\dg M]{\extrsymb(#1,#2)}
...
%        is the same as the one specified by $\cal B$. 
        among all distributions consistent with $\PDGof(\mathcal B)$, the
unique distribution with a minimun score is the one specified by
${\mathcal B}$ (\Cref{thm:bns-are-pdgs}).  This allows us to recover the semantics of Bayesian
...
%joe7
%                          subjective confidence in the cpd $\bp$. 
...
%joe7: Again, I don't know why cyou keep using nonstandard fonts.
%Can't swe just use calligraphic letters for PDGs?        
...
marginal of $\mu$ on $X$). We then sum over all the edges $L$ in the PDG,
...
%oli9: we mention "inconsistent PDGs" a lot in the intro, and this definition may clarify what we mean. The inconsistency is also related to the normalization constant of a factor graph, which we would ideally explain if we had room. 
...
%\inco (M; \mu) . 
		\[ \inco(\dg M) = \inf_{\mu \in \Delta [\V(\dg M)]}
                \inco (\dg M; \mu) . \]
...
%to measure it.
%oli9*: I don't like my wording before, but I strongly dislike this wording. I was trying to hide the fact that we're implicitly specifying an independece structure: to talk about this properly we need \alpha, and even then it seems like we're selling ourselves short by starting in the middle of the argument. We make a big deal out of not having to keep independencs around earlier, and I think that story is a better one. 
%oli9*: I also think this is even less precise. There is literature about the ``most natural'' distribution in the maimum entropy literature, but there is no definition of an independence structure specified by a PDG. We also cannot define it properly without \alpha.
%
...
%joe7*: I don't find it all that ``compelling''; I agree that there's
%a precedent.  But more importantly, this is not what we're doing.
%oli9*: We can remove ``compelling'', but this is totally what we're doing with Extra. I think that's one of the strongest features of our story.
...
%joe7**: I was doing fine until I got to thie paragraph, at which
%point I point I fell off a cliff.   I couldn't make sense of the
%story, so I rewrote it completely.  Before you do more rewriting,
%let's discuss it.   I think that the
%next paragraph tells the wrong story (in part, because I can't really
%make sense of it).  We are not trying to maximize entropy in this
%setting, as near as I can tell, and I don't think it helps to bring
%in counterfactuals.  I  took a pass at rewriting it. 
\commentout{                
...
%oli8: rewritten
		To maximize entropy in this setting, we need to
                control for the counter-factual nature of this
                constraint. 
...
correctly represented by the PDG ${\dg M}$, then $\extrsymb'(\dg M;
\mu) = 0$, where
...
%          \extrsymb'(\dg M; \mu) := \sum\alle \H_\mu(Y\mid X) - \H_\mu.
    \extrsymb(\dg M; \mu) := \sum_{\ed{L}XY \in \Ed^{\dg M}} \H_\mu(Y\mid X) - \H(\mu).
	  \label{eqn:alt-extra}
	\end{equation}
In general, we can view $\extrsymb'(\dg M; \mu)$ as a measure of how
...
%%oli8:
%	 $\inco$ is the cost of not including the specified features
%of a distribution, and $\extrsymb$ is the cost of including
%unspecified ones. 
...
a distribution $\mu$. The first, $\inco({\dg M}, \mu)$ measures how
close $\mu$ is to being consistent with ${\dg M}$, and is 0 if $\mu$
...
%joe7**: why do we focus on this case?  We need to motivate this.
        ...
        where $\gamma$ is small, making $\inco$ the more important
        term. 
...
\begin{prop}\label{prop:convex-if-gamma-small}
	If $\dg M$ is a PDG and $\beta_0$ is a constant less than any $\beta_L \in \beta^{\dg M}$, then for any $\gamma < \beta_0$, $\bbr{\dg M}_\gamma$ is a strictly convex function of $\mu$.%
...
%        quantitative data above the independence assumptions.  
        but the third semantics ($\bbr{\dg M}_*$) can be
...
%joe7: added
We start by generalization the construction of
...
%joe7
%                with $X_i$ in $\cal B$, and for the projections, we
                with $X_i$ in $\cal B$;  for each from $\Pa(X_i)$ to
                $X_j$ for $X_j \in \Pa(X_i)$, we
                set  
...
%	  If $\cal B$ is a Bayesian network, then there is a
	  If $\cal B$ is a Bayesian network, then the 
%oli9
          % unique probability distribution $\mu^* \in
          unique probability distribution $\mu_* \in
...
%joe7: Nevertheless, we should use M_{\cal B}.  \Gamma(B) does not
%look like a PDG.  You have to use notation that involves an \M, so it
%looks like a PDG.  The B doesn't have to be a subscript.
%                \SD{\PDGof(\mathcal B)}$ consistent with the PDG
%oli9: a distribution doesn't represent a BN, because the BN has extra stuff like directions of arrows.
		% \bbr{\PDGof(\mathcal B)}_*$ is the one that represents $\mathcal B$.
...
\begin{defn}
	A \emph{factor graph} is a collection of random variables $\mathcal X = \{X_i\}$ and a collection of \emph{factors} $\{\phi_J\colon \V(X_J) \to \mathbb R_{\geq0}\}_{J \in \mathcal J }$, where each $J \in \mathcal J$ is associated to a subset of $X_J \subseteq \mathcal X$. 
	Together with a vector of weights $\{ \theta_J \in \mathbb R \}_J$ (or otherwise assuming each $\theta_J = 1$),
	it defines a joint distribution on $\V(\mathcal X)$  by 
	\[ \Pr_\Phi(\vec x) 
		= \frac{1}{Z_\Phi} \prod_{J \in \cal J}
								\phi_J(\vec x_J)^{\theta_J}
		\quad\parbox{0.9in}{\centering minimizing its \\ ~~\emph{free energy},  }\quad \mathcal G_\Phi(\mu) := \!\E_{\vec x\sim\mu}\left[  \sum_{J \in \cal J} \theta_J \log\frac1{\phi_J(\vec x_J)}\right] - \H(\mu)  ,
	\]
	where $\vec{x}$ is a joint setting on all of the variables, $\vec{x}_J$ is the restriction of $\vec{x}$ to only the variables $X_J$, and $Z_\Phi$ is the constant required to normalize the distribution. 
\end{defn}
The global semantics of a BN make it the product of its cpts. We extend this to arbitrary PDGs.
...
% variables, but it looks strange.
		variables $(\N,\V)$ by 
	$\Phi(\dg M) := \smash{ \{ x,y \mapsto \bp(y \mid x)\}_{\ed LXY \in \Ed}}$ and weights $\theta_L := \beta_L$.
\end{defn}
The translation in \Cref{def:PDG2fg} clearly destroys the direction of the arrows, which makes the structure impossible to recover. Instead, we can add a new edge, with a completely different asserting a joint probability. 

\begin{defn}[factor graph to PDG] \label{def:fg2PDG}
	If $\Phi=(\{\phi_J\}_{J \in \cal J})$ is a factor graph, then let $\PDGof(\Phi)$ be the PDG generated by inserting joint variable node $X_J := \prod_{j \in J} X_j$ for every factor node $J \in \mathcal J$, together with projection edges $X_J \tto X_j$ for each $X_j \in X_J$ (as done in \Cref{def:bn2PDG}), and an edge $\sf 1 \to X_J$ whose associated cpd $\bp[J]$ is the joint distribution on $X_J$ obtained by normalizing $\phi_J$; let $\beta_L := \theta_L$.%
\end{defn}

Surprisingly, despite garbling the structure (see \Cref{fig:fg2PDG,fig:fg-intro-examples}), when we fix $\gamma=1$, the two operations preserve most of their semantics.
...
\begin{theorem}\label{thm:pdg-is-fg}
	If $\dg M$ is a PDG with every $\beta_L = 1$, then
	% for any joint distribution $\mu$ on $\V(\mathcal X)$, we have
 	$\mathcal G_{\Phi(\dg M)} = \bbr{\dg M}_{1}$.
 	% $\kldiv{\mu}{\Pr_{\Phi(\dg M)}} = \bbr{\dg M}_{1}(\mu)$
	% In particular, $\Pr_{\Phi(\dg M)} = \bbr{\dg M}_*^{\gamma := 1}$
\end{theorem}
\begin{theorem}\label{thm:fg-is-pdg}
	If $\Phi$ is a factor graph on $\mathcal X$, then
 	% for any joint distribution $\mu$ on $\V(\mathcal X)$, we have $\kldiv{\mu}{\Pr_\Phi} = \bbr{\PDGof{(\Phi)}}_{1}(\mu)$
	$\mathcal G_\Phi = \bbr{\PDGof{(\Phi)}}_{1}$.
\end{theorem}
It immediately follows that in both cases, that the distribution $\mu_*$ that minimizes the functions is the same; for the factor graph $\Phi$, $\mu_*$ is the distribution $\Pr_\Phi$, while for $\dg M$, we get $\mu_* = \bbr{\dg M}_*^{\gamma=1}$, where we have fixed $\gamma$ to 1 rather than taking the limit in \eqref{eq:uniqdist}. 

Therefore, when $\gamma = 1$, a PDG with each $\beta_L = 1$ is equivalent to the associated factor graph with $\theta = 1$. Not only do they determine the same distribution, but they are are also equivalent in the stronger sense of sharing a cannonical scoring function. We explore the parallel with free energies further in \Cref{sec:thermo}.
...
%	By using the alternate formulation $\extrsymb'$ of the extra
%        information \eqref{eqn:alt-extra},
\Cref{thm:pdg-is-fg,thm:fg-is-pdg} can be almost seen directly by rewriting the scoring semantics in \eqref{eqn:full-score} as follows, and comparing the defintion of $\mathcal G_\Phi$.
...
Comparing \eqref{eq:semantics-breakdown} shows us that the qualitative correspondence only occurs if the middle term is zero, which can only happen when every $\beta_L=\gamma$. This view also clarifies additional properties.

In particular, although both are measures of confidence, the way that a factor graph varies with $\theta$ 
	% \footnote{\cite{wainwright2008graphical}}
is quite different from the way a PDG varries with $\beta$. Increasing $\theta$ makes the resulting distribution more deterministic, while increasing $\beta$ makes an agent more certain that the cpt is the way it is, as in \Cref{ex:overdet}.

\begin{example}\label{ex:overdet}
	Consider the PDG $\dg M$ containing just $X$ and $1$, and two edges $p, q: 1 \to X$. suppose $p$ and $q$ are both attached to the same $p(x) = q(x)$ which places 0.7 probability on $x_1$ and 0.3 on $x_2$. Now $\UD{\dg M} = p(x) = q(x)$, while $\UD{\dg M}^{\gamma=1} = \Pr_{\Phi(\dg M)}$ places probability 0.85 on the event $X\!=\! x_1$ and $0.15$ on $X\!=\! x_2$. 
\end{example}

This is undesirable behavior for PDGs as we have described them; we point out further issues in \Cref{sec:fg-issues}. Taking a limit as $\gamma$ goes to zero fixes some of these issues, intuitively by ensuring that there is local regularization, so that the resulting distribution is consistent with the semantics, by \Cref{prop:consist}.
