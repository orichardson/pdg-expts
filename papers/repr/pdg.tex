...
%	We give the definition below for the finite case.  
	...
	\begin{defn}\label{def:model}
		A \textit{\modelname} (s\MN) is a \modelnamehyper\ where every link has just one source and target node.
	\end{defn}
...
%		\todo{define ``equivalence'' and prove.}
	\end{theorem}
	\begin{proof}
		This is done to introduce a new node whose domain corresponding to every set of variables that participates in a link. For instance, the \MN\ in \cref{ex:planet} is translated as follows:
		\begin{center}
			\scalebox{0.8}{
			\begin{tikzpicture}
				\node[dpadded] (L) at (1.3,0) {$L$};
