...
%	\moveme{Rather than representing a probability distribution, \MNs\ can be thought of as \emph{constraints} on distributions.}
	...
	\begin{defn}[\MN]\label{def:model}
		...
		\begin{itemize}[nosep]
			\item $\N$~~is a finite collection of nodes, or variables
			\item $\Li \subseteq \N \times \N \times \mathrm{Label}$~~is a set of directed links, each of which has a source and target in $\N$.%
			...
			\item $\bmu\colon \big(\!({A,B,\ell})\colon \! \Li \big) \to \V(A) \to \underline\Delta\V(B)$
			...
			gives a (sub)-distribution over $\V_B$ given each possible value of $\V_A$ and for every link $({A,B,\ell}) \in \mathcal L$.
...
%	\end{defn}
	...
	\begin{itemize}
		\item A special unary variable $\sf 1$ that always takes its unique value $\star$. It is used to represent unconditional distributions, as in \cref{ex:guns-and-floomps,ex:smoking}. For strict \MNs, this is no different from any other variable that can take only a single value. 
