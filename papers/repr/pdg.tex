...
% with exactly 2 overlapping variables, this amounts to a statement of independence. 
...
% definition"---but you're probably right to state this explicitly. I'm reinstating
% your version.
...
%oli17*: No, this is much weaker than what I want to say. These terms are 
% necessary to have an optimal distribution $\mu$ that is not a point mass
% on a particular world. 
%oli17: Unsure if helpful, but the thermodynamic analogy is temperature=0 Kelvin,
...
% the ``regularization'' terms are thus necessary to capture $\bbr{\dg
  % M}_\gamma$. 
the ``regularization'' terms are thus necessary for these distributions to have any uncertainty.
...
% terms (in the limit as \gamma -> 0) and the cpds that that optimize it remain
% the same. This is very different from just increasing the strength of the 
...
%oli17: because it results in a KL divergence, as we motivated at the beginning.
...
% agent's beliefs _are_ the scores. It's just like an agent's beliefs are a set
% of weighted probability distributions in your book. Why are we suddenly backing
% off of this identification? It's not hard to see that increasing \theta 
% makes the relative scores of distributions that have uncertainty worse, relative to the ones
% that optimize the liklihood term (which are always on the hull of the distribution simplex).
%oli17: as an aside, I get the sense that when you're thinking through this you're
% not really looking at the equation. I think the equation clarifies a lot of the
...
%
%oli16 Of course, none of this matters very much here because conditional independence is not
...
%oli17: We introduce H(Y | X) and H(X) in the IDef paragraph. This discussion seems
...
%oli17: I agree that it's not necessarily a high-value thing for the average reader,
% but I view it as an important technical underpinning that actually has a fair amount
% of subltety (and I say this speaking as someone who wrestled with this point when
% impelmenting PDGs). 
%
%
...
%oli17: I agree with you that it's not a great use of space in the main text, for the 
...
% distribution, represented by some data. This is also standard, and from 
...
%      making BNs, factor graphs, and PDGs the same in terms of turning them into
%      distributions.
%oli17: Until we resolve the above, I will (without marking) turn every \mu_{\cal B}
...
%oli17: The "furthermore" below is now clear, but a little sparse. It takes a little thinking to see why this follows from Fact A.4. Do you agree?
%joe15
...
%oli17: In conjunction with the display-mode \forall compromise above, I'm ok
