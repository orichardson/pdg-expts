...
%	However, these representations are ultimately committed to representing probability distributions; I argue that the graphical model bit is a
	...
	This paper describes a more local, forgiving, dynamic, and intuitive representation of uncertainty, which we call a \modelname, or \MN. The theory we present here can be motivated from several wildly different angles (\cref{sec:multiple-views}), but for the time being, we will think of them as a new general class of graphical model. \MNs\ look like, and are closely related to, Bayesian Networks (BNs), but our semantics makes them much more expressive and modular. In particular, they are able to represent inconsistent belief states (which we argue is extremely important in \cref{sec:consistency-ethos}), and further supports modularity by enabling cheap and reversible combinations of beliefs. Moreover, this view gives us a more natural, robust picture of what ``possible worlds'' are. To understand how we get any of this, let's look at some examples of things \MNs\ can do that BNs cannot.
	
	\begin{example}
		You have arrived in a foreign country well-known for having very clear laws. From prior reading, you have credence 0.95 that owning guns is against the law. Upon landing, you end up talking to some teenagers who use the local slang---a conversation that imparts upon you 0.1 credence that the law prohibits floomps.  
		
		Let's try to represent this as a Bayesian Network. make a graph with two nodes: let $\mathbbm{2}_F$ be the binary variable indicating the legality of floomps, and $\mathbbm{2}_G$ indicate the legality of guns. We can't put any edges between them though, because we don't know the probability of either one conditioned on the other, so our only choice is to assert for now that they are independent. This is not the most unreasonable thing to do --- after all, it is what the maximum entropy folks would have prescribed.  Finally, we can also add a dummy variable $\mathbf 1$ which can only take a single value, which does nothing except to visually give us a place to write our numbers. You now have the following BN.
		
		\begin{center}
		\begin{tikzpicture}
			\node[dpadded] (true)  at (0,1.7) {$\mathbf 1$};
			\node[dpadded] (floomp) at (-1.2,0) {$\mathbbm{2}_F$};
			\node[dpadded] (gun) at (1.2,0) {$\mathbbm{2}_G$};
			
			\draw[arr] (true) -- node[left]{0.1}(floomp);
			\draw[arr] (true) -- node[right]{0.9}(gun);
		\end{tikzpicture}
		\end{center}
		
		You later discover that floomps are guns (with credence 0.99). This piece of information conflicts with the other two. There are three different dimensions along which this could be resolved: rejecting either piece of information, or the new observation; in general any mixture will do. \changeoff
...
%			\item Operating under the assumption that your mental state was consistent before learning that guns are firearms, you have only one option: to reject the impossible observation. 
	...
	This example was intended to be simple and plausible, but if we ex
	
	
	We want to talk about the process of revising uncertain beliefs, but it is difficult to do this without being able to talking about intermediate stages. 

	It's actually quite difficult to be certain that your mental state is logically consistent---as an empirical matter, people regularly come to realize that they hold incompatible beliefs about the world, and very few would be so bold as to assert that their beliefs are completely consistent. In some sense, this is the reason arguments exist in the first place: it is possible to get a person to agree to premises and reject a conclusion, revealing inconsistency---inconsistency which can then be used to . 
