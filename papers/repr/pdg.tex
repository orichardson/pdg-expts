...
\begin{equation}
%oli12:
% \IBal{\dg M}(\mu) := \sum_{\ed{L}XY \in \Ed^{\dg M}} \H_\mu(Y\mid X)
% - \H(\mu).  
...
%that \IBal > 0?  This is not at all obvious to me 
...
% Also I'm not convinced by the argument in any setting: just because the ones that give probability 1 and 0 are the ones that minimize \IBal, doesn't mean that all intermediate distributions score the same. Actually the scores definitely will not cancel.
