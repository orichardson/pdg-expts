...
%oli6
	% are closest to being consistent with $\mu$, 
	 are closest to being consistent with $\dg M$, 
...
%oli8: rewritten
		However
		Intuitively, two distributions that distributions that share 
...
%useful intuition.  Could hou explain it better.
	Alternatively, we can think of the negation of the extra
        information, $-\H^\dg M(\mu)$ as the uncertainty in $\mu$,
...
%        \subsubsection{Putting The Scores Together}
	Both the extra information $\H^\dg M$ and inconsistency
        $\inco$ can be thought of measures of cost of fit for a
...
%	
	We now consider the sum of the two scores, with the trade-off
        ...
        follows:  
	\begin{align*}
		\mathcal U_{\gamma}(\dg M; \mu) &:= \inco(\dg M;\mu) + \gamma \H^\dg M(\mu)\\
		&= \sum_{ X \xrightarrow{\!\!L} Y  \in \Ed } \!\! \E_{x \sim \mu_{\!_X}}  \left[\gamma
%oli8: no more \alpha
% \alpha_L\cdot
 	\H (\bp (x)) + \beta_L \cdot \kldiv[\Big]{ \mu(Y | X \sheq x) }{\bp(x) }  \right]  - \gamma \H(\mu) \\
...
If $\gamma > 0$, then $\mathcal U_\gamma(\dg M;\mu)$ is $\gamma$-strongly convex in $\mu$.
...
%oli8: reformat with equation, added the limit.
	\begin{equation}
		 \bbr{\dg M}^* := \lim_{\gamma\to 0^+}\argmin_{\mu \in \Delta\V(\dg M)} \mathcal U_\gamma(\dg M;\mu)
			\label{eq:uniqdist}
	\end{equation}
    % \begin{defn}
    % 	For $\gamma > 0$,
...
%		\end{tikzpicture}}
	...
	\begin{conj}\label{thm:maxent-hull}
		For any PDG $\dg M = \mnvars[]$ containing variables $X, Y$, the maximum entropy conditional marginal $\bbr{\dg M}\MaxEnt(y \mid x)$ is a convex mixture of the conditional marginals generated by the paths from $X$ to $Y$.  That is, there exist weights $\{\alpha_i \geq 0\}$ on the paths in $\sf M$ and a bias weight $\alpha_0$ with $\sum_i {\alpha_i} = 1$ and
...
\end{vfull}
	...
	\begin{proof}
		...
		\begin{align*}
			...
			\sum_{(A, B) \in \Ed}~\E_{a\sim p(a)} \kldiv{\bp(a) }{\lambda p_1 + (1-\lambda) p_2} &\leq \sum_{(A, B) \in \Ed}~\E_{a\sim p(A)}\lambda \kldiv{\bp(a)}{p_1} + (1-\lambda) \kldiv{\bp(a)}{p_2} \\
		\intertext{and so}
