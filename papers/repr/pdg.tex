...
%		\note{Part of the reason that it's hard to bring in modularity here is that the nature of the improvement differs depending on the model in question. Factor graphs arguably offer the same modularity---but there are other arguments against factor graphs that don't really bear on BNs}
	...
	\changed{The second feature of \MNs\ to highlight is their modularity. We can leverage the new freedom to entirely remove the overhead and assumptions required to make structural changes to the graph, skirting computational costs and enabling concept formation, without also sacrificing interpretability, as factor graphs do (see \cref{sec:factor-graphs}). }
