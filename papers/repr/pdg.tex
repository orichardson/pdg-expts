...
%oli3: It's required by UAI style guidelines afaik?
...
Our original motivation for introducing PDGs was to be able to capture inconsistency. While inconsistency is clearly not something we endorse, we wanted to be able to model the process of resolving it; to do so, we had to model the inconsistency itself. But our approach to modeling inconsistency has many other advantages.
In particular, \MN s are significantly more modular than other directed graphical models: they can be easily restricted or combined in ways that are problematic for other models.
...
%However, it may not be in your best interest to sort this out right away.
		...
		\begin{figure}[h]
			...
			\begin{tikzpicture}
				...
				\node[below=1em of C] {
					\color{heldout}
					$\mat r =\!\!\!$\begin{idxmat}[\footnotesize\color{heldout}\text]{$f$,$\overline f$}{$g$, $\overline g$}
						0.92 & 0.08 \\ 0.08 & 0.92 \\
					\end{idxmat}
				};
			\end{tikzpicture}
	...
	%oli3:
	A BN compactly represents distribution by assuming that every node is independent of its non-descendants given its parents. A choice not to make such assumptions can be cast in either a postive or negative light. 
	On one hand, eliminating unfounded assumptions is important, and independences in particular are notoriously difficult to demonstrate emperically: there is always a possibility of unobserved confounders. On the other hand, conditional independences are valuable, and as \textcite{pearl2014probabilistic} has argued forcefully, omnipresent.
...
% 
		\begin{figure*}[ht!]
			...
			\begin{subfigure}[b]{0.5\textwidth}
				...
				\draw[very thick, |-|, color=orange!50!black] (2.7, 1.35) --node[above]{Restricted \MN\ for \Cref{ex:grok-ablate,ex:grok-union}} (7.1,1.35);
				
				\end{tikzpicture}}
				\caption{Corresponding \MN, $\Gamma(\mathcal B)$ and a restriction}
				\label{subfig:smoking-pdg}
			\end{subfigure}
		
			\caption{Graphical models representing conditional relationships in \Cref{ex:smoking,ex:grok-modularity,ex:grok-union}}
...
% We do not make these assumptions when giving semantics to a PDG.
		...
		We now return to the issue of independence (2). There are many distributions consistent with the conditional marginal probabilities in the cpts, and the independences presumed by $\cal B$ need not hold for them. Rather than encoding this into the diagram, we develop a maximum-entropy semantics for \MNs\ in general, which allows us to (temporarily) view the \MN\ $\Gamma(\mathcal B)$ as representing the unique distrubion that maximizes entropy, among all those consistent with the marginals. In \Cref{thm:bns-are-pdgs}, we will show that this distribution is the same as the one specified by $\cal B$.
...
%		\end{vfull}
		...
		It is now technically possible for our knowledge to be inconsistent. For instance, if the distribution on $C$ given $S,H$ by its original cpt were always deterministically ``has cancer'' for every possible value of $S$ and $H$, but the distribution according to the new cpt from $T$ were deterministically ``no caner''. 
		For this example, such a selection of cpts seems like an implausible edge case; we conclude that the additonal modularity we gain from a \MN\ is useful even when there is no inconsistency.
...
%  - Graph union.
	...
	\begin{example}[restriction]\label{ex:grok-ablate}
		...
		%oli4: this sentence is less useful, and can be removed; its purpose is to pre-emptively push against a desire to margnialize and get a new BN. 
		Incidentally, the rise of the communist party had come with changes in smoking habits, so a new unconditional distribution on $S$ could not be obtained by marginalization. 
		
		Grok knows the useful part of this model is no longer a BN, but rather a \emph{conditional} BN \parencite{koller2009probabilistic}. While a BN would require an association prior beliefs with $S$ and $\mathit{SH}$, a conditional BN (CBN) allows for these nodes to be marked as observations; observation nodes do not have associated beliefs. 
		Although restrictions of CBNs are still CBNs, they are stil not always well-behaved when nodes are removed. For instance, by forgetting the node $B$ in a chain 
		\scalebox{0.6}{
		\begin{tikzcd}[dpad={light pad}, column sep = 1.3em, AmpRep]
			A \ar[r] \& B \ar[r] \& C
		\end{tikzcd}},
		one concludes that $A$ and $C$ are independent, which is in some sense incompatible with the original chain.  
	...
	Being able to restrict your knowledge and look at a local picture may be a nice property, but a much more compelling reason to \MNs\ is the ability to combine knowledge from various parts.
	
	\begin{example}[union and implications]\label{ex:grok-union}
		...
		Grok believes people will make fun of her if she uses a tanning bed and gets cancer, making becoming $\it SL$ impossible. This epistemic state is shown in the left of \Cref{fig:grok-combine}.
		
		\begin{figure}\label{fig:grok-combine}
		...
		Grok is reading about graphical models because she vaguely remembers that the variables in \Cref{ex:smoking}, are the same as those in her epistemic state. When she finishes reading the statistics on smoking and the original study on tanning beds (associated to a cpt $\mat p$ in \Cref{ex:smoking}), but before she has time to reflect, her epistemic state contains the union of the two graphs (call this $\sfM$), depicted graphically on the right of \Cref{fig:grok-combine}.  
		%
		Several features here are noteworthy. 
		\begin{enumerate}[itemsep=0.2em, leftmargin=7pt] % label=\textbf{F\arabic*.},
...
%	\end{enumerate}
	...
	Having seen some examples, \MNs; we now provide the formal definitions.
	Although it is possible to formalize the hyperedges directly, we opt for a different approach here, in which they are actually a shorthand for a small widget which is only possible in MNs.
	Before we explain the weidget, we start with the definition of a \MN.%
		\footnote{Although we call the objects in \cref{def:model} \MNs, they are a specific case of a very similar, but more general representation for which we want to reseve the name; to reduce confusion for future readers, these are technically \emph{strict} \MNs.}
...
% You've addressed my more general concerns in the email.
		...
		\end{itemize}
	\end{defn}
	If $\sfM$ is a \MN, we will reserve the names $\mnvars[^\sfM]$ for its components, so that we may reference one (e.g., $\Ed^\sfM$) without naming them all explicitly. When the choice of $\sfM$ is clear we may omit the subscript.
	We will also write $\V(\sfM)$ for the set of all possible joint settings of the variables in $\sfM$. 
...
is really shorthand for the following \MN:
	\begin{center}
		...
		\begin{tikzpicture}
			...
			\node[dpadded,light pad] (CT) at (-2.9, 0){$\scriptsize C \times T$};
...
%oli4: there are labels on edges again 
          ...
          $p(Y\sheq y \mid X\sheq x)$ is the $xy$ entry in the cpt given by
          $\bmu_{X,Y}$.
...
%	\vspace{0.5em}
	...
	\begin{theorem}[restate=thmbnsRpdgs]\label{thm:bns-are-pdgs}
		If $\cal B$ is a Bayesian Network, and $\Gamma(\mathcal B)$ is the conversion to a \MN\ as described at the end of \Cref{sec:formal+syntax},%
		 	\footnote{(the fully formalized version is in \Cref{def:bnconvert-formal})}
		then $\bbr{\Gamma(\mathcal B)}\MaxEnt = \Pr_{\cal B}$.
	\end{theorem}
	\begin{proof}
