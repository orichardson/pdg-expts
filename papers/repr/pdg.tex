...
\definecolor{note-fg}{rgb}{0.0,.3,0.2}
...
%	Because writing down distributions explicitly is intractable, and so it is necessary to have.
	{\color{blue}
%	I'm still not sold on a story. I think the "there are lots of these, this one has new properties" is more descriptive than engaging. 
	%JOE-1
...
%	\end{enumerate}
	...
	\begin{example} \label{ex:planet}
		...
		\begin{center}
			...
			\node[dpadded,light pad,fill opacity=0] (CC) at (3, 1.2) {Composition};
...
%			\item The addition of a new factor 
	...
	\begin{enumerate}[nosep]
		...
		\item In conjunction with  agents to merge, split, and compress variables, we also make it possible for agents to design their own representations. With these tools, in conjunction with consistency.
