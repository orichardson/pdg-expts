...
%	\begin{defn}
	Here $\underline\Delta(X)$ is the collection of sub-distributions on $X$ --- i.e., distributions on $X \cup \{*\}$, where $\{*\}$ is an optional element. 
	We will call a \MN\ is \emph{strict} if $\bmu$ assigns all of its probability mass. 
%	\end{defn}

	\begin{theorem}[restate=thmhyperequiv]
		Every \modelnamehyper\ is equivalent to a \MN\ with additional variables.
