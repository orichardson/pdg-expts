...
%		\end{enumerate} 
		...
		The original state of knowledge consists of all three nodes and the two black edges from $\mathsf 1$. This is like our Bayes Net \changed{from before}, except \changed{we no longer assume $F$ and $G$ to be independent% (but will later if necessary)
		...
		\note{\emph{This is from our discussion but it seems out of the way of the story so I haven't integrated it.}
			...
			If we wanted to explicitly encode such an independence assumption without making it part of the the background machinery for all diagrams, one would need to either give a link $F \to G$ (or vice versa) whose table has an identical distribution over $G$ every row, or specify the joint 
...
%		\footnote{\changed{Probably not going to make the cut but it might be worth saying at some point that Jeffrey updates are usually reversible but tricky b/c of downstream consequences and order dependence}}
	\end{example}			
