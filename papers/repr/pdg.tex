...
%but that's a guess.  I view the next sentence as a significant net
...
%of the cpt on the edge L?  (Actually, at best, increasing \beta
...
% no ambiguity between the set { X, Y } and the node X \times Y 
  ...
  %
% Of course, none of this matters very much here because conditional independence is not
...
%added: you said I needed to define joint entropy when I wrote it last time, so...
...
%As a general rule, if you're going to use two different notations for
...
% I would much prefer to keep further away from this quantifier muddling, either
...
$\IDef{\PDGof{\mathcal B,\beta}}(\Pr_{\cal B}) = 0$ and  
  $\IDef{\PDGof{\mathcal B,\beta}}(\mu') \ge 0$ for all distributions
$\mu$.  Thus, for all $\gamma > 0$ and all distributions $\beta$, we
have that         $\bbr{\PDGof{\mathcal B, \beta}}_\gamma( \mu_{\cal
...
%joe14*: new proof
So suppose that $\mu \ne \Pr_{\cal B}$.  Then $\mu$ must have the same
cpds as $\Pr_{\cal B}$, for otherwise $\Inc_{\PDGof{\mathcal B,
\beta}}(\mu) > 0$, and we are done.  Thus, it must be case that for
