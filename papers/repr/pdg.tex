...
%This is really bad notation.  Pick one and stick with it (and its variants)
				the entire distribution. Therefore, $\zeta(\sfM; \mu) =
...
%                $p^* = \mu$. Furthermore, for $\alpha > 0$,
				\kldiv{\mu}{\mu}$, so the inconsistency is just the information $\mu$ and $p$, so is minimized when $\mu$ is $p$,
				\begin{vfull}
					and corresponds quite litterally to the distribution of minimum Gibbs free energy for the the log liklihood defined by $\sfM$ (see \Cref{sec:thermo}).
				\end{vfull}
...
%                better than the beliefs you actually hold. 
	...
	\begin{defn}
		The \emph{extra information}, $\H^{\sfM}$, given by a distribution $\mu$ that satisfies the constraints of a PDG $\sfM = \mnvars[]$ is given by
		\[ \H^{\sfM}(\mu) := \sum\alle \Big[ \E_{x \sim \mu_X}  \H (\bp (x))  \Big] - \H(\mu) \] 
		where $\mu_X$ is the marginal of $\mu$ on the variable $X$, and $\H(\bp(x))$ is the entropy of the distribution on $Y$ specified by the cpt $\bp$ when $X = x$. 
	\end{defn}
	% The cpts may not (in the entropy sense) 
	We can think of the extra information as the sum of the entropies that \emph{actually} result from each table, in the context of distribution $\mu$, minus the total entropy of the distribution. Alternatively, we can think of the negation of the extra information, $-\H^\sfM(\mu)$ as the information in $\mu$, which has not already been specified by the cpts in $\sf M$. We will show in \Cref{sec:bn-convert} the distribution with minimal extra information with respect to the cpts of a BN $\cal B$ is the unique one specified by $\cal B$.
	\begin{vfull}
...
% 
	...
	Both the extra information $\H^\sfM$ and inconsistency $\zeta$ can be thought of measures of cost of fit for a distribution $\mu$, with higher numbers being worse. We can think of either as defining an energy landscape on possible distributions, in which we intuitively expect distributions with low energy to be more likely.  
	
	We now consider the sum of the two scores, with the trade-off controlled by a parameter $\alpha \ge 0$, giving us a score as follows: 
	\begin{align*}
			%\bbr{\sfM}^\alpha(\mu)
			\mathcal U_{\alpha}(\sfM; \mu) &:= \zeta(\sfM;p) + \alpha \H^\sfM(\mu)\\
			&= \Bigg(\sum_{ X \xrightarrow{\!\!L} Y  \in \Ed } \E_{x \sim \mu_{\!_X}}  \left[\alpha\cdot \H (\bp (x)) + \beta_L \cdot \kldiv[\Big]{ \mu_Y(\cdot | X \!=\! x) }{\bp(x) }  \right] \Bigg) - \alpha \H(\mu) \\
			\numberthis\label{eq:freeenergy-weighted}
	\end{align*}
	
	For $\alpha = 0$, the only thing that matters is consistency.  For $\alpha$ arbitrarily small, the preference for consistency dominates, and extra information is used only to break ties. If an agent is very certain that their cpts are well-informed, which we will call the `low ambient uncertainty' case, this may be appropriate. 
	As $\alpha$ grows, an agent becomes willing to compromise on some extra inconsistency for extra information. For instance, an agent that does not even have consistent knowledge might be better off hedging with an uncertain estimate, for the same reason that uniform randomness is optimal in rock-paper-scisors against an unknown adversary. Even in situations where there is a consistent distribution, one might be justified in picking something else, as the next example shows. 
	
	\begin{example}
		Consider the PDG with variables $\sf 1$ and $X$, and two distributions $p, q$ on $X$. 
		\todo{figure out if this example can be formulated without substochasticity. This is the one discussed in meeting, where two experts disagree on their except for $\epsilon$ overlap.}
	\end{example}
...
$\infty$.  This can be seen as a weighted set of distributions, as given in \cite{halpern2015weighted} once we transform it so that it is always
non-negative.
