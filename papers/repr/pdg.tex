...
% 
	...
	Both the extra information $\H^\sfM$ and inconsistency $\zeta$ can be thought of measures of poorness of fit, with higher numbers being worse. We can think of either as defining an energy landscape on possible distributions, in which we intuitively expect distributions with low energy to be more likely.  
	
	We therefore consider a generic sum of the two quantities, with the trade-off controlled by a parameter $\alpha \ge 0$, giving us a score as follows: 
	\begin{align*}
			%\bbr{\sfM}^\alpha(\mu)
	...
	For $\alpha = 0$, the only thing that matters is consistency.  For $\alpha$ arbitrarily small, this roughly corresponds to selecting the distribution .
	As $\alpha$ grows, one becomes willing to trade off some consistency for extra information.
	
	\begin{example}
		Consider the PDG with variables $\sf 1$ and $X$, and two distributions $p, q$ on $X$. 
		\todo{figure out how to formulate this example without substochasticity. This is the one discussed in meeting, where two experts disagree on their except for $\epsilon$ overlap.}
	\end{example}
