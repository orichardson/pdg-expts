...
\newcommand{\modelname}{Conditional Pseudomarginal Graph}
\newcommand{\modelnames}{\modelname s}
\newcommand{\MN}{CPG}%	allow for much more modularity than available to Bayesian Networks (examples below), and
\newcommand{\MNs}{\MN s}
...
\begin{document}
	...
	\begin{abstract}
		Graphical models have enjoyed success in compressing probability distributions over joint settings of random variables, by making use of independence assumptions. 
...
%	\note{Changed. The focus was wrong in the previous one you told me to cut, but this part is still useful I think.} 
	For those staring at the details, the biggest difference between \MNs\ and BNs is the interpretation of two colliding arrows, as highlighted in \cref{ex:smoking}.
...
%	We give the definition below for the finite case.  
	\changeon
	We have seen some examples of \MNs, but elided all of the details; we now begin a more careful treatment of these mathematical objects. Rather than representing a probability distribution, \MNs\ can be thought of as \emph{constraints} on distributions.
	...
	\begin{defn}\label{def:model}
		...
		\begin{itemize}[nosep]
			...
			\item $\mathcal L \subseteq {\cal N} \times {\cal N}$~~is a set of directed links between nodes
