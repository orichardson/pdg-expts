...
In particular, \MN s are significantly more modular than other approaches: they can be easily ablated or combined in ways that preserve their semantics.
...
%oli3: I'm 
   and give one of the two unconditional probabilities and a conditional probability table. As there is no reason to believe that either variable depends on the other, the natural choice is to assume independence, giving us the following BN:
		...
		It is easy to check that there is no probability distribution that is consistent with the two original priors on $F$ and $G$ and also the cpt $T$, so if we are to represent our information with a BN, which always represents a consistent distribution, we must resolve the inconsistency.  
...
%However, it may not be in your best interest to sort this out right away.
		...
		\begin{figure}[h]
			...
			\begin{tikzpicture}
				...
				\node[below=1em of C] {
					\color{heldout}
					$\mat p =$\begin{idxmat}[\color{heldout}\smallt]{$f$,$\overline f$}{$g$, $\overline g$}
						0.92 & 0.08 \\ 0.08 & 0.92 \\
					\end{idxmat}
				};
			\end{tikzpicture}
...
%joe3: might want to believe what?
	...
	\begin{example}\label{ex:smoking}
		We now consider the classic (quantitative) Bayesian network $\cal B$, which has four binary variables indicating whether a person ($C$) develops cancer, ($S$) smokes, ($SH$) is exposed to second hand smoke, and ($PS$) has parents who smoke, presented graphically in \Cref{subfig:smoking-bn}. We now walk through what is requried to represent $\cal B$ as the \MN $\Gamma(\mathcal B)$, shown in \Cref{subfig:smoking-pdg}.
% 
		\begin{figure*}[ht!]
			\centering
			...
			\begin{subfigure}[b]{0.5\textwidth}
				...
				\draw[arr,dashed] (T) -- (C);	
				\end{tikzpicture}}
				\caption{Corresponding \MN, $\Gamma(\mathcal B)$}
...
% We do not make these assumptions when giving semantics to a PDG.
		...
		We start with just nodes corresponding to the variables in $\cal B$, and also a special node $\sf 1$ from \Cref{ex:guns-and-floomps}. and an edge ${\sf 1}$ to $PS$, to which we associate the unconditional probability that one's parents smoke, given by the table attached to $PS$ in $\cal B$.
		The cpts on $S$ and $SH$ we can also repurpose for the PDG directly, assigning them to the edges $PS \to S$ and $PS \to SH$, respectively. There are two remaining problems: (1) modeling the last table in $\cal B$, which corresponds to the conditional probability of $C$ given $S,SH$, and (2) recovering the additional independece assumptions in the graph.
		
		We start with (1). We should not simply add the edges $S \to C$ and $SH \to C$ that are present in $\cal B$, because as we saw in \Cref{ex:guns-and-floomps}, this would need an interpretation as two separate tables, one indicating the probability of $C$ given  $S$, and the other indicating the probability of $C$ given $SH$. Though we could get both tables by marginalizing, this would lose an important correlation that is present in $\cal B$.  
		To distinguish the joint dependence on $S$ and $SH$, for now, we draw an edge with two tails, sometimes called a \emph{hyperedge}, which completes the diagram in \Cref{fig:smoking-pdg}.

		We now return to the issue of independence (2). There are many distributions consistent with the marginal probabilities described, and the independences presumed by $\cal B$ need not hold for them. To model this, we supply a maximum-entropy semantics, that allows us to temporarily view the \MN\ $\Gamma(\mathcal B)$ as representing the unique distrubion that maximizes entropy, among all those consistent with the marginals. In \Cref{thm:bns-are-pdgs}, we will show that this distribution is the same as the one specified by $\cal B$.
...
%		\end{vfull}
		...
		Now, suppose we read a thorough empirical study which demonstrates that people who use tanning beds have a 10\% incidence of cancer, compared with 1\% in the control, and would like to add this information to $\cal B$. 
		...
		Just as in the previous example, this cannot be encoded directly into the Bayesian Network. The \MN\ $\Gamma(\mathcal B)$ has no trouble:
...
% - 
	...
	\begin{example} \label{ex:planet}
		...
		%oli3: I've removed the ndoe
		%		Ignoring the faint node labeled $W$ for now,
		...
		added) is not a correct representation of the situation as a PDG. 
		
		The problem is that, in a PDG, the edges from $S$ to $L$ and from $C$ to
...
%                          and target in $\N$.
%joe3: no labels.
%oli3: the \notation{...} is a comment. I don't have to mention labels explicitly, but they need to be hyper-graphs. I don't know how to define constructions or defend things otherwise, and lots of things in my head and tricks to prove things simply will not work. I'm going to replace the word "set" with the word "collection" where uniqueness is hazier. 
...
%oli3: I like \V(N) better but I needed the room to make my equations fit... we'll see if
% I can find the room elsewhere.
...
%something you need to address in general.  
%oli3: Any distribution with p'(x) = 0 verything matches in this case. I guess I put this in the set of distributions section?
%joe3: Moving up  a level, this
%is not a terribly insightful example.  It might be useful to consider
%the examples in the intro.
%oli3: With these modifications, the example is indeed vacuous. But if you look inside the construction it's just a single quantifier alternation away from being litterally the definition of p. I don't know why you chose to modify it this way. 
