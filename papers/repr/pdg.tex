...
%	We give the definition below for the finite case.  
	...
	\begin{defn}\label{def:hypermodel}
		...
		\begin{itemize}[nosep]
			...
			\item $\mathcal L : \mathrm{List}(2^{\cal N} \times 2^{\cal N})$~~is a set of directed links from one subset of $\cal N$ to another.
...
%	\section{Semantics}\label{sec:semantics}
	...
	\begin{defn}
		If $M = (\mathcal N, \mathcal L, \mathcal V, \boldsymbol\mu)$ is a \MN, let $\llbracket M \rrbracket_\Set$ be the set of distributions over the variables in $M$ consistent with $\boldsymbol\mu$ on every \changed{pseudomarginal}. Formally,		
	...
	\changed{
		Notice that $\bbr{
%			\begin{tikzcd}
%			[ampersand replacement=\&, every matrix/.append style={nodes={draw, fill = grey!20!white, circle, outer sep=5pt, inner sep=1.5pt}}, column sep=2em]
%				A \ar[r] \& B 
%			\end{tikzcd}
			A \overset{p}{\to} B
		}$ is the set 
	\[ \{ \mu \in \Delta(\mathcal V(A) \times \mathcal V B)  : \mu(b \mid a) = p(b\mid a) \forall a, b \}\]
	 of $\mu$.  
	is exactly the conditional distribution $p( B \mid A)$ achieved by coloring nodes and explicitly separating out the "observed" nodes at the beginning of your construction.
	}
...
%	This corresponds to a lexicographical ordering on \emph{all} distributions (as opposed to simply the ones in $\bbr{M}_\Set$), where we order first by satisfaction of constraints, and then by entropy. In general, we might be willing to relax the constraints a little, and sacrifice some fit for aa more general distribution. This allows us to define a free energy; see \cref{sec:thermo} for more.
	...
	\changeon
	Conditional distributions can be thought of as probabilistic programs. As a result, we can compose and run them --- one  benefit to keeping local data in the form of conditional distributions is that you can compose them; paths from $X$ to $Y$ correspond to estimates of $Y$ from $X$.
	
	Specifically, if 	
	\[(\mathbf g\circ \mathbf f) [c \mid a] :=  \sum_{b \in \mathcal V( B)} \mathbf f[ b \mid a]~ \mathbf g[c \mid b] \]
...
%	\end{align*}
	...
	For factor graphs, we already know how to use the thermodynamic parameters, in this case, they line up for all temperatures, as shown in the following, stronger result:
