...
%	We give the definition below for the finite case.  
	...
	\begin{defn}\label{def:hypermodel}
		...
		\begin{itemize}[nosep]
			...
			\item $\bmu\colon (({\bf A,B})\colon \mathcal L) \to \prod\limits_{A\in \bf A} \mathcal V(A) \to \underline\Delta\left[\prod\limits_{B \in \bf B}\mathcal{V}(B)\right]$ is a family of conditional probability distributions on the joint settings of $\bf B$ indexed by the joint settings of variables in $\bf A$ for every link $({\bf A,B}) \in \mathcal L$ %
		\end{itemize}
	\end{defn}

	The name is chosen because each arrow is a pseudomarginal (in the sense of not necessarily)
...
%	\end{conj}
	...
	\begin{defn}
		A weighted \MN\ is a \MN\ $M = (\cal N, L, V, \bmu)$ together with an inverse temperature $\beta_L \in \mathbb R$ for each link $L \in \mathcal L$.
	\end{defn}
...
%	\vspace{0.5em}
	...
	\todo{Finish defining factor graphs}
...
%	As we discuss in section \ref{sec:set-of-distribution-semantics}
	...
	Where $Z_U(\beta) = \sum_{w \in W} \exp(-\beta U(\beta))$ is a normalizing factor, sometimes called the ``partition function''.	
	...
	\end{align*}
	The constant factor $-\frac{1}{\beta} \ln Z_U(\beta)$ coincideds with the Heimholtz free energy of the system.
	
	
	We now turn the question of what probability distributions are most favorable, for which we use formulate the Gibbs free energy, $G_U(\nu) := {\E}_\nu( U )  - H(\nu)$. If $U$ was generated by a probability distribution $\mu$, we then have
	
	\begin{align*}
		G_\mu(\nu) &= {\E}_\nu( E_\beta(\mu) )  - T H(\nu) \\
		&= \sum_{w \in W}\nu(w) \frac{1}{\beta} \ln \left(\frac{1}{\mu(w)}\right) - T \left[\sum_{w \in W} \nu(w) \ln \left(\frac{1}{\nu(w)}\right)\right]\\
		&= D \left(\mu || \nu \right)
	\end{align*}
