...
\begin{document}
	...
	\begin{abstract}
		Graphical models have enjoyed success in compressing probability distributions over joint settings of random variables, by making use of independence assumptions. However, because they make , they are not expressive enough to represent mental states of agents who might not have globally consistent beliefs. 
...
%	\tableofcontents
	...
	(corroborating evidence means more when there is a possibility of conflict instead; see \cref{ex:corrob}),
...
%		\note{Part of the reason that it's hard to bring in modularity here is that the nature of the improvement differs depending on the model in question. Factor graphs arguably offer the same modularity---but there are other arguments against factor graphs that don't really bear on BNs}
	...
	The graphical model we propose, which we call a \modelname\ (\MN),
	...
	\begin{example}\label{ex:guns-and-floomps}
		...
		The semantics of a Bayes Nets offer us two choices: assume that the two are independent, or choose a direction and make up some numbers to put in the table.
		...
		You later discover that ``floomp'' is likely to be another word for gun, and come to believe that if floomps are legal (resp. illegal), then there's a 92\% chance guns are as well; let $E$ be the CPT associated to this realization.
