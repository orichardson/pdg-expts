...
\begin{document}
	...
	\begin{abstract}
		...
		%oli1: this feels like the wrong sense: they make it easier to re-structure information!
		\commentout{without having to restructure
		the representation of current information.}
		and restructure the representation, without having to finalize a distribution.
		%joe1*: this is a point you should trumpet loudly
		%oli1: untangle the sentence
		\commentout{As we show, when viewed as a \MN, a BN $G$ represents many}
		As we show, a BN $G$, when viewed as a \MN\ $\sfM$, may represent
		distributions, but we can recover the one given by the semantics of BNs as
...
%oli1: what do you think of this footnote? 
...
%	Despite their flexibility, \MNs\ perhaps counter-intuitively offer a way to recognize when something has gone wrong, and 
	...
	The modifications we make bring graphical models in line with other notions of locality in mathematics, 
...
%oli1: why do should explain it here? We haven't fully explained anything yet. I mention it again when I the union precisely. I was hoping to advertise another strand of the story: that 
	such as manifolds and sheaves, in which multiple restricted pictures can be glued together to form something with global structure.
...
%oli1: is cpt intentionally lowercase? 
...
%		We will represent the CPT for a link $L \!\colon\! A\!\to\! B$ as matrices $\mat L$, whose element $\mat L_{a,b}$ at row $a$, column $b$ is the conditional probability $\Pr(B \!\!=\!\!b \mid A \!\!=\!\! a)$. 		
		...
		\MN, where the edges from $\mathsf 1$ to $F$ and $G$ are
...
% 1. beginning to introduce `over-constrained' and `under-constrained' terminology. I think this is useful for understanding why the set semantics are not enough.
% 2. no segue into conditional BNs, or acknowledgement that they are the standard solution to under-constrainedness
% 3. no signposting on the difficulty of adding parents.
% >> shortening of paragraph above
%The modularity of PDGs is useful even when there is no inconsistency, as the following example shows.        
...
%		Suppose we have a belief about how size and composition affect the habitability of a planet: say we're astrobiologists, and we have some sense of how likely we are to find life on a given planet, supposing we knew its size (big or small) and its composition (mostly made of rocks vs gas), resulting in the CPT from \cref{fig:planet-initial}.
		...
		\begin{figure}[h]
			...
			\scalebox{0.8}{
				...
				\node[dpadded,light pad,fill opacity=.2] (SS) at (0.2, 1.2) {S};
%joe1*: please remove the coloring; it's unnecessary, and a
%distraction.  
%oli1: it is necessary to distinguish the observed nodes
% for this diagram to make any sense. There is a very clear
% standard for how to do this: to make them darker. Do
% you think we should have a technically incorrect diagram
% and try to explain around it, or 
				\node[dpadded,light pad,fill opacity=.2] (CC) at (2.6, 1.2) {C};
				\node[dpadded,light pad,fill opacity=0] (LL) at (1.4,0) {L};
...
%oli1: as for example3: there is one additional independence. The tables alone uniquely determine a distribution here, but there are still many in example3 because of the correlations. This example has the benefit of also showing the under-constrained representation and being smaller. I think for now I'll describe more here and dramatically reduce example3. 
		...
		Of course, we also want to keep the joint dependence on both $S$ and $C$ together; to distinguish this from the case where we had wanted to separate out `water', we use an edge with two tails, sometimes called a hyper-edge. The resulting \MN\ is shown in \Cref{fig:plannet-pdg}.
		...
		The two edges in \Cref{fig:plannet-pdg} correspond to the two conditional distributions we have on $L$: one from $S \times C$ and the other from $W$. We can now combine our two beliefs, without also providing information about the correlations between $W$ and $S\times C$. As with the previous example, there is now a possibility of being inconsistent, in the sense that there exist CPTS for the two edges that are not compatible with any joint distribution---for instance, if all estimates of $L$ from the $W$ are strictly smaller than any probability estimate of $L$ from $S \times C$.
	\end{example}
...
%		The BN is a compact representation of a joint distribution over all four variables, which achieves compactness by taking advantage of independence between variables. 
		...
		A \MN\ does not make these assumptions. Depending on our chosen semantics (we offer several in section \ref{sec:semantics}), we can further interpret the constraints. For instance, by looking at the maximum entropy distribution consistent with the constraints, we get back the independence assumptions from the BN, and can thereby view \Cref{subfig:smoking-pdg} as representing the same distribution as the BN.
				
		Suppose you read a very thorough empirical study which demonstrates that people who use tanning beds have a 10\% incidence of cancer, compared with 1\% in the control. Just as in the previous example, this cannot be encoded directly into the Bayesian Network. 
		The \MN\, once again has no trouble: we can simply merge the graphs, as in \ref{subfig:smoking-pdg}.
...
%	\moveme{Rather than representing a probability distribution, \MNs\ can be thought of as \emph{constraints} on distributions.}
	...
	\begin{defn}[\MN]\label{def:model}
		...
		%oli1: the union doesn't work out if they're not labeled. There are definitely examples where you have parallel edges (two candidate probabilities b/c you've deleted a node and haven't resolved it, or two conflicting observations...). Having parallel edges will make the formalism cleaner as well, because we will definitely have parallel paths. 
			\item $\Ed \subseteq \N \times \N \times \mathrm{Label}$~~is a set of directed links, each of which has a source and target in $\N$.%
...
%oli: I do introduce them, just below. I've thought a lot about where to put this. I don't want to have a totally separate definition for the strict version because it takes up so much space. 
			\item $\bmu\colon \big(\!({A,B,\ell})\colon \! \Ed \big) \to \V(A) \to \underline\Delta\V(B)$
			...
			gives a (sub)-distribution over $\V_B$ given each possible value of $\V_A$ and for every link $({A,B,\ell}) \in \Ed$.
...
%oli: it requires natural language to parse. Also, is there a difference between associating to each element of x a distribution on B and associatinga  distribution on B with each element x?. I also cant express my preferred currying: I immediately see that \mu_L is a distribution on B --- and I now need more text to explain what I mean by \bmu_L(a). I've been in the position of intuitively thinking I understand an applied math paper only to realize I've subtly misunderstood the type so many times.
% It is also a much better visual anchor, and I find it way easier to read than a block of description, even if the description took the same amount of paper space.
% Finally, I have a personal quest to get people to explicitly use their dependent types. 
%			 \item $\mu$: for each edge $(X,Y)$, $\mu$ associates a distribution on $B$ with each element $x \in \V(X)$.
...
%		\begin{itemize}[nosep]
%			\item $\N$~~is a finite collection of nodes
%			\item $\Ed \subseteq 2^{\N} \times 2^{\N} \times \mathrm{Label}$~~is a set of directed links, each of which has a source and target subset of $\N$.
%			\item $\V$ associates each node $N \in \mathcal N$ with a set $\V(N)$ or $\V_N$, representing the values that node $N$ can take.
%			\item $\bmu$
%			 % $\colon\!\big(\!({\bf A,B})\colon \! \Ed \big) \to \prod\limits_{A\in \bf A} \!\! \V(A) \to \underline\Delta\left[\prod\limits_{B \in \bf B}\!\!\V(B)\right]$
%			%%% Above is the type of $\bmu$. I think it's important to have it there.
%			associates conditional probability (sub)-distributions on the joint settings of $\bf B$ indexed by the joint settings of variables in $\bf A$ for every link $({\bf A,B}) \in \Ed$ %
...
%	\end{defn}
	...
	\begin{itemize}
		\item A special variable $\sf 1$ that always takes its unique value $\star$. It is used to represent unconditional distributions, as in \cref{ex:guns-and-floomps,ex:smoking}. For strict \MNs, this is no different from any other variable that can take only a single value. 
		...
		\item Double-headed arrows $A \doubleheadrightarrow B$ indicating the degenerate special case of a CPT corresponding to a deterministic function $f : A \to B$, the distribution assigning a point mass to $f(a)\neq\none$ for each $a \in A$.
		\begin{examplex}
	...
	An astute reader is probably wondering how \cref{def:model} can capture the multi-tailed arrows we needed in \cref{ex:planet,ex:smoking}; the answer that this notation is roughly shorthand for a new node at the junction, with projections. For instance, the \MN\ in \cref{ex:planet} is translated as follows:
	\begin{center}
		\scalebox{0.8}{
			\begin{tikzpicture}
				\node[dpadded] (L) at (1.3,0) {$L$};
...
%	You may have noted that this only includes data for the normal arrows, rather than the multi-tailed ones described in the examples. This can be achieved either by generalizing $\Ed$ to higher orders, or introducing product nodes; we opt for the latter as it simplifies the formalism and paves the way for additional special nodes in \cref{sec:algebra}.
	...
	If $\sfM$ is a \MN, then $\mnvars[^\sfM] := \sfM$, allowing us to use internals without de-structuring the entire tuple.
...
%	Finally, let $\mu_\none$ be the unique distribution on $\V_\None(M)$ such that 
...
%	If a probability distribution is a soft constraint on the set of possible worlds, then a \MN\ is a soft constraint on the set of probability distributions.
	As far as $\bbr{-}_\Set$ goes, all inconsistent \MNs\ are equivalent, which is unsatisfying --- for instance, there's a difference between thinking a 60\% head-biased coin will land heads 63\% of the time, as opposed to 0\% of the time. %and estimates of distributions from samples are necessarily going to be a little off. 
	
 	We therefore move to generalize the semantics in the previous section. For a candidate distribution $\nu$, rather than assigning it a 0/1 score of whether or not it is consistent, we give it a continuous one, intuitively encoding how far we have to nudge things to make it consistent. We only want to score distributions based on the links $\Ed$ that an agent actually has expressed, and we will measure similarity by relative entropy: the number of bits required to encode the $\bmu^\sfM$ given an optimal code for $\nu$.  Perhaps one might think that this seems backwards: we're giving a 
...
%			= -\log \sup_{p \in \Delta[W_{\cal V}]} \bbr{M}_\zeta (p)
	...
	\begin{defn}
		A weighted \MN\ $(M, \beta)$ is a \MN\ $\sfM$ together with an inverse temperature $\beta_L \in \mathbb R^+ \cup \{\infty\}$ for each link $L \in \Ed^\sfM$.
	\end{defn}
...
%			\item The addition of a new factor 
	...
	Let $\Phi(M)$ be the factor graph associated to $M$, and $\Phi(\mathcal B)$ be the one associated with a Bayesian network. That is, the clique index set is $\Ed$, and where each link from a product of variables $\{A_i\}$ to 
	each variable 
...
%	\end{align*}
	...
	\todo{Fix up}
	A \MN\ clearly encodes more information than just the distribution: this is true for both Bayesian Networks and Factor Graphs as well. In both cases, this is often cast as a flaw, as this makes them poor choices as cannonical descriptions of distributions, which is why so much attention is given to I-maps in \cite{koller2009probabilistic}. However, directed models causal connotation distinct from the distribution also lends to their success as an epistemic representation.
	...
	One solution, by \cite{frey2012extending} is to also enforce some local constraints. While this indeed solves issues \todo{make full list of problems to refer to, select from notes} \ref{fg-problem:}, it still leaves issues $\#-\#$.
...
%	A conditional random field (CRF) is an undirected graphical model which represents a conditional distribution, making it \emph{under-constrained}.
	...
	There is also an opportunity to modify $\beta$, but in standard graphical model literature, people set $\beta = 1$ and forget about it.%
		\footnote{A similar complaint, that temperature parameters are ignored, is filed in \cite{fixing-broken-elbo}, in which many information theoretic trade-offs, such as $\beta-$VAEs, can be exposed by allowing thermodynamic $\beta \neq 1$}
...
%		\end{tikzpicture}}
	...
	\thmsetconvex*
	\begin{proof}
		Choose any two distributions $p, q \in \bbr{M}_\Set$ consistent with $M$, any mixture coefficient $\alpha \in [0,1]$, and any link $(A,B) \in \Ed$.
	...
	\begin{proof}
		...
		Choose any link $L \in \Ed$ from $A$ to $B$, and also any $a \in \mathcal V(A)$. 
	...
	\begin{vcat}
		...
		\begin{defn}
			...
			\begin{itemize}
				...
				\item $\Ed' = \Ed \cup \Ed^\sigma$ is $\Ed$ extended with extra links for operations that are 
			\end{itemize}
		\end{defn}
