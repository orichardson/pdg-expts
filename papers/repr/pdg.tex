...
\begin{document}
	...
	\changed{
	Standard models of mental states such as a probability distributions and existing graphical models cannot represent such inconsistency.
	While we agree that inconsistency is problematic, it is also 
	}
	...
	\changed{This representation, which we call a \modelname\ (\MN),}
	offers significant advantages over more standard approaches, such as Bayesian Networks and Factor Graphs, including 
	both better modularity and \changed{an increased expressiveness which which enables inconsistency}.
	
%	We also want to talk about the process of revising uncertain beliefs, but it is difficult to do this without being able to talking about \changed{inconsistent} intermediate stages. 
	The rest of this section consists of examples focusing on these two benefits (modularity and the possibility of inconsistency), why we want them, and the failure of other epistemic representations to quite capture what we have in mind.
...
%			\footnote{We are because the Bayes Net is committed to ultimately producing a distribution, and to do this we need to actually decide on whether these will be correlated or not.}
		...
		In order to represent unconditional probabilities cleanly (among other things; see section \ref{sec:algebra}), we introduce the \emph{unit variable}, $\mathsf 1$, or `$\mathrm{true}$', which always takes its unique value (which we call $\star$). 
		...
		Regarding point \ref{item:waiting-good}, we can stay in this conflicted state as long as necessary (again not even representable in other graphical models), and the operation is even reversible: all we need to do to recover our original belief state is to delete the new edge.			
	\end{example}			
...
%	\Cref{ex:guns-and-floomps} 
	...
	\begin{example} \label{ex:planet}
		...
		, we would also require distributions over the values of  the root nodes `Size' and `Composition'---things we may not know anything about. \changed{This table alone is therefore not enough information to nail down a unique joint distribution, making it \emph{under-constrained}; \MNs\ }
		...
		\changed{This means that we have} two conditional probability tables on $L$: one from $S \times C$ and the other from $W$. This would allow us to combine our two beliefs, without also \changed{requiring us to provide information we do not have}. As with the previous example, there is now a possibility of being inconsistent, in the sense that is possible to specify the conditional distributions in the links in such a way that no joint distribution on all variables will marginalize out to them --- for instance, if all estimates of $L$ from the $W$ are strictly smaller than any probability estimate of $L$ from $S \times C$.
	\end{example}
	
%	Though perhaps difficult to see in example \Cref{ex:guns-and-floomps},
	\MNs\ are actually strictly more expressive than Bayesian Networks, and there is a straightforward conversion from a BN to a \MN (section \ref{sec:bn-convert}). 
...
%		This is again an illustration of the modularity and the possibility for inconsistency; compare the right half of the diagram (shaded slightly darker) with the topological equivalent in example \ref{ex:planet}.
	...
	While other graphical representations (such as factor graphs) can deal with some of these concerns, they raise other problems (see \cref{sec:factor-graphs}).
	
	\todo{A summary paragraph goes here? Is that a good use of space? }	
	

	\begin{vfull}
		\subsection{A More Comprehensive List of \MN\ Benefits}\label{sec:list-of-benefits}
		Beyond what we've mentioned here, we can go quite a bit further with this formalism. Below is a more comprehensive list of issues we can now address.
...
%	We give the definition below for the finite case.  
	\changeon
	So far we have seen some examples of \MNs, but swept most of the details under the rug. We now begin with a more careful treatment of these mathematical objects. Rather than representing a probability distribution, \MNs\ can be thought of as representing \emph{constraints} on distributions.
	
	Compared to a Bayseian Network, a \MN\ still consists of a directed graph, and the interpretations of the edges are still conditional probability tables, but now each edges is interpreted individually, rather than in clumps for each target node. For instance, the graph
	...
	\changed{As this definition only includes data for the normal arrows, rather than the special ones in
	}
...
%			
		...
		\end{tikzpicture}
		\end{center}
		We will sometimes use double headed arrows like this to emphasize degenerate conditional distributions, which are fully deterministic.
...
%	\begin{conj}
%		If $\mathcal P$ is a closed convex set of probability measures, then there is a \MN\ $M$ such that $\bbr{M}_\Set = \mathcal P$ 
%	\end{conj}
...
%			\item The addition of a new factor 
	...
	\section{The Full Model: Sub-stochastic Transitions}
...
%	As we discuss in section \ref{sec:set-of-distribution-semantics}
	...
	\changed{
	Better than classifying \MNs\ as either consistent or not, we can also give a continuous value for the amount of inconsistency:
	}
