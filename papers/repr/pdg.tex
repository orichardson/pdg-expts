...
\begin{document}
	...
	\begin{abstract}
		gGraphical models cleverly compress probability distributions over joint settings of random variables, by making use of independence assumptions.
...
%	We give the definition below for the finite case.  
	...
	\begin{defn}[\MNH]\label{def:hypermodel}
		...
		\begin{itemize}[nosep]
			...
			\item $\bmu\colon\!\big(\!({\bf A,B})\colon \!\Li \big) \to \prod\limits_{A\in \bf A} \!\! \V(A) \to \underline\Delta\left[\prod\limits_{B \in \bf B}\!\!\{V}(B)\right]$ is a family of conditional probability (sub)-distributions on the joint settings of $\bf B$ indexed by the joint settings of variables in $\bf A$ for every link $({\bf A,B}) \in \mathcal L$ %
		\end{itemize}
