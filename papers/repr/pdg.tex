...
%oli7: for instance, by generating a joint cpt as a product distribution
% of two incoming cpts, so that it can be interpreted as a BN.
...
%suggest ``PDGs make it easy to remove nodes, edges, and cpts'', which
...
%consistently assign one cpt p, and the other cpt q, if you can't even
...
%joe6: So why not take the label to be the cpt itself?  That is, we
%can take E to be a set of labeled edges such that the label is a cpt.
%Or say that we'll abuse notation slightly and allow different instances of the
%same element in a multiset to be associated with different cpts?  We
...
%effectively controls how important it is to preserve the CPT
...
%                \ldots, y_100\}$, and consider the cpt $\mat p$
                \ldots, y_{100}\}$, and consider the cpt $\mat p$
...
%formalism use the 
% be cpt as the label, because we there's no reason to preclude two
        %copies of the same cpt.
...
% to the cpt (as the model imagines), as compared to by the true distribution.
...
% every cpt, at least one edge will incur a positive cost, and Inc(-),
...
%		\[ \H^{\sfM}(\mu) := \sum\alle \alpha_L \Big[ \E_{x
		...
		where $\mu_X$ is the marginal of $\mu$ on the variable
                ...
                distribution on $Y$ specified by the cpt $\bp$ when $X
                = x$.
%joe6*
%The extra information of an unweighted PDG is given by assuming
%$\alpha_L = 1$ for each $L \in \Ed^\sfM$. 
	\end{defn}
...
\commentout{
    ...
    can represent inconsistent knowledge states. That said, the
%joe6
%    semantics we give here is still important, as it will allow us to
    semantics we give here is still important, as it allows us to
    compare with the semantics of existing graphical models, and
%joe6
...
%    for specifying a distribution. 
	...
	The idea is to simply select the distribution with the 
...
%    weight, which is unique, due to the following result: 
    ...
    show, the score is strongly convex.  It then follows from standard
...
%      $\mathcal U_\gamma(\sfM;\mu)$ is strongly convex in $\mu$, for 
      %      any $\gamma > 0$.
If $\gamma > 0$, then $\mathcal U_\gamma(\sfM;\mu)$ is strongly convex in $\mu$.

    \end{prop}
	%oli7: I can do a better job here; I was feeling rushed when I wrote it.
    \begin{proof}
	%joe6*: Is it obvious/well known that \inconsist is
	...
	%oli7: The proof that $\inconsist$ is convex, was already in the appendix, but I forgot
	...
	%oli7: minor updates for clarity
		$\inconsist(\sfM; \mu)$ is convex in $\mu$  (\Cref{thm:inc-convex}), $(-\H)(\mu)$ is strongly convex in $\mu$, and $\sum\alle \E_{x\sim \mu_X} \H(\bp(x))$ is linear in $\mu$. Therefore a non-negatively weighted linear combination of the three must be convex in $\mu$ \todo{Insert Reference}, and strongly so when the coefficient on $-\H$ ($\gamma$) is positive \todo{Insert Reference}.
	\end{proof}
...
%acknowledge that a cpt 
% results in a distribution at its target, whose entropy depends on
% the distribution at its 
% source. Therefore, the cpt results in a different constraint,
% depending on what the distirbution 
% at the source is (the cpt counterfactually contains information
...
% about the distribution, we have to control for the fact that cpts
...
%of the form X -> Y and point out that the cpt for X gives us the
%actual distribution on X, but the cpt lets us detemine the marginal
...
%I still don't understand it).                  
		That is, so long as we take each $\alpha_L = 1$, the
                distribution defined by the cpts of $\Gamma(\mathcal
...
%joe6
%\Pa_X \!\!=\!\! \vec y)$ to simply be the cpt at $X$.
\Pa_X \!\!=\!\! \vec y)$; that is, $\phi_X$ is determined by the cpt of $X$.
...
PDGs: we take the factors to be defined by the cpts.
