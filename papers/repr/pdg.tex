...
%	We give the definition below for the finite case.  
	...
	\begin{defn}\label{def:hypermodel}
		...
		\begin{itemize}[nosep]
			...
			\item $\mathcal L : \mathcal P (2^{\cal N} \times 2^{\cal N}) \times \mathrm{Id}$~~is a (multi)-set of directed links from one subset of $\cal N$ to another.
...
%	\begin{defn}
	Here $\underline\Delta(X)$ is the collection of sub-distributions on $X$ --- i.e., distributions on $X \cup \{*\}$, where $\{*\}$ is a `null' element. We will call a \MN\ is \emph{strict} if $\bmu$ assigns no mass to $\bullet$. The examples we have seen thus far are strict, but we explore the implications carefully in section \ref{sec:full-model}.
%	\end{defn}
