...
%joe1: you need to send me this file
...
\begin{document}
	...
	\begin{abstract}
%joe1: this abstract doesn't describe what we do, nor does it make a
%good sales pitch. Rewrote
\commentout{
          Graphical models cleverly compress probability
distributions over joint settings of random variables,
by making use of independence assumptions. 

However, because they model probability distributions, they are not
expressive enough to represent mental states of agents who might not
have globally consistent beliefs.  }
We introduce a new type of graphical model, \emph{Probabilistic
  Dependency Graphs} (PDGs).  They can capture inconsistent beliefs in a
natural way and are, in a precise sense, more modular than standard
approaches such as Bayesian networks (BNs), in that they make it easier for
a modeler to incorporate new information without having to restructure
the reprsentation of current information.
%joe1*: this is a point you should trumpet loudly
As we show, when viewed as a PDG, a BN $G$ represents many
distibutions,  we can recover the one given by the semantics of BNs as
the one that maximizes entropy among all the distributions consistent
with $G$ viewed as a PDG.  
%joe1: we may want to say a bit more, or beef up this sentence
We show by examples how PDGs are a natural modeling tool.
}
...
%tell a story.  This is not telling our story.  I've rewritten the
        %whole beginning
        \commentout{
	We introduce the \ModelName\ (\MN), a directed graphical model
        for specifying local beliefs. They are strictly more
        expressive and modular than existing directed graphical
        models, and in particular this allows them represent
	...
	Most graphical models eliminate the possibility of inconsistency by fiat, %so as to better represent probability distributions. 
	...
	and	recasts standard algorithms such as belief propagation
        and conditioning on evidence as resolutions of inconsistency
        (\cref{sec:algorithms}).
        }
%joe1: \end{commentout}	
...
%approaches.  
Bayesian networks (BNs), factor graphs, and Markov networks.  Why does
the world need one more?

Our original motivation for introducing PDGs was to be able to capture
inconsistency.  While being inconsistent is clearly not a good thing,
we wanted to be able to model the process of resolving inconsistency;
to do so, we had to be able to model the inconsistency itself.  But our
approach to modeling inconsistency has many other advantages.  In particular,
as we shall see, it gives us greatly improved modularity; we can add
new information to a graph without having to change what's already
there.  Moreover, PDGs extend BNs in a natural way.

We start by giving some examples that motivate PDGs and display some
of their advantages.
	%%% ATTEMPT 1
...
%	Despite their flexibility, \MNs\ perhaps counter-intuitively offer a way to recognize when something has gone wrong, and 
	...
	The modifications we make bring graphical models in line with
...
%mention it, you need to add references.
        other notions of locality in mathematics, such as manifolds
        and sheaves, in which multiple restricted pictures can be
        glued together to form something with global structure. 
...
%	skirting computational costs and enabling concept formation, without also sacrificing interpretability, as factor graphs do (see \cref{sec:factor-graphs}). }
	...
	\begin{example}\label{ex:guns-and-floomps}
		You arrive in a foreign country known for having very
%joe1: slight wording change
%                clear laws. From prior reading, you have subjective
                %                probability 0.95 that owning guns is against the
                          clear laws. From prior reading, you ascribe
                probability 0.95 to owning guns being against the
                law. Upon landing, you end up talking to some
%joe1
      %                teenagers who use the local slang---after which you
%                believe with 10\% probability that the law prohibits
                teenagers who use the local slang, after which you
                believe that with probility .1, the law prohibits
                floomps. 
		
%joe1
%		Let's try to represent this as a Bayesian
%            Network. Make a graph with two nodes: let $F$ be the
%            binary variable (taking values $\{f, \overline f\}$) 
 The obvious way to represent this as a BN involves two binary random
 variables, $F$ (taking values $\{f, \overline f\}$), indicating   
                the legality of floomps, and $G$ (taking
%joe1: for consistency
                %   values $g, \overline g$) indicate the legality of guns.
          values $g, \overline g$), indicating the legality of guns.  
	  The semantics of a Bayes Nets offer us two choices:
%joe1
%          assume that the two are independent, or choose a
          %                direction and make up some numbers for the table.
we either assume that $F$ and $G$ are independent, and then give
priors for $F$ and $G$, or we choose a
direction of dependency, in which case we must give a conditional
probability table.           
As there is no reason to believe that either variable depends on the
%joe1:
%other, we opt for the latter, resulting the following ``network'':
other, the natural choice is to asume independence, giving us the
following BN:
...
%joe1
%		You later discover that ``floomp'' is likely to be
Now suppose that you later discover that ``floomp'' is likely to be
                another word for gun, and come to believe that if
%joe1
%                floomps are legal (resp. illegal), then there's a 92\%
%                chance guns are as well; let $E$ be the Conditional
                floomps are legal (resp., illegal), then there's a 92\%
                chance that guns are as well.
%joe1: I don't like this notation.  It makes E seem like a random
%variable, just like F and G.
%joe1
                %                let $E$ be the Conditional
%              Probability Table (CPT) associated to this realization
                Let $E$ be the conditional
              probability table (cpt) that describes  this belief.
 first reaction might be to simply add this conditional information by
%joe1
% adding $F$ as a parent of $G$ and incorporating $E$ into the Bayes
% Net, but it is not clear what to do with our original CPT on $G$. 
 adding $F$ as a parent of $G$, and then associating the cpt T with $G$.
But then what should we do with the original prior probability we had
...
%	In fact, $E$ conflicts with the other two tables, in the sense
%        that there is no joint distribution on $\{F, G\}$ such that
%        all three tables are accurate.
%		Therefore, there is no Bayes Net that contains all
%three pieces of information exactly: one has to resolve the
%inconsistency first. 
It is easy to check that there is not probability distribution that is
consistent with the two original priors on $F$ and $G$ and the cpt
...
%Note that it may not be in your best interest to sort this out right away.
%The choice of resolution may be clearer if you can get confirmation
%that guns are indeed floomps, or read the laws more carefully---it
However, it may not be reasonable to sort this out right away.
How to resolve it may be clearer if you can get confirmation
that guns are indeed floomps, or read the laws more carefully.
%joe1: this seems redundant
%---it  may be worth sitting on the inconsistency so that you
%      can resolve it properly later, rather than resolving
%                it as best you can immediately. 
...
%joe1
%By contrast, consider the corresponding \MN, in which the CPTs 
By way contrast, consider the corresponding \MN.  In a \MN, the cpts
are attached to edges, rather than nodes of the graph.  
%joe1: why switch from ``edges'' to ``links''.  And why use the (nonstandard)
%notation L: A -> B
%We will represent the CPT for a link $L \!\colon\! A\!\to\! B$ as matrices
%matrices $\mat L$, whose element $\mat L_{a,b}$ at row $a$, column $b$
The cpts associated with an edge $L$ is a matrix
matrices $\mat L$, where the element $\mat L_{a,b}$ at row $a$ and column $b$
...
%joe1
%	        , we introduce a \emph{unit variable}, $\mathsf 1$, or
%                `$\mathrm{true}$', which always takes its unique
                %                value, $\star$.
               	        , we introduce a \emph{unit variable},
                $\mathsf 1$, which takes on only one possible value,
                which we denote $\star$.
%joe1: added
                Thus, we have the following
                \MN, where the edges from $\mathsf$ to $F$ and $G$ are
                associated with the prior probabilities of $F$ and
		...
		The original state of knowledge consists of all three
                nodes and the two black edges from $\mathsf 1$. This
                is like our Bayes Net from before, except we no longer
%joe1
%                assume $F$ and $G$ to be independent% (but will later
                %                                % if necessary)
assume $F$ and $G$ to be independent; 
		we merely record the constraints imposed by the
                given probabilities. 
	
                %joe1: shrunk this to a sentence; this is indeed misplaced.
\commentout{
...
%joe1: $E$ is not a link; it's associated with a link!
%	$E$ can be added directly to the model as a link $F$
%to $G$
%without changing the meaning of the link from $\mathsf 1 \to
%G$, which we retain: the added modularity lets us simply include
...
of the information (the graph in Figure~\ref{fig:gun-floomp-diagram}
...
%        belief, in which it is possible for there to be inconsistency,
%        can also be valuable even when there isn't any.  
%	Unlike other models, which require special coloring of nodes,
%\MNs\ also cleanly represent `under-constrained' beliefs, such as
%conditional ones, without modification. 
%	Our next example illustrates both, and highlights one more way
        %in which BNs fail to be as modular as we might hope.
The modularity of PDGs is useful even wehn there is no inconsistency,
as the following example shows.        
	
	\begin{example} \label{ex:planet}
%joe1: a bit too wordy
%	  Suppose we have a belief about how size and composition affect
          %        the habitability of a planet: say we're astrobiologists, and
%        we have some sense of how likely we are to find life on a
%        given planet, supposing we knew its size (big or small) and
%        its composition (mostly made of rocks vs gas), resulting in
%        the CPT from \cref{fig:planet-initial}. 
...
likely we are to find life on a
        ...
        in the cpt in Figure \ref{fig:planet-initial} (where $S$ and
        $C$ standard for ``Size'' and ``Composition''; ignore the
        faint node labeled $W$ for now): 
		
		\begin{figure}[h]
			\centering
			\scalebox{0.8}{
				\begin{tikzpicture}[center base]
				\node[dpadded,light pad,fill
                                  opacity=.2] (SS) at (0.2, 1.2)
%joe1*: please remove the coloring; it's unnecessary, and a
%distraction.  Please also use C and W for consistency with what's below.
                                %                                {Size};
                                                                {$S$}; 
...
Ignoring the faint ``water'' node for now, the diagram on the left
%joe1: you have to say more about what a conditional Bayes net is and
%add a reference; I've now done this
%corresponds to the \emph{conditional} Bays Net on the left---note that
%the nodes `Size' and `Composition' would need tables of their own to
%be fully interpreted as a Bayesian Network. Without any such beliefs,
%we would have to use a \emph{conditional} Bayes Net, and explicitly
%joe1: why talk about ``type level''?? You're throwing unnecessary words
%at the reader.  
%mark both nodes as `observed'---a rigid distinction enforced at a type
%level which is unnecessary in \MNs. 
corresponds to the \emph{conditional} Bayes Net \cite{KF09} on the
left.  While a BN would require us to associate prior beliefs with Size and
Composition, a conditional BN allows us to view these nodes as
observations; observation ndoes do not have associated beliefs.
In the \MN, since there is no edge leading to Size and Composition, we
do not require a cpt; we do not need a special distinction between
observation nodes and other nodes.
		
		\begin{vfull}
	A proponent of BNs will have no trouble getting this to work
        by using \emph{conditional} BNs, which represent conditional
        distributions rather than joint ones, with two slight
        inconveniences: (1) they would have to commit which variables
        are conditioned up-front, and so adding or removing
        information changes the type of mathematical object
        represented, and (2) there is no graphical distinction between
        having a distribution over $\sf Size$ or not, and so we have
        to color certain nodes and keep track of this additional
        information. \MNs\ have neither of these issues, but the
        bigger payoff is in representing \textit{over-constrained}
        states. 
		\end{vfull}

%joe1                
%Now our biologist friend now reminds us that life requires water, and
%computes probabilities for the existence of life on a planet, with and
%   computes probabilities for the existence of life on a planet, with and
%without water. We trust this friend completely. We'd like to introduce
%a new node, `Water', but if we make it a parent of `Life', because we
%don't know how this information interacts with size, and composition;
...
without water. Of course, wewould now like to incorporate this
...
will have a problem with a BN representation of the situation, since
...
variables  $S$, $C$, and $W$.
%don't know how this information interacts with size, and composition;
%neither are we prepared to give a probability of live given a full
%description of the three, and this table would be much larger.% 
...
%\footnote{If \textit{Size} and
%                          \textit{Composition} each had
%                          $\approx\sqrt{N}$ elements, and
%                          \textit{Water} had $\approx N$ elements, it
%                          would be $O(N^2)$ to store a full joint
%                          table, compared to $O(N)$ for the two
%                          individual ones.} %At this point the
%                                %Bayesian Network is not at all a
%                                %convenient way of storing the
%                                %information at hand. 
%		Let $S, C, W, L$ be the respective shortenings of our
%variables.
%joe1: After talking about edges and links, now you're talking about
%arrows!  Please be consistent!
%\MNs\ allow us to interpret the last arrow individually, using a
%picture like this:
In a $\MN$, in a PDG, we can just add $W$ and an edge from $W$ to $L$,
and associate with the edge from $W$ to $L$ the cpt describing the
probabilities provided by the biologist.  
...
%joe1*: I don't understand the 	story here.  Let's discuss it when we meet
	...
	\begin{example} \label{ex:teacher}
		...
		You have some distribution $G_B$ of likely grades for
                students in $B$, but you also have a good model of the
                distribution $G_A$ of grades a student from $A$ might
                get. We want to model this epistemic state, and now
                present three alternative modeling choices. 
...
%	\end{enumerate}
	...
	\section{Formal Construction and Syntax}\label{sec:formal+syntax}
	
%	We give the definition below for the finite case.  

	%joe1
%	We have seen some examples of \MNs, but elided the details; we
%        now begin a more careful treatment of \MNs\ as mathematical
      %        objects.
...
%	\moveme{Rather than representing a probability distribution, \MNs\ can be thought of as \emph{constraints} on distributions.}
	...
	Compared to a Bayesian Network, a \MN\ still consists of a directed graph, and the edges still inform conditional probability tables, but now each edge is interpreted individually. Consider the graph
...
%\item $\Li \subseteq \N \times \N \times \mathrm{Label}$~~is a set of
%directed links, each of which has a source and target in $\N$.% 
%				\footnote{Though likely to be
%                                  inconsistent, a \MN\ might admit
%                                  parallel edges, in which we need to
%                                  tag edges with a label to identify
%                                  them, but in most cases, we will use
%                                  empty labels.}
%joe1: why not use the much more standard \E rather than \L
\item $\L$ is a set of directed edges
			\item $\V$ associates each node $N \in
                          \mathcal N$ with a set $\V(N)$ or $\V_N$,
%joe1
%      representing the values that node $N$ can
          representing the values that variable $N$ can
                          take. 
...
%         \item $\bmu\colon \big(\!({A,B,\ell})\colon \! \Li \big) \to
                          %         \V(A) \to \underline\Delta\V(B)$
			% $\colon\!\big(\!({\bf A,B})\colon \! \Li
			% \big) \to \prod\limits_{A\in \bf A} \!\!
			% \V(A) \to
			% \underline\Delta\left[\prod\limits_{B \in
			  % \bf B}\!\!\V(B)\right]$
%joe1: it's much more standard to use X nd Y for variable names in BNs.
                        \item $\mu$: for each edge $(X,Y)$, $\mu$
                          associates a distribution on $B$ with each
                          element $x \in \V(X)$.
			%%% Above is the type of $\bmu$. I think it's
%%% important to have it there. 
%			gives a (sub)-distribution over $\V_B$ given
%each possible value of $\V_A$ and for every link $({A,B,\ell}) \in
                          %\mathcal L$.
...
%				\note{The type of $\bmu$ doesn't take up much space and answers questions about the words above. I still want to keep it.}
		\end{itemize}
		
	\end{defn}

        %joe1*: NO.  If you must introduce this, it should go in a
        %later section on extensions of PDGs.
	\commentout{
	Here $\underline\Delta(X)$ is the collection of
        sub-distributions on $X$ --- i.e., distributions on $X \cup
        \{\none\}$, where $\none$ is a `null' element. We will call a
        \MN\ \emph{strict} if $\bmu$ never assigns mass to $\none$. So
        far only example \ref{ex:teacher} makes use of this, but we
        explore the implications carefully in section
        \ref{sec:full-model}.
        }
...
%			\note{The type of $\bmu$ is $\big(\!({\bf A,B})\colon \! \Li \big) \to \V(A) \to \underline\Delta\V(B)$. It doesn't take up much space and answers lots of questions about the words above.}
%		\end{itemize}
%
%	\end{defn}
	...
	we will often use two additional bits of syntax to represent
        common constraints:  
	\begin{itemize}
%joe1: all variables are unary          
%		\item A special unary variable $\sf 1$ that always
          %                  takes its unique value $\star$. It is used to
          	\item A special variable $\sf such that $\V(\sf) = \{\star\}$.
%                  represent unconditional distributions, as in
%                  \cref{ex:guns-and-floomps,ex:smoking}. For strict
%                  \MNs, this is no different from any other variable
%                  that can take only a single value.  
                  %joe1: cutting this
\commentout{
		...
		\item Double-headed arrows $A \doubleheadrightarrow B$
                  indicating the degenerate special case of a cpt
                  corresponding to a deterministic function $f : X \to
%joe1
%         B$, the distribution assigning a point mass to
    Y$; that is, we use a double-headed arrow if, for each $x \in \V(X)$,
    the distribution assigns probability 1 to some element $y \in \V(Y)$.
%joe1*: I don't know what the \none is there; if it's important, it
%needs explanation.  You should also use the notation that you've defined
    %    assigning a point mass to
%    $f(a)\neq\none$ for each $a \in A$.$f(a)\neq\none$ for each $a \in A$.
%                  $f(a)\neq\none$ for each $a \in A$. 
\commentout{
    \begin{examplex}
...
%joe1
        %	An astute reader is probably wondering how \cref{def:model}
        %        can capture the multi-tailed arrows we needed in
We can now explain how we capture the multi-tailed edges that were
used in         
...
%\cref{ex:planet,ex:smoking} the answer that this notation is
Figure~\ref{ex:planet,ex:smoking}. This notation is
        really shorthand for the result of adding a new node at the
        junction representing the cross-product its parents, and then
        a projection. 
       For  instance, the \MN\ in \cref{ex:planet} is
...
the edge from $S \times C$ to $S$ gives probability 1 to $s$ when
given input $(s,c) \in S \times C$, and similarly for the edge from $S
\times C$ to $C$.  

%joe1*: ``Composition'' comes out of the blue.  If you think that more
%needs to be said here, let's discuss it.  All you would need to say
%is ``see \cref{sec:hyper-convert} for more discussion''.  But I'd
%prefer to cut it altogether
\commentout{
There are some additional technical concerns regarding
        composition; see \cref{sec:hyper-convert} for the full
        example. The choice to formalize \MNs\ this way is a design
        consideration that will make some things cleaner, but we can
        just as well formalize multi-tailed edges directly: 
}

%joe1*: NO!!  If you are going to do this, it needs lots more
%discussion.  It can't just come out of the blue
\commentout{
	...
	\Cref{thm:hyperequiv} shows \MNs\ and \MNH s to be equivalent, though in different cases one may seem more natural than the other.
...
%joe1*: I cut this whole subsection; see below for more specific
%comments
\commentout{
...
%joe1: I  also have no clue of what this means
%or why we need it.
	...
	be the settings of $S$ with and without $\none$, and $\V(\sfM), \V_\none(\sfM)$ be shorthand for the settings of all variables, $\V^\sfM(\N^\sfM)$ and $\V_\none^\sfM(\N^\sfM)$, respectively.\footnote{We leave ${\sf 1}$ out in the second case because it cannot take $\none$, and the sets are isomorphic either way.}
	
	For instance, if $M$ is as in \cref{ex:guns-and-floomps}, 
	$W_M = \{fg, \bar f g, f \bar g, \bar f \bar g \}$ and 
	$W_\sfM^\none = W_\sfM \cup \{ \none g, f \none, \none\none \}$. As this $\sfM$ is strict, all of these additional worlds get zero probability. \todo{Replace with \cref{ex:teacher} once finalized for a more illustrative example.}
...
%	In each examples in the introduction, something has happened
In each of the examples in the introduction, something has happened
        causing us to combine pieces of diagrams, or make
        additions. The formal underpinning of this process is a
        \emph{union} of \modelnames. 
...
%	We view of \modelnames\ as being a representation of beliefs
%        in and of themselves, rather than a compression of something
%        more fundamental such as a probability distribution. That
%        said, it is still incredibly important to interpret them in
%        various ways, as this will give us ways of comparing them to
%        other graphical models, prove things about \MNs, and give a
%        lot more intuition about how they work. 
There is more than one way of giving semantics to a \MN.  We discuss
three approachs below.  The first is perhaps the most natural: associating
with a PDG a set of distributions. This set will be empty if the PDG
is inconsistent.  The second approach also associates with a PDG a set
of distributions, but associates with each one a \emph{weight}
(cf.~\cite{HL12}), which intuitively can be thought of as a measure of
the ``goodness'' of that distribuiton.  For an inconsistent PDG, some
distributions may still be better than others.  Intuitively,
distributions that are consistent with small perturbations of a PDG
are better than ones that are consistently only with a large perturbation.
Our third approach chooses a particular distribution from the weighted
set of distributions defined by the second approach, this associating
...
%joe1
%\subsection{As Sets of
\subsection{PDGS as sets of
          distributions}\label{sec:set-of-distribution-semantics} 
	If we think of a \MN\ as kind of like a BN except without
        independence assumptions, then just as a BN represents a
        single distribution on joint settings of its variables, a
        \MN\ might be thought of as representing the set of all
        distributions that are compatible with it exactly, with or
        without the independence assumptions. 
	...
	\begin{defn} \label{def:set-semantics}
		...
		~\text{$\forall (A, B,\ell) \!\in\! \mathcal L$, $a \!\in\!\mathcal V_A$, $b \!\in\! \mathcal V_B$} \end{array}\!\!\! \right\}\]
	\end{defn}
	...
	\begin{example}
		...
		Though true for \MNs, subgraph of a BN may or may not be a BN itself, and may or not even be compatible with the original even if interpreted as a conditional BN, due to the independence assumptions. Once again, the standard model struggles more to deal with structural changes to the underlying graph.
	\end{example}
	
	
	We can also now finally give a formal definition of consistency.
	\begin{defn} \label{def:weak-consistent}
		$\sfM$ is consistent if there is some $\mu \neq \mu_\none \in \bbr{\sfM}_\Set$, and strictly consistent if there is some $\mu \in \bbr{\sfM}_\Set$ that assigns no mass to any world where a variable takes $\none$.
	\end{defn}
	
	\note{By contrast, $\sfM$ is over-constrained if there were \emph{some $\bmu$} of the same type as $\bmu^\sfM$ such that $\bbr{\N^\sfM, \Li^\sfM, \V^\sfM, \bmu}_\Set = \emptyset$, and under-constrained if every $\bmu$ of this type yields $\big| \bbr{\N^\sfM, \Li^\sfM, \V^\sfM, \bmu}_\Set \big| > 1$.
	}
...
%wrong place for this discussion.  We should be focusing purely on
	...
	is the set of distributions over joint settings of $X$ and
        $Y$, whose marginals $\mu_X$ and $\mu_Y$ are each compatible
        with some distribution in $R$ and $S$ respectively.  
	
	This it the natural extension of an intersection to distributions on different, possibly overlapping sets --- in particular, if $X = Y$, then $R \dcap S$ = $R \cap S$ and if $X \cap Y = \emptyset$, then $R \dcap S = R \times S$. 

	\begin{prop}
		$\bbr{M}_\Set \dcap \bbr{M'}_\Set = \bbr{M \cup M'}_\Set$.
	\end{prop}

	From the two special cases of $\dcap$ discussed above, one can see that adding new links, (which we will see correspond to observations in \cref{sec:belief-update}), cuts down the set of possible distributions, just like conditioning, and adding new variables to a consistent model freely increases the number of valid distributions like one would expect. Note that all of this is done by combining \MNs, which is only possible because of their extreme modularity.
	
	There is one final property of $\bbr{-}_\Set$ that may be interesting on its own, and will become useful later:
...
%joe1
        %	\subsection{As Weighted Distributions}
        	\subsection{PDGs as sets of weighted distributions} 
...
%joe1                
                %	As far as $\bbr{-}_\Set$ goes, all
                %inconsistent \MNs\ are        equivalent, which is
                %unsatisfying --- for instance, there's a 
Our first appraoch treats all inconsistent \MNs\ as equivalent, 
which is unsatisfying.  For instance, there's a
        difference between thinking a 60\% head-biased coin will land
        heads 63\% of the time, as opposed to 0\% of the time. %and
                                %estimates of distributions from
                                %samples are necessarily going to be a
                                %little off.  
        %joe1: removed paragraph break
	%
% 	We therefore move to generalize the semantics in the previous
 	We now generalize the semantics in the previous
        section.
...
%        For a candidate distribution $\nu$, rather than
        %        assigning it a 0/1 score of whether or not it is consistent,
        %        we give it a continuous one, intuitively encoding how far we
%                have to nudge things to make it consistent. 
%joe1: I don't know what it means to ``express'' an edge; I would cut this
        %        actually has expressed, and we will measure similarity by
        We can view the previous semantics for a PDG $G$ as assigning each
distribution a score of either 1 or 0, depending on whether it is
consistent with $G$.  We now give each distribution $\nu$ a score (weight)
in $[0,1]$, 
        intuitively encoding how far we
        have to perturb $G$ to make $\nu$ consistent.
        ...
        %should be no tal about temperature.  
        We want to
        score distributions only on the edges
        actually has expressed, and we will measure similarity by
        relative entropy: the number of bits required to encode the
        $\bmu^\sfM$ given an optimal code for $\nu$.
%joe1: this seemed to dangle
%        Perhaps one might think that this seems backwards: we're giving a  
 		
	\begin{defn}\label{def:zeta-score}
		The (strict) inconsistency, $\zeta$, of a \MN\ $\sfM = \mnvars[]$ with respect to a distribution $p \in \Delta[\V_{\none}(\sfM)]$ (in $\Delta \V(\sfM)$ respectively) is given by
		\[
			\zeta(M ; p) := %\inf_{p \in \Delta(W^{\mathcal V})}~
			\sum_{\substack{L \in \cal L}{L = (A,B, \ell)}}\!\mathop{\mathbb E}_{a \sim p(A)} \left[\kldiv[\Big]{\bmu_{L}(a) }{ p(B \mid A \sheq a) } \right]
		\]
		where $D_{\mathrm{KL}}$ is the relative entropy, taken between the two distributions over $B$: one given by the link from $\bmu_{A,B}(a)$ and the other given by the marginal distribution of $p$ conditioned on $A = a$, over the variable $B$. 
		
		When $\mu, \nu$ are sub-distributions, we define $\kldiv{\nu}{\mu} = \inf \kldiv{\nu'}{\mu'}$ to be the minimum possible for strict distributions $\mu' \geq \mu$, $\nu' \geq \nu$ compatible with $\mu,\nu$. 
	\end{defn}
	...
	One nice consequence of this definition is that the maximum value of $\bbr{M}_\zeta(\mu)$ is $1$, in keeping with the standard on weighted distributions \cite{halpern2015weighted}; indeed all consistent distributions have weight 1. Still, there's something missing: we've now captured inconsistency

	\begin{prop}[restate=thmzetaconvex]
		$\zeta$ is a convex function in $p$.
	\end{prop}
	As a result, $\bbr{M}_\zeta$ is a quasiconvex function, and hence there is a unique global minimum, and local optimization will find it.
...
%			= -\log \sup_{p \in \Delta[W_{\cal V}]} \bbr{M}_\zeta (p)
	...
	We may also consider the links as having different strengths: if we associate a positive `inverse temperature' coefficient $\beta$ to each edge, we can define
	\[
		\zeta(M ; p) := %\inf_{p \in \Delta(W^{\mathcal V})}~
		\sum_{(A,B,\ell) \in \cal L}~\mathop{\mathbb E}_{a \sim p} \frac{1}{\beta_L} \left[\kldiv[\Big]{\bmu_{A,B,\ell}(a) }{ p(B | A \sheq a) } \right]
	\]
	
	\begin{defn}
		A weighted \MN\ $(M, \beta)$ is a \MN\ $\sfM$ together with an inverse temperature $\beta_L \in \mathbb R^+ \cup \{\infty\}$ for each link $L \in \Li^\sfM$.
	\end{defn}
	...
	\begin{prop}
		If $M$ is consistent, $\bbr{M^{(\infty)}}^* = \bbr{M}\MaxEnt$.
	\end{prop}


	\begin{example}[continues=ex:guns-and-floomps]
		Recall the \MN\ from our first example, in figure \ref{fig:gun-floomp-diagram}. Suppose that both the of the initial links $1 \to F$ and $1\to G$ have $\beta = 1$, and we give our new observation $\beta = 10^{-3}$.
...
%	\qquad\text{for all $\mu' \in \Delta \prod \mathcal V$} \end{array} \right\}\]
	We promised way back in \cref{ex:guns-and-floomps} that we'd be able to get the same mileage out of representing maximum entropy distributions that Bayesian Networks do; to do this we also offer a way of interpreting a \MN\ as a single distribution, and relate this to the weighted distribution semantics above.	
	...
	Now that we have all stated all three formal semantics of \MNs, we can now formally state the equivalence between \MNs\ and \MNH s. 
	
	\begin{theorem}[restate=thmhyperequiv]\label{thm:hyperequiv}
		Every \modelnamehyper\ $M$ is equivalent to a \MN\ $M'$ with additional variables. That is, for each semantics $\bbr{-}$ we define, $\bbr{M} = \bbr{M'}$.
	\end{theorem}
...
\commentout{        
	...
	One final way of viewing \MNs\ is as a set of probabilistic programs, corresponding to the links. 
	...
	\[ \bbr{M}_\lambda = \Big\{ \text{paths } p = N_0 \xrightarrow{p^1} N_1 \xrightarrow{p^2}\cdots\xrightarrow{p^n}N_n \Big\} \]
	...
	\begin{example}
		...
		Conversely, factorization of a table $A \to C$ into tables $A \to B$ and $B \to C$ (i.e., a stochastic matrix factorization) is a form of simplification, and is likely to be smaller if $|B|$ is small.
	\end{example}	
	...
	\begin{defn}
		A \MN\ $M$ is \emph{strongly consistent} if every collection of paths $P \subseteq \bbr{\sfM}_\lambda$ is compatible, in that 
...
\end{commentout}
	...
	Bayesian Networks may be the most widespread graphical models, but they are certainly not the only ones. In some sense, each can emulate the others, to various degrees of ease. A large number of them can be seen in \cref{fig:model-transformations}. Here we will only a few of the most important classes; see \cref{sec:many-relations-graphical-models} for a full description of the diagram.
...
%	\subsection{Relatstion to Lower Probability Measures}
	...
	For us, belief revision is simply adding links to the picture, and then resolving inconsistencies. 
