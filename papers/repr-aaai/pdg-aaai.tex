...
In fact, \Cref{thm:bns-are-pdgs} would follow from \emph{any} choice of divergence, so the fact that PDGs extend Bayesian Network semantics does not hinge on this choice.
...
%oli19: before I had ", where x is ..."; I don't know if it is acceptable to wait
...
%oli19: later cut the text for line. Should reinstate if we drop the reference beneath it.
...
\begin{figure*}
	...
	\caption{The conversion from a PDG to a factor graph to factor graph, and vice versa, as defined in \Cref{def:fg2PDG}. The blue edges carry the (renormalized) cpds corresponding to the original factors, and product nodes (displayed as smaller grey rectangles) are inserted for every edge.}
	\label{fig:fg2PDG}
\end{figure*}
...
increasing $\beta$  makes it more important that a distribution match the given cpd. This is illustrated in \Cref{ex:overdet}.
