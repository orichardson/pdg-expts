...
%oli21: a macro for weighted factor graphs
...
%  		to each link $L$,
% %joe18: it would be fine if we could ever agree on what it meant.  (and we have tried
% % many times in the past ...)
% indicating an agent's confidence in the functional
...
$S$ of variables; in particular, we write 
...
%oli21: relatedly, are there short (sequences of) letters that would help you
% keep them straight without referring back to them? Or is (a)/(b) good enough?
...
According to $G$, \ref{item:localinfo} should be small (it would be zero were it not for the allowance for noise), while  \ref{item:globalinfo} is the information required to eliminate all noise. 
...
% not present in $\mu$
If \ref{item:localinfo} $>$ \ref{item:globalinfo}, then there are correlations in $\mu$ that allow for a more compact representation than $G$ provides.
...
%oli21: I've rewritten this sentence probably 50 times. I think finally found something
...
$\mu$ must also display further uncertainty on variables $G$ does not constrain, because a world requires
% we give $\mu$ bonus credit for displaying uncertainty about random variables $G$ does not constrain, and requires
% $\mu$ must also align with $G$'s indeterminateness by displaying uncertainty about random variables $G$ does not constrain, because $\mu$ requires
%oli21: comma
...
%oli21: this is not worth saying. My guess is over 99% of the people who
% get this far into the paper will know this, there's no vagueness, 
...
Factor graphs \cite{wainwright2008graphical,kschischang2001sumproduct},
 		like PDGs, generalize BNs and have a
...
%        $\mathcal X$;
%oli21: somehow this definiton is also wrong.
	...
	% $X_j\subseteq \mathcal{X}$, and maps
	%values of $\V(X_j)$ to non-negative real numbers.
...
%oli21: I'm envisioning some disagreement about this notation; putting this in a 
% macro to make this smoother
...
variable $X_J := \prod_{j \in J} X_j$ for every factor $J \in \mathcal J$%
%oli21: This is pretty awkward; also the discussion is not in the appendix anymore.
% I'm swapping order, as the projections are less important. The cut material
% appears in slightly altered form at the end of the definition
	% ; there are edges $X_J \tto X_j$ for each $X_j \in X_J$ (as in the appendix)%
...
normalizing $\phi_J$; finally, let \valpha{$\alpha_L = $}$\beta_L := \theta_L$. 
...
%unconditional distributions. I think your original edit introduced this ambiguity.
...
(blue) %oli21: AAAI only begrudgingly accepts color I will make sure all the 
...
$\bbr{\dg M}_{\gamma} = \gamma\,\mathcal G_{ \WFGof{\dg M} }$ and
...
%oli21: A more accurate takeaway is that if \gamma is strictly positive, then a messed
...
%oli21: Huh, I've always heard the adjective "strength" used to describe the magnitude
...
\begin{defn}
	...
	$\mathcal D$ defines a joint distribution on $\V(\N)$ by an ordered Gibbs sampler. The process of repeatedly cycling through the variables (ordered by $\prec$).
\end{defn}
...
%oli21: updating theorem statement
	...
	If $\mathcal D$ is a consistent dependency network with positive local distributions, then $\SD{\PDGof{\mathcal D}} =  \{ \Pr_{\cal D}^\prec \}$, where $\Pr_{\cal D}^\prec$ is the unique stationary distribution of the $\prec$-ordered Gibbs sampling procedure. 
\end{theorem}

It follows from \Cref{thm:dns-are-pdgs,prop:sem3} that $\bbr{\PDGof{\mathcal D}}^* = \Pr_{\cal D}$. This is comforting, but not particularly surprising; it merely states that PDGs and DNs have the same notion of inconsistency. Perhaps more surprisingly, the independencies of $\mathcal D$ are captured by the qualitative term $\IDef{}$.  
