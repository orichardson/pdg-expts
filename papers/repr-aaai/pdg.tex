...
A \emph{Probabilistic Dependency Graph} is a tuple $\dg M = (\N,\E,\V,\mat
p, \beta,\alpha)$, where 
...
Note that we allow multiple edges in $\E$ with the same source and
target; thus $(\N,\E)$ is a multigraph.  We occasionally write a PDG
%joe20
%as $\dg M = (\Gr,\mat p, \beta,\alpha)$, where $\Gr = (\N,\E,\V)$,
%and
as $\dg M = (\Gr,\mat p, \alpha,\beta)$, where $\Gr = (\N,\E,\V)$, and
...
%oli22: this last statement seems out of place in the definition
%itself; 
$L$ 
except in \cref{sec:expfam},
...
%projection}  
%%joe20
%%(again, see \citeauthor{KF09}) of the product of every $\bp$ into
...
\end{theorem}
\begthm{theorem}{thm:pdg-is-fg}
	$\bbr{\dg N}_{1}^* = \Pr_{\FGof{\dg N}}\;$ for all unweighted PDGs $\dg N$. 
\end{theorem}
...
\end{equation}
In ths case, each specification in~\ref{item:localinfo} may be weighted by differently, so that some edges are more qualitatively certain than others, and correspondingly it may be more or less important to describe them properly.
...
We now turn to extending \cref{def:PDG2fg}.
Since PDGs have two sets of weights, and WFGs only have one, 
there are more PDGs than WFGs, and so there is not a perfect way to translate back.
We have chosen here to preserve $\beta$.

\begin{defn}[PDG to WFG]
Given a (weighted) PDG $\dg M =
(\dg N, \beta)$, we take its corresponding WFG to be $\WFGof{\dg M} := (\FGof{\dg N}, \beta)$ by setting $\theta_L := \beta_L$. 
\end{defn}
...
but only for the value of $\gamma$ equal to the constant $k$ used in the translation. 
...
the inconsistency between them is the learning task \cite{doersch2016tutorial}. In all three of these
