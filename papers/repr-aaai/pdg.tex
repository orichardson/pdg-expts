...
%joe19*
\item[$\alpha$] is a function that 
associates a positive real number $\alpha_L$ with each edge $L$;
...
%joe19
%associates a positive real number $\beta_L$ to each edge $L$,
is a function that 
...
implicitly taking $\alpha_L=1$ for all edges $L$.  (The one place where
we use $\alpha_L$ is in our discussion of ...)  
\end{defn}
...
%joe19: it's not in particular, since it's not a special case
...
%joe19*: no \alphas here.  But are you claiming that you get the same
%result with \alpha_L = 1 and \alpha_L = \theta?  That's strange (and
%inconsistent with what you wrote later
%normalizing $\phi_J$; finally, let \valpha{$\alpha_L = $}$\beta_L
normalizing $\phi_J$; finally, let $\beta_L
%joe19
%:= \theta_L$.
= \theta_L$.  
