...
%joe17: undid; I don't think that they object unless they specifically
%don't allow it
...
\declaretheorem[name=Definition,qed=$\square$]{defn} %numberwithin=section
...
%joe17: this will help us see how much space we really have
%\specialcomment{vleftovers}{\begingroup\color{vleftoverscolor}}{\endgroup}
\newcommand{\vleftovers}[1]{}
...
%oli19: no setion numbers AAAI
%joe17*: Do they explicitly insist on this?  I have a fairly strong preference
%for numbering within section, and in the past, they've allowed it
...
%We provide three semantics for PDGs, each derrived from an
We provide three semantics for PDGs, each derived from an
information-theoretically motivated scoring function (on joint
distributions over the network's variables). For the PDG corresponding
...
%Seoncd why is the PDG inconsistent?  Third, why stress this, even if
...
%joe17: why did you add it?  What is a simple BN?
        \caption{A simple BN (left) and corresponding PDG (right), which can
...
\end{defn}
    ...
    %oli19: again, justify spacing is messed up. Changing this...
    $\pdgvars[^{\dg M}]$
    % $\N^{\dg M}$, $\Ed^{\dg M}$, and so forth,
        for the components of $\dg M$, so that we may reference one 
    %oli19: commented line below, seems unnecessary
    % (e.g.,$\Ed^\dg M$)
         without naming them all explicitly. 
...
%oli19:  merged the once-a-definition with its header above
...
which $\Pr_\Phi$ minimizes, called the \emph{variational Gibbs free energy}.
...
probabiility distribution over the variables associated to each
factor.  
...
\begin{figure*}[htb]
	...
	\caption{The conversion from a PDG to a factor graph to factor
...
%be expanded a bit.
original factors, and product nodes (displayed as smaller grey
	rectangles) are inserted for every edge.} 
	\label{fig:fg2PDG}
\end{figure*}
...
%oli19: copying and re-doing for line-break
we have 
...
\bbr{\dg M}(\mu) =  \sum_{\mat w \in \V(\dg M)} \mu(\mat w) \in \! \Bigg\{
...
%As $\gamma \to 0$, the local terms
The scoring function that we use for PDGs can thus be viewed as
extending ${\mathcal G}_{\Phi}$ by including what we have called the local
regularization term.
...
%deterministic.  
%By contrast, $\beta_L$ controls both the likelihood and the local
%uncertainty together: 
%increasing $\beta$  makes it more important that a distribution match
%the given cpd. This is illustrated in \Cref{ex:overdet}. 
...
% exist. I am totally on board with the scores representing an agent's
% second-order 
% beliefs, but the beliefs themselves must be connected to the
% second-order beliefs. 
% In this case, we have a natural definition of the first-order
% beliefs as the minima 
...
mic However, the analogue of \Cref{prop:consist} does not hold
