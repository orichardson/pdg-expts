...
\begin{proof}%[Proof of Theorem~\ref{thm:bns-are-pdgs}]
	...
	%
	\textbf{Fact 2:}
	In $\Gamma(\cal B)$ is exactly one hyperedge $E$ for each node $X_i$, from the set $\Pa(X_i)$ to $X_i$
	...
	As a result, we discover that $\H^{\Gamma(\mathcal B)}(p)$, the sum of these terms, is non-negative, and equal to zero if and only if \emph{every} variable is independent of all previous variables given its parents, according to $p$. 
	...
	\textbf{v2. Uniqueness first, via convexity.}	
