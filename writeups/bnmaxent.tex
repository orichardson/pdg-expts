...
\begin{proof}%[Proof of Theorem~\ref{thm:bns-are-pdgs}]
	...
	Because relative entropy is strongly convex, $\H^{\Gamma(\mathcal B)}$ is a sum of strongly convex functions (from the final line of the proof of \Cref{lem:bnmaxent-component}), and hence strongly convex itself. Therefore, because the set of distributions that match the cpts is convex (\Cref{lem:convex}), $\H^{\Gamma(\mathcal B)}$ has a unique global minimum $\mu^*$ at zero. Because $\Pr_{\cal B}$ satisfies the independences from \Cref{lem:bnmaxent-component}, we must have $\H^{\Gamma(\mathcal B)}(\Pr_{\cal B}) = 0$, and so by uniqueness, $\Pr_{\cal B} = \mu^*$.
\end{proof}
