...
\begin{defn}%[database]
    ...
    \begin{tabular}{@{~~~}r@{}lp{13cm}}
    	...
    	$\sch$ & \oftype{\Idx \to \Attrs \to \Set,} & called the \emph{schema} of $\D$, assigns a multi-set of attribute names $\Attrs$ to each table name (corresponding to the table columns).
...
\begin{figure}
	...
	\caption{A simple relational database $\D$ \subref{subfig:tables}, its schema \subref{subfig:schema}, and an encoding as a PDG \subref{subfig:indexed} with a ``variable'' for a row index. Note that we do not interpret two arrows into an attribute (such as those from $R_1$ and $R_2$ to $B$) as a joint dependence, so this directed representation should not be interpreted as a BN, but rather as a PDG.} \label{fig:sketches}
\end{figure}
